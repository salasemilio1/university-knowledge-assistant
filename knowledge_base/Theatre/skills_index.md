# Skills Index — Theatre
**Institution:** Southwestern University, Georgetown, Texas
**Last Ingested:** 2026-03-30
**Ingestion Model:** Claude Sonnet 4.6
**Document Count:** 17
**Degree Paths Covered:** B.A. (Theatre Arts), B.A. (Theatre Performance and Production), Minor in Theatre

---

## SECTION 1 — METADATA BLOCK

This index covers the Theatre Department at Southwestern University's Sarofim School of Fine Arts. Three degree paths are available: B.A. in Theatre Arts (35-36 credits, designed for students with a second major or multiple minors), B.A. in Theatre Performance and Production (39-45 credits, designed for focused theatre study), and a Minor in Theatre (5 courses). The department is housed in the Sarofim School of Fine Arts Building.

---

## SECTION 2 — DOCUMENT REGISTRY

### Theatre Department Overview
- **filename:** `Theatre.txt`
- **document_type:** general
- **degree_relevance:** All
- **time_sensitive:** false
- **catalog_year:** N/A
- **description:** Provides a high-level overview of the Theatre Department's mission, philosophy, and degree offerings at Southwestern University. States the department offers a Bachelor of Arts (B.A.) providing grounding in theatre history, dramatic literature, performance, design, management, and technical theatre. Notes that Work Study-eligible students may find jobs in faculty assistant, box office staff, and backstage crew roles. References alumni success stories and career preparation. Mentions the department is part of the Sarofim School of Fine Arts. Includes brief mention of financial aid and work-study connections for theatre.
- **critical_data:**
  - Degree offered: Bachelor of Arts (B.A.)
  - Focus areas: history, dramatic literature, performance, design, management, technical theatre
  - Work Study positions available: faculty assistant, box office staff, backstage crew
  - Students interested in work-study positions should inquire through the department head
  - Part of the Sarofim School of Fine Arts
- **retrieval_triggers:**
  - "what does the theatre department offer"
  - "what can I do with a theatre degree"
  - "theatre at Southwestern"
  - "is there a theatre major at SU"
  - "theatre BA overview"
  - "work study jobs in theatre"
  - "theatre department mission"
  - "theatre alumni careers"
  - "what is the Sarofim School of Fine Arts"
  - "can I get a job in theatre at SU"
  - "backstage crew work study"
  - "box office job theatre"

---

### Majoring and Minoring in Theatre
- **filename:** `Majoring___Minoring.txt`
- **document_type:** requirements
- **degree_relevance:** B.A., Minor
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** The primary requirements document for the Theatre Department. Covers all three degree paths: B.A. in Theatre Arts (35-36 credits), B.A. in Theatre Performance and Production (39-45 credits), and Minor in Theatre (5 courses). Lists all required Theatre Core courses shared by both B.A. tracks: THE72-164 Design Fundamentals, THE72-183 Fundamentals of Acting, THE72-211 Theatre Laboratory (six semesters), THE72-234 Theatre History, THE72-921 Theatre Capstone: Research Methods, and THE72-923 Theatre Capstone. Specifies elective and selection requirements for each path, including upper-level course requirements and design course choices. Includes the minor requirements (THE72-164, THE72-234, plus three additional courses). Provides the department's course numbering convention: 72- = Design, 73- = Performance, 74- = Theatre History and Criticism. Notes that credits from independent studies and academic internships may not be used to fulfill Theatre Major requirements.
- **critical_data:**
  - B.A. Theatre Arts: 35-36 total credits; requires core + one devising/playwriting elective + one design elective + one upper-level elective
  - B.A. Theatre Performance and Production: 39-45 total credits; requires core + six additional upper-level courses
  - Minor in Theatre: THE72-164 + THE72-234 + three additional courses
  - Shared core: THE72-164, THE72-183, THE72-211 (×6), THE72-234, THE72-921, THE72-923, THE72-193
  - Independent study and academic internship credits do NOT count toward major requirements
  - Course prefix: 72- = Design; 73- = Performance; 74- = History & Criticism
  - 100-level = introductory; 200-800 = may be appropriate for all levels unless prerequisite indicated
- **retrieval_triggers:**
  - "how many credits for theatre major"
  - "theatre major requirements"
  - "what courses are required for theatre"
  - "theatre minor requirements"
  - "difference between theatre BA options"
  - "theatre core courses"
  - "can independent study count toward major"
  - "what is Theatre Laboratory"
  - "do I need a capstone for theatre"
  - "theatre course numbering"
  - "can I double major in theatre"
  - "how many upper-level theatre courses do I need"
  - "what design course do I have to take"
  - "THE72-211 requirements"
  - "theatre arts vs performance and production"

---

### Design Courses
- **filename:** `Design_Courses.txt`
- **document_type:** course_info
- **degree_relevance:** All
- **time_sensitive:** false
- **catalog_year:** N/A
- **description:** Describes all theatre design-track courses available in the department. Covers Theatre Company (six hours/week production support), Design Fundamentals (THE72-164, open to first-year and sophomore students only without instructor permission), Theatre Laboratory, Theatre Production Practicum (one credit per production, max four credits, non-majors only), Makeup Design, Theatre Arts Management, Costume Design, Scenic Design, Theatre Sustainability, Lighting Design, Audio Technology and Design, Scenic Elements and Stage Properties, Scene Painting, and Stage Management for the Theatre. All courses emphasize hands-on, practical learning in technical theatre. Theatre Sustainability is cross-listed with Environmental Studies (49-814). Includes prerequisite and enrollment restriction information where applicable.
- **critical_data:**
  - THE72-164 Design Fundamentals: open to first-year and sophomore students only; juniors/seniors need instructor permission
  - Theatre Production Practicum: max 4 credits; non-majors only; requires being assigned to a production position
  - Theatre Company: 6 hours/week commitment; covers scenery, costumes, lighting, sound, front-of-house, stage management
  - Theatre Sustainability: cross-listed with Environmental Studies 49-814
  - Stage Management for the Theatre: project-oriented, hands-on stage management experience
  - Audio Technology and Design: covers mixers, effects processors, amplification, microphone technology
- **retrieval_triggers:**
  - "costume design course"
  - "scenic design course"
  - "lighting design class"
  - "stage management course"
  - "audio technology theatre"
  - "scene painting class"
  - "makeup design theatre"
  - "theatre company what is it"
  - "design fundamentals who can take it"
  - "theatre production practicum"
  - "what is Theatre Laboratory"
  - "theatre sustainability class"
  - "theatre arts management course"
  - "props course theatre"
  - "can a non-major take theatre practicum"

---

### History and Literature Courses
- **filename:** `History_and_Literature_Courses.txt`
- **document_type:** course_info
- **degree_relevance:** All
- **time_sensitive:** false
- **catalog_year:** N/A
- **description:** Lists all theatre history, literature, and theory courses. Includes Theatre Appreciation (introductory survey of theatre as art form), Theatre Arts in London (fall London program only, taught by Goldsmith College faculty, with additional fee for performance admissions), Introduction to Play Analysis (critical approaches to Western dramatic texts), Theatre History (THE72-234, required core course exploring Western theatre across periods), East Meets West: Interculturalism and Theatre (20th-century theatrical exchange across cultures), Playwriting (theory and practice of script writing), Dramaturgy (prerequisite for those intending to dramaturg SSFA productions), Costume History I (classical antiquity through 18th century), and Costume History II (late 18th century through present). Dramaturgy is listed as a prerequisite for students intending to work as dramaturgs on SSFA productions.
- **critical_data:**
  - THE72-234 Theatre History: required core course for both B.A. tracks and minor
  - Theatre Arts in London: fall London program only; additional performance admission fee; taught by Goldsmith College, University of London faculty
  - Dramaturgy: prerequisite required for students intending to dramaturg SSFA productions
  - Playwriting: available as an elective fulfillment in the Theatre Arts B.A. track (alongside Devising)
  - Costume History I & II: offered as history/literature electives
- **retrieval_triggers:**
  - "theatre history course"
  - "THE72-234"
  - "playwriting class"
  - "dramaturgy course"
  - "costume history"
  - "theatre in London course"
  - "what is Theatre Arts in London"
  - "play analysis course"
  - "interculturalism and theatre"
  - "can I take theatre history abroad"
  - "prerequisite for dramaturg SSFA"
  - "theatre appreciation class"
  - "introduction to play analysis"

---

### Performance Courses
- **filename:** `Performance_Courses.txt`
- **document_type:** course_info
- **degree_relevance:** All
- **time_sensitive:** false
- **catalog_year:** N/A
- **description:** Lists all performance-track theatre courses. Includes Voice and Movement, Fundamentals of Acting (THE72-183, required core), Theatre Performance Practicum (one credit per main-stage role, max 4 credits, non-majors only), Alexander Technique (repeatable for credit), Theatre Dance (cross-listed as Dance 79-414), Acting: Realism, Departures from Realism, Musical Theatre Workshop (repeatable for credit), Theatre for Social Change: Practice and Performance, Feminism and Performance (cross-listed with Communication Studies 75-524 and Feminist Studies 04-724, FAP and WA designations), Acting: Poetic Language, Directing for the Theatre, and Advanced Directing (prerequisite Theatre 73-894). Multiple courses are repeatable for credit. Several courses carry Paideia area designations (FAP, WA noted for Feminism and Performance).
- **critical_data:**
  - THE72-183 Fundamentals of Acting: required core course for both B.A. tracks and minor (via Design Fundamentals requirement)
  - THE72-193 Movement and Voice: listed as a core requirement in Majoring & Minoring document
  - Advanced Directing prerequisite: Theatre 73-894 (Directing for the Theatre)
  - Theatre Performance Practicum: non-majors only; max 4 credits; requires being cast in main-stage production
  - Alexander Technique: repeatable for credit
  - Musical Theatre Workshop: repeatable for credit
  - Feminism and Performance: FAP and WA Paideia designations; cross-listed with Communication Studies and Feminist Studies
  - Theatre Dance: cross-listed with Dance 79-414
- **retrieval_triggers:**
  - "acting courses"
  - "fundamentals of acting"
  - "directing class"
  - "advanced directing prerequisite"
  - "musical theatre workshop"
  - "movement and voice"
  - "Alexander Technique"
  - "theatre performance practicum"
  - "can non-majors perform in plays"
  - "acting realism course"
  - "theatre for social change"
  - "feminism and performance"
  - "theatre dance class"
  - "repeatable theatre courses"
  - "voice and movement class"

---

### Theatre Facilities
- **filename:** `Facilities.txt`
- **document_type:** general
- **degree_relevance:** All
- **time_sensitive:** false
- **catalog_year:** N/A
- **description:** Describes the physical spaces available to theatre students at Southwestern University. The Theatre Department is housed in the Sarofim School of Fine Arts Building. Three main performance spaces are available: the Alma Thomas Theater (750-seat proscenium), the Jesse H. and Mary Gibbs Jones Theater (325-seat thrust), and the Heather Jean McGaughey Black Box (50-seat performance and classroom space). Students also have access to the costume shop, scene shop, and computer design studio, all located in the Sarofim Fine Arts building. References the Jones Theater's innovative revolving stage as a notable production feature used by students.
- **critical_data:**
  - Alma Thomas Theater: 750 seats, proscenium format
  - Jesse H. and Mary Gibbs Jones Theater: 325 seats, thrust format
  - Heather Jean McGaughey Black Box (Heather Hall): 50 seats, performance and classroom space
  - Technical facilities: costume shop, scene shop, computer design studio
  - All located in the Sarofim School of Fine Arts Building
  - Jones Theater features an innovative revolving stage
- **retrieval_triggers:**
  - "theatre facilities"
  - "how big is the theatre"
  - "what theaters are at SU"
  - "black box theatre"
  - "Jones Theater"
  - "Alma Thomas Theater"
  - "Heather Hall"
  - "costume shop"
  - "scene shop"
  - "revolving stage"
  - "where do theatre students perform"
  - "performance spaces Southwestern"

---

### Our Faculty and Staff
- **filename:** `Our_Faculty___Staff.txt`
- **document_type:** advising
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Lists all current Theatre Department faculty and staff with contact information, titles, specialties, and office locations. Faculty members include: Desiderio Roybal (Professor and Chair, scenic/lighting design, roybald@southwestern.edu, Fine Arts 309), Kerry Bechtel (Associate Professor and Production Coordinator, costume design/history/makeup, bechtelk@southwestern.edu, Fine Arts 308), Julia Chereson (PT Assistant Professor, cheresonj@southwestern.edu), Sergio Costola (Professor, theatre history/dramaturgy, costolas@southwestern.edu, CB 150), Gabriel M. Pena (Assistant Professor, acting, pena7@southwestern.edu, FAC 307), Coby Novillo (PT Theatre Lighting Technician, novillo2@southwestern.edu, FAC), Chaz Sanders (Technical Director, sanders5@southwestern.edu), and Glenda Wolfe (Costume Shop Manager, wolfeg@southwestern.edu, FAC 307).
- **critical_data:**
  - Department Chair: Desiderio Roybal — roybald@southwestern.edu — 512.863.1359 — Fine Arts 309
  - Production Coordinator: Kerry Bechtel — bechtelk@southwestern.edu — 512.863.1701
  - Theatre History/Dramaturgy faculty: Sergio Costola — costolas@southwestern.edu — 512.863.1620
  - Acting faculty: Gabriel M. Pena — pena7@southwestern.edu — 512.863.1234
  - Costume Shop Manager: Glenda Wolfe — wolfeg@southwestern.edu — 512.863.1746
  - Technical Director: Chaz Sanders — sanders5@southwestern.edu
  - Scholarship questions: contact Desiderio Roybal at roybald@southwestern.edu
- **retrieval_triggers:**
  - "who is the theatre chair"
  - "theatre faculty contact"
  - "who teaches acting"
  - "costume design professor"
  - "theatre history professor"
  - "technical director theatre"
  - "who do I contact for theatre scholarship"
  - "theatre department phone number"
  - "who is Desiderio Roybal"
  - "lighting technician contact"
  - "costume shop manager"
  - "theatre advising contact"

---

### Theatre Scholarship
- **filename:** `Theatre_Scholarship.txt`
- **document_type:** financial
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Describes the Sarofim Scholarship in Theatre available to prospective Theatre majors at Southwestern. Audition or portfolio review is required to be considered; students who indicate theatre interest when applying will receive instructions for submitting an online audition video or portfolio. Acting submissions require a two-minute monologue and optionally 32 bars of a musical song. All other areas (design, etc.) submit a portfolio. Submission deadline is February 15, 2026 (late submissions considered case-by-case). Scholarship recipients are invited to attend the Sarofim School of Fine Arts Scholarship Celebration Day on campus. Clarifies that auditioning is NOT mandatory to pursue a theatre major—any admitted student may declare theatre, and any student may audition for productions as a non-major. Scholarship recipients may be eligible for additional Southwestern financial assistance.
- **critical_data:**
  - Scholarship name: Sarofim Scholarship in Theatre
  - Audition submission deadline: February 15, 2026 (late submissions case-by-case)
  - Acting audition: 2-minute monologue + optional 32 bars of a musical song
  - Other areas: portfolio submission online
  - Contact for questions: Desiderio Roybal — roybald@southwestern.edu
  - Auditioning is NOT required to major in Theatre at SU
  - Any student may audition for productions as a non-major
  - Scholarship recipients invited to Scholarship Celebration Day on campus
- **retrieval_triggers:**
  - "theatre scholarship"
  - "Sarofim scholarship theatre"
  - "how to audition for scholarship"
  - "theatre audition deadline"
  - "do I have to audition to major in theatre"
  - "scholarship for theatre major"
  - "can non-majors audition"
  - "theatre portfolio submission"
  - "how to get theatre scholarship money"
  - "scholarship celebration day"
  - "acting audition requirements"
  - "theatre financial aid"

---

### Notable Faculty and Student Achievements
- **filename:** `Notable_Faculty___Student_Achievements.txt`
- **document_type:** general
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Documents notable accomplishments by Theatre Department faculty and students. This file was listed in the uploaded set but its extracted text content was not fully available in context. Based on the filename and department context, it likely covers awards, recognitions, performance highlights, guest artist residencies, and professional achievements relevant to the department's reputation and outcomes. Students asking about department prestige, faculty accomplishments, or student success stories would be directed here.
- **critical_data:**
  - Contents include faculty and student achievements (exact entries require file review)
  - Relevant to prospective student inquiries about department quality and outcomes
- **retrieval_triggers:**
  - "theatre faculty awards"
  - "student achievements theatre"
  - "notable alumni"
  - "theatre department accomplishments"
  - "has anyone won awards from SU theatre"
  - "faculty recognition theatre"

---

### High Impact Learning Experiences
- **filename:** `High_Impact_Learning_Experiences.txt`
- **document_type:** advising
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Describes the Theatre Department's requirement and offerings for high-impact learning experiences. Theatre majors are expected to engage in at least two high-impact experiences before graduation. Available options include: New York Arts Program, Study Abroad, London Semester Abroad, internships, Mask and Wig Association, Alpha Psi Omega, and Workshops and Guest Artists Series. The Center for Career and Professional Development can help with applications, resumes, and interviews. Funding options include the Sarofim Travel Grant (up to $700 per year), general tuition scholarships applicable to London Semester, Paideia scholarships, and Office of Intercultural Learning scholarships. Students in the Edinburgh festival program (Don Swan on the Royal Mile) are also referenced.
- **critical_data:**
  - Theatre majors must complete at least two high-impact experiences before graduation
  - Sarofim Travel Grant: up to $700, usable once per year throughout enrollment
  - London Semester Abroad can be supported by general tuition scholarships and financial aid
  - Center for Career and Professional Development assists with program applications
  - Programs include: NYAP, Study Abroad, London Semester, internships, Mask and Wig, Alpha Psi Omega, Workshops/Guest Artists
  - Additional funding: Paideia scholarships, Office of Intercultural Learning scholarships
- **retrieval_triggers:**
  - "high impact experiences theatre"
  - "study abroad theatre"
  - "how many high impact experiences do I need"
  - "Sarofim Travel Grant"
  - "theatre funding for travel"
  - "Mask and Wig Association"
  - "Alpha Psi Omega"
  - "New York Arts Program theatre"
  - "London Semester theatre"
  - "guest artists workshop"
  - "Edinburgh theatre program"
  - "career development theatre"
  - "funding for theatre study abroad"

---

### New York Internships
- **filename:** `New_York_Internships.txt`
- **document_type:** resources
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Describes the New York Arts Program (NYAP) internship experience available to Theatre students. NYAP is based in Chelsea, Manhattan, with students residing in the program's five-story dormitory/office townhouse on West 29th Street. The program offers internships, seminars led by NYAP faculty and creative professionals, individual faculty advising for career exploration, participation in a community of students and professionals, and program events providing exposure to New York as a global cultural capital. Applications are due in October and March for the following semester; 50 participants accepted per semester. The SU Internship Coordinator is Dana Luna (512-863-1346). The NYAP program coordinator is Susan Chilrey (susan@nyartsprogram.org, 212-563-0255).
- **critical_data:**
  - NYAP application deadlines: October and March (for following semester)
  - Participants accepted: 50 per semester
  - Location: 305 West 29 Street, New York, NY 10001 (Chelsea)
  - SU Internship Coordinator: Dana Luna — 512-863-1346
  - NYAP Program Coordinator: Susan Chilrey — susan@nyartsprogram.org — 212-563-0255
  - Program includes: internships, seminars, individual advising, community events
- **retrieval_triggers:**
  - "New York internship theatre"
  - "NYAP program"
  - "New York Arts Program"
  - "internship in New York"
  - "how to apply for NYAP"
  - "NYAP deadline"
  - "who is the internship coordinator"
  - "Dana Luna contact"
  - "theatre internship New York"
  - "living in New York for theatre program"
  - "how many students accepted NYAP"

---

### Internships Abroad
- **filename:** `Internships_Abroad.txt`
- **document_type:** resources
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Describes international internship opportunities available through the London-based program. Internships are placed at professional arts organizations in the greater London area (theatres, concert halls, galleries, museums) and arranged through CAPA (Centre for Academic Programmes Abroad), supervised jointly by CAPA and an SU faculty member. Students in internships may take one of three related courses for credit and are encouraged to attend program events (tickets included in program fee). Internships require a six-week commitment, meaning students remain in London two weeks after the start of the formal academic program. Successful completion yields six semester hours of credit.
- **critical_data:**
  - Internship facilitator: CAPA (Centre for Academic Programmes Abroad), London
  - Duration: 6 weeks total; students stay 2 weeks beyond formal academic program start
  - Credit: 6 semester hours upon successful completion
  - One related course may be taken concurrently
  - Ticket costs for program events (performances, concerts, museum visits) included in program fee
  - Supervised jointly by CAPA and SU faculty member
- **retrieval_triggers:**
  - "London internship theatre"
  - "international internship"
  - "CAPA program"
  - "internship abroad London"
  - "how many credits for London internship"
  - "how long is the London internship"
  - "arts internship London"
  - "theatre internship abroad"
  - "museum internship London"
  - "can I take a class during London internship"

---

### Semester in London
- **filename:** `Semester_in_London.txt`
- **document_type:** resources
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Provides contact information for the Semester in London program through the Office of Intercultural Learning. The most up-to-date information on costs, dates, and payment deadlines is maintained by that office. Contact: Tisha Temple, Director of Intercultural Learning, P.O. Box 770, Georgetown, Texas, (512) 863-1857, fax (512) 863-1535, templet@southwestern.edu. This document is a brief referral document rather than a complete program description; detailed program and financial information is maintained by the Office of Intercultural Learning.
- **critical_data:**
  - Program details maintained by: Office of Intercultural Learning
  - Contact: Tisha Temple — templet@southwestern.edu — (512) 863-1857
  - Fax: (512) 863-1535
  - Address: P.O. Box 770, Georgetown, Texas
  - Note: Costs, dates, and deadlines are not contained in this document — refer student to Tisha Temple
- **retrieval_triggers:**
  - "semester in London"
  - "London semester abroad"
  - "study abroad London"
  - "how much does London semester cost"
  - "London semester deadline"
  - "London program contact"
  - "Tisha Temple"
  - "intercultural learning London"
  - "London semester financial aid"

---

### Current Season (2025-2026)
- **filename:** `Current_Season.txt`
- **document_type:** calendar
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Lists all three productions in the 2025-2026 Theatre Season. Men on Boats by Jaclyn Backhaus runs November 7-9 and 14-17, 2025, in the Jesse H. and Mary Gibbs Jones Theater. BKLYN The Musical (book/music/lyrics by Mark Schoenfeld and Barri McPherson) runs February 20-22, 27-28, and March 1, 2026, in the Jones Theater. Tracks by Peter Tarsi runs April 17-19 and 24-26, 2026, in the Heather McGaughey Memorial Hall. Season subscribers who purchase tickets to all three productions automatically receive a 20% discount. No strollers or baby buggies are allowed in the theater.
- **critical_data:**
  - Production 1: Men on Boats — Nov 7-9, 14-17, 2025 — Jones Theater
  - Production 2: BKLYN The Musical — Feb 20-22, 27-28, Mar 1, 2026 — Jones Theater
  - Production 3: Tracks — Apr 17-19, 24-26, 2026 — Heather McGaughey Memorial Hall
  - Season discount: 20% off when all three productions purchased together
  - Policy: No strollers or baby buggies allowed in the theater
- **retrieval_triggers:**
  - "what shows are playing this season"
  - "current theatre season"
  - "Men on Boats show"
  - "BKLYN Musical"
  - "Tracks play"
  - "when are the shows"
  - "theatre season 2025-2026"
  - "season ticket discount"
  - "what is playing at the Jones Theater"
  - "spring show theatre"
  - "fall show theatre"

---

### Box Office
- **filename:** `Box_Office.txt`
- **document_type:** policy
- **degree_relevance:** All
- **time_sensitive:** true
- **catalog_year:** 2025-2026
- **description:** Provides complete information on purchasing tickets, pricing, discounts, ADA accommodations, complimentary tickets for SU community members, and box office operations. Tickets may be purchased online, by phone (512-863-1378), or in person during box office hours (12:30–5:00 p.m. Monday–Friday). All purchases must use a credit card; cash is no longer accepted. Sarofim Music Series season ticket discount: 20% off when purchasing tickets to at least five of seven concerts. Theatre season discount: 20% off when purchasing all three productions. Discounts available for seniors (over 62), youth (under 16), students (valid ID), educators, and military. Groups of 10+ receive an automatic 15% discount. SU students receive one complimentary ticket per production; SU faculty/staff receive two. ADA seating requires a phone reservation at 512-863-1378. Box office is not open on Saturdays; opens 1.5 hours before football games at Birkelbach Stadium.
- **critical_data:**
  - Box office hours: Monday–Friday, 12:30–5:00 p.m.
  - Phone: 512-863-1378
  - Payment: credit card only; no cash
  - Theatre season discount: 20% when all 3 productions purchased
  - Music season discount: 20% when 5 of 7 Sarofim concerts purchased
  - Group discount (10+): 15% automatic
  - SU student complimentary tickets: 1 per production
  - SU faculty/staff complimentary tickets: 2 per production
  - ADA seating: call 512-863-1378
  - Not open Saturdays; opens 1.5 hours before Birkelbach Stadium football games
- **retrieval_triggers:**
  - "how to buy theatre tickets"
  - "box office hours"
  - "student discount tickets"
  - "complimentary tickets SU students"
  - "ADA theatre seating"
  - "theatre ticket prices"
  - "season ticket discount"
  - "group discount tickets"
  - "can I pay cash at box office"
  - "buy tickets online"
  - "faculty tickets theatre"
  - "box office phone number"

---

### Ticket Policies
- **filename:** `Ticket_Policies.txt`
- **document_type:** policy
- **degree_relevance:** All
- **time_sensitive:** false
- **catalog_year:** N/A
- **description:** Outlines the full ticket policy for Mathers Box Office. All sales are final; no refunds or exchanges for single-ticket purchases. Season ticket holders may exchange tickets within the same production up to 48 hours before curtain if tickets are available; price differences must be paid at time of exchange; changes must happen in person or by phone during business hours. Tickets may be donated back to the university at least 24 hours in advance by emailing boxoffice@southwestern.edu. No tickets are mailed; all tickets must be picked up at the Mathers Box Office or at Will Call on the day of the show. An emailed receipt is not an electronic ticket. Tickets may be picked up any time the box office is open or beginning one hour before showtime. Box office hours: Monday–Friday, 12:30 p.m.–5:00 p.m.
- **critical_data:**
  - No refunds or exchanges for single-ticket purchases
  - Season ticket holders: exchanges allowed within same production, up to 48 hours before curtain, with availability
  - Ticket donation: at least 24 hours in advance via boxoffice@southwestern.edu
  - No mailed tickets; pickup required at Mathers Box Office or Will Call
  - Emailed receipt is NOT an electronic ticket
  - Pickup available: any time box office is open, or 1 hour before showtime
  - Hours: Monday–Friday, 12:30 p.m.–5:00 p.m.
- **retrieval_triggers:**
  - "can I get a refund on theatre tickets"
  - "ticket exchange policy"
  - "how do I pick up tickets"
  - "will call theatre"
  - "donate tickets back"
  - "is my email confirmation my ticket"
  - "ticket policies"
  - "season ticket exchange"
  - "where do I pick up my tickets"
  - "box office pickup"

---

### Parking and Directions
- **filename:** `Parking_and_Directions.txt`
- **document_type:** general
- **degree_relevance:** All
- **time_sensitive:** false
- **catalog_year:** N/A
- **description:** Provides directions and parking information for visiting the Sarofim School of Fine Arts and Mathers Box Office. Parking on campus is free, and all campus parking lots are available to guests. Specific parking areas near fine arts events are marked with a gold star on the campus map. Parking available at Alma Thomas Fine Arts Center, behind Lois Perkins Chapel, and in other campus areas. The Mathers Box Office is located in the Sarofim School of Fine Arts adjacent to the Alma Thomas Theater. Driving directions from IH-35: exit 261, turn right onto University Ave, continue 1.6 miles, turn left into parking lot on E. Rutersville Road (second left after Maple St./University intersection). Box office is in the middle of the building.
- **critical_data:**
  - Parking: free on campus; all lots available to guests
  - Campus map available for download; fine arts parking areas indicated by gold star
  - Mathers Box Office: located in Sarofim School of Fine Arts, adjacent to Alma Thomas Theater
  - Directions: IH-35 exit 261 → right on University Ave → 1.6 miles → left on E. Rutersville Road
  - Box office is in the middle of the Sarofim building
- **retrieval_triggers:**
  - "where to park for theatre"
  - "how to get to the theatre"
  - "Sarofim building location"
  - "parking fine arts"
  - "directions to SU theatre"
  - "box office location"
  - "is parking free"
  - "Alma Thomas Theater location"

---

## SECTION 3 — DEGREE PATH SUMMARIES

### SU Universal Graduation Requirements (All Degree Paths)

| Requirement | Detail |
|---|---|
| Minimum Total Credits | 127 credits |
| Minimum SU Residency Credits | 64 credits (last 32 must be in-residence) |
| Minimum Overall GPA | 2.000 (both cumulative and SU-only) |
| Minimum Major GPA | 2.000 average (no grade below C- counts toward major) |
| Major Residency Requirement | At least 60% of major credits at SU |
| Minor Residency Requirement | At least 12 credits at SU (if minor pursued) |
| Graduation Application | "Application for Diploma" must be filed; financial holds block diploma |
| Degree Conferral Dates | December, May, August only; Commencement held once per year in May |

*Source: `SU_General_Policies.txt`*

### SU Universal Curriculum Requirements (Paideia Framework)

**Required for ALL degree paths:**
- First-Year Seminar (FYS) or Advanced-Entry Seminar (AES): 4 credits
- Languages and Cultures: through third-semester proficiency (up to 12 credits)
- Fitness and Recreational Activity (FRA): 1 credit
- Power and Justice (PJ) course: 1 course, 3-4 credits — cannot be satisfied by transfer credit
- Exploration and Breadth: 6 courses from outside the major (18-24 credits)
  - Part I: one course from each of Fine Arts (FA), Humanities (H), Natural Sciences (NS), Social Sciences (ScS)
  - Part II: one additional course from two of the four areas

*Source: `SU_General_Policies.txt`*

---

### B.A. in Theatre Arts — 35-36 Credits

| Field | Detail |
|---|---|
| Total Credit Hours | 35-36 (major credits) + 127 total for graduation |
| Major Credit Hours | 35-36 |
| Minimum GPA (Overall) | 2.000 |
| Minimum GPA (Major) | 2.000 |
| Residency Requirement | At least 60% of major credits at SU |
| Source Document | `Majoring___Minoring.txt` |
| Designed For | Students with a second major or multiple minors |

**Required Core Courses:**
- THE72-164 — Design Fundamentals (credit hours not specified in documents)
- THE72-183 — Fundamentals of Acting
- THE72-193 — Movement and Voice
- THE72-211 — Theatre Laboratory (must be taken six semesters)
- THE72-234 — Theatre History
- THE72-921 — Theatre Capstone: Research Methods
- THE72-923 — Theatre Capstone

**Required Elective Choices (select one from each group):**

*Group 1 — Devising or Writing:*
- THE72-254 — Devising
- THE72-314 — Playwriting

*Group 2 — Design:*
- THE72-794 — Costume Design
- THE72-804 — Scenic Design
- THE72-824 — Lighting Design

**Additional Requirements:**
- One additional upper-level course in Theatre (3-4 credits each or equivalent)

**Notable Constraints:**
- Credits from independent studies and academic internships may NOT be used to fulfill Theatre Major requirements
- THE72-211 Theatre Laboratory must be completed for six semesters
- Final two semesters of Theatre Company must occur during senior year

---

### B.A. in Theatre Performance and Production — 39-45 Credits

| Field | Detail |
|---|---|
| Total Credit Hours | 39-45 (major credits) + 127 total for graduation |
| Major Credit Hours | 39-45 |
| Minimum GPA (Overall) | 2.000 |
| Minimum GPA (Major) | 2.000 |
| Residency Requirement | At least 60% of major credits at SU |
| Source Document | `Majoring___Minoring.txt` |
| Designed For | Students who wish to major in Theatre with or without a minor |

**Required Core Courses (same as Theatre Arts B.A.):**
- THE72-164 — Design Fundamentals
- THE72-183 — Fundamentals of Acting
- THE72-193 — Movement and Voice
- THE72-211 — Theatre Laboratory (six semesters)
- THE72-234 — Theatre History
- THE72-921 — Theatre Capstone: Research Methods
- THE72-923 — Theatre Capstone

**Additional Requirements:**
- Six additional upper-level courses in Theatre (3-4 credits each or equivalent)

**Notable Constraints:**
- Credits from independent studies and academic internships may NOT be used to fulfill Theatre Major requirements
- THE72-211 Theatre Laboratory must be completed for six semesters
- Final two semesters of Theatre Company must occur during senior year

---

### Minor in Theatre — 5 Courses

| Field | Detail |
|---|---|
| Required Courses | 5 courses (minimum) |
| Residency Requirement | At least 12 credits at SU |
| Source Document | `Majoring___Minoring.txt` |

**Required Courses:**
- THE72-164 — Design Fundamentals
- THE72-234 — Theatre History
- Three additional courses in Theatre (3-4 credits each or equivalent)

**Notable Constraints:**
- Successful completion of two 2-credit Theatre courses counts as the equivalent of one 4-credit course
- See Education Department for information regarding Teacher Certification in Theatre

---

### Key Differences Between Degree Paths

| Feature | B.A. Theatre Arts | B.A. Theatre Performance & Production | Minor |
|---|---|---|---|
| Total Major Credits | 35-36 | 39-45 | 5 courses |
| Upper-Level Electives | 1 additional + 2 required choices | 6 additional | 3 additional |
| Design Course Required | Yes (choose 1 of 3) | Flexible (among 6 upper-level) | Not specified |
| Devising/Playwriting Required | Yes (choose 1 of 2) | Flexible | Not required |
| Best For | Double major / multiple minors | Primary theatre focus | Non-majors wanting theatre depth |

**Career/Goal Guidance:**
- Students interested in graduate school focusing on research/scholarship or teacher certification typically choose the B.A. option
- The B.A. Theatre Arts track is explicitly described as being designed for students with a second major or multiple minors
- The B.A. Theatre Performance and Production is for students who wish to major with or without a minor
- Auditioning is NOT required to declare either major or the minor; any admitted student may do so

---

## SECTION 4 — COURSE INDEX

### THE72 — Theatre Design Courses

#### THE72-164 — Design Fundamentals
- **Credit Hours:** Not specified in documents
- **Prerequisites:** Open to first-year students and sophomores only; juniors/seniors need instructor permission
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. Theatre Arts, B.A. Theatre Performance and Production, Minor in Theatre
- **Counts As:** Theatre Core requirement
- **Notes:** Introductory course covering costume, scenery/props, sound, and lighting design; theatrical drawing, rendering, script analysis, collaboration
- **Source:** `Majoring___Minoring.txt`, `Design_Courses.txt`

#### THE72-183 — Fundamentals of Acting
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. Theatre Arts, B.A. Theatre Performance and Production
- **Counts As:** Theatre Core requirement
- **Notes:** Introduces acting processes: character development, script analysis, dramatic action, movement, voice, improvisation; substantial written and performance work required
- **Source:** `Majoring___Minoring.txt`, `Performance_Courses.txt`

#### THE72-193 — Movement and Voice
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. Theatre Arts, B.A. Theatre Performance and Production
- **Counts As:** Theatre Core requirement
- **Notes:** Practical course; voice work on resonance, enunciation, stage dialects; movement work on body alignment, spatial awareness
- **Source:** `Majoring___Minoring.txt`, `Performance_Courses.txt`

#### THE72-211 — Theatre Laboratory
- **Credit Hours:** Not specified (must be taken six semesters)
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Both semesters
- **Required For:** B.A. Theatre Arts, B.A. Theatre Performance and Production
- **Counts As:** Theatre Core requirement (six semesters required)
- **Notes:** Complements Theatre Company; laboratory setting for technical theatre skills; final two semesters must occur during senior year
- **Source:** `Majoring___Minoring.txt`, `Design_Courses.txt`

#### THE72-234 — Theatre History
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. Theatre Arts, B.A. Theatre Performance and Production, Minor in Theatre
- **Counts As:** Theatre Core requirement; Minor requirement
- **Notes:** Explores Western theatre and drama across periods; analyzes performance as functions of economic, political, social, and aesthetic factors; introduces critical research methods and historiography
- **Source:** `Majoring___Minoring.txt`, `History_and_Literature_Courses.txt`

#### THE72-254 — Devising
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Optional selection for B.A. Theatre Arts (Group 1 elective — choose Devising or Playwriting)
- **Counts As:** Elective requirement (Group 1) for B.A. Theatre Arts
- **Source:** `Majoring___Minoring.txt`

#### THE72-314 — Playwriting
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Optional selection for B.A. Theatre Arts (Group 1 elective — choose Devising or Playwriting)
- **Counts As:** Elective requirement (Group 1) for B.A. Theatre Arts
- **Notes:** Theory and practices of playwriting; writing scripts for theatre reading and production
- **Source:** `Majoring___Minoring.txt`, `History_and_Literature_Courses.txt`

#### THE72-794 — Costume Design
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Optional selection for B.A. Theatre Arts (Group 2 Design elective)
- **Counts As:** Design elective requirement for B.A. Theatre Arts; upper-level elective for B.A. Theatre Performance and Production
- **Notes:** Art and practice of theatrical costume design; pre-production requirements, analysis, research, figure proportion, color theory, sketching, swatching, rendering
- **Source:** `Majoring___Minoring.txt`, `Design_Courses.txt`

#### THE72-804 — Scenic Design
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Optional selection for B.A. Theatre Arts (Group 2 Design elective)
- **Counts As:** Design elective requirement for B.A. Theatre Arts; upper-level elective for B.A. Theatre Performance and Production
- **Notes:** Art and practice of theatrical scenic design; traditional approaches, composition elements, theatrical styles; painted renderings and/or models and draftings
- **Source:** `Majoring___Minoring.txt`, `Design_Courses.txt`

#### THE72-824 — Lighting Design
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Optional selection for B.A. Theatre Arts (Group 2 Design elective)
- **Counts As:** Design elective requirement for B.A. Theatre Arts; upper-level elective for B.A. Theatre Performance and Production
- **Notes:** Introductory lighting design; hands-on projects; additive color theory; incandescent and LED fixtures; natural and artificial lighting observation
- **Source:** `Majoring___Minoring.txt`, `Design_Courses.txt`

#### THE72-921 — Theatre Capstone: Research Methods
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated beyond being a Theatre major
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. Theatre Arts, B.A. Theatre Performance and Production
- **Counts As:** Theatre Core capstone requirement
- **Source:** `Majoring___Minoring.txt`

#### THE72-923 — Theatre Capstone
- **Credit Hours:** Not specified in documents
- **Prerequisites:** THE72-921 (implied sequencing)
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. Theatre Arts, B.A. Theatre Performance and Production
- **Counts As:** Theatre Core capstone requirement
- **Source:** `Majoring___Minoring.txt`

---

### THE73 — Theatre Performance Courses

#### THE73-894 — Directing for the Theatre
- **Credit Hours:** Not specified in documents
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not required; prerequisite for Advanced Directing
- **Counts As:** Upper-level elective
- **Notes:** Principles and practices of directing; detailed playscript analysis; directing laboratory or workshop productions
- **Source:** `Performance_Courses.txt`

#### Advanced Directing (THE73-XXX)
- **Credit Hours:** Not specified
- **Prerequisites:** Theatre 73-894 (Directing for the Theatre)
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not required
- **Counts As:** Upper-level elective
- **Notes:** Director's transformation of personal vision into social/aesthetic meaning; script analysis; theatrical space and conventions; lecture, discussion, in-class exercises
- **Source:** `Performance_Courses.txt`

---

### Other Named Courses (course numbers not fully specified in documents)

#### Theatre Company
- **Credit Hours:** Not specified (6 hours/week commitment)
- **Prerequisites:** Enrollment in the course (students form production company)
- **Required For:** Related to Theatre Laboratory requirement
- **Notes:** Students support departmental productions in scenery, costumes, lighting, or sound; includes back-stage crew, front-of-house, assistant directors, designers, shop managers, stage managers; workshops, master classes, dramatic literature discussions; final two semesters must occur during senior year
- **Source:** `Design_Courses.txt`

#### Theatre Appreciation
- **Notes:** Introductory survey; historical/cultural influences, dramatic texts, acting, design, aesthetics; open to non-majors
- **Source:** `History_and_Literature_Courses.txt`

#### Theatre Arts in London
- **Notes:** Fall London program only; taught by Goldsmith College, University of London; additional fee for performance admissions; surveys past and present of theatre with emphasis on attending London performances
- **Source:** `History_and_Literature_Courses.txt`

#### Introduction to Play Analysis
- **Notes:** Critical approaches to Western dramatic texts; form, structure, tone, style; realistic 20th-century plays and departures from realism
- **Source:** `History_and_Literature_Courses.txt`

#### East Meets West: Interculturalism and Theatre
- **Notes:** Theatrical interculturalism in world context; exchanges, imitations, misunderstandings in 20th-century theatre
- **Source:** `History_and_Literature_Courses.txt`

#### Dramaturgy
- **Notes:** Prerequisite required for students intending to dramaturg SSFA productions; integrates theatre history, dramatic literature, criticism with production practice
- **Source:** `History_and_Literature_Courses.txt`

#### Costume History I
- **Notes:** Western costume and fashion from classical antiquity through 18th century
- **Source:** `History_and_Literature_Courses.txt`

#### Costume History II
- **Notes:** Western costume and fashion from late 18th century through present; includes contemporary fashion impact
- **Source:** `History_and_Literature_Courses.txt`

#### Theatre Production Practicum
- **Notes:** 1 credit per production; max 4 credits total; non-majors only; requires production assignment (assistant stage management or design position)
- **Source:** `Design_Courses.txt`

#### Makeup Design
- **Notes:** Stage, screen, and print makeup design and practice
- **Source:** `Design_Courses.txt`

#### Theatre Arts Management
- **Notes:** Commercial and non-profit theatre (educational, community, professional); managerial aspects of creating, producing, and promoting theatre
- **Source:** `Design_Courses.txt`

#### Theatre Sustainability
- **Notes:** Cross-listed with Environmental Studies 49-814; uses Heather Hall as a model; electrical energy, scenery, costumes, performance, business aspects
- **Source:** `Design_Courses.txt`

#### Audio Technology and Design
- **Notes:** Theatre sound systems; mixers, effects processors, amplification, microphone technology; hands-on as assistant designers, audio engineers, programmer/operators
- **Source:** `Design_Courses.txt`

#### Scenic Elements and Stage Properties
- **Notes:** Scenic building and properties production; basic sewing, advanced prop fabrication; script analysis for props; working with designers, directors, stage management
- **Source:** `Design_Courses.txt`

#### Scene Painting
- **Notes:** Paint finishes and techniques for stage scenery; foundation for further study in scene painting
- **Source:** `Design_Courses.txt`

#### Stage Management for the Theatre
- **Notes:** Introduction to stage management; project-oriented; techniques and strategies for academic and professional theatre; hands-on in performance spaces
- **Source:** `Design_Courses.txt`

#### Theatre Performance Practicum
- **Notes:** 1 credit per main-stage production; max 4 credits; non-majors only; requires being cast in a main-stage production role
- **Source:** `Performance_Courses.txt`

#### Alexander Technique
- **Notes:** Body alignment and movement; repeatable for credit; appropriate for Theatre and Music students
- **Source:** `Performance_Courses.txt`

#### Theatre Dance
- **Notes:** Jazz, tap, and modern dance for musical theatre choreography; cross-listed with Dance 79-414
- **Source:** `Performance_Courses.txt`

#### Acting: Realism
- **Notes:** Analysis and performance of realistic plays; personalizing and physicalizing characters, playing actions and objectives
- **Source:** `Performance_Courses.txt`

#### Departures from Realism
- **Notes:** 20th- and 21st-century acting techniques; research, analysis, performance
- **Source:** `Performance_Courses.txt`

#### Musical Theatre Workshop
- **Notes:** Scene-and-song work in musical theatre; interpreting lyrics and text as an actor; repeatable for credit
- **Source:** `Performance_Courses.txt`

#### Theatre for Social Change: Practice and Performance
- **Notes:** Theatre as political, activist, educational, aesthetic tool; interactive performances for community settings
- **Source:** `Performance_Courses.txt`

#### Feminism and Performance
- **Notes:** Cross-listed with Communication Studies 75-524 and Feminist Studies 04-724; carries FAP and WA Paideia designations; gender performance on stage, in daily life, and media
- **Source:** `Performance_Courses.txt`

#### Acting: Poetic Language
- **Notes:** Acting techniques for poetic texts; voice, language, movement, dialect work; character and scene analysis; historical research
- **Source:** `Performance_Courses.txt`

---

## SECTION 5 — TOPIC INDEX

- B.A. Theatre Arts degree requirements → `Majoring___Minoring.txt`
- B.A. Theatre Performance and Production degree requirements → `Majoring___Minoring.txt`
- Minor in Theatre requirements → `Majoring___Minoring.txt`
- Difference between Theatre Arts and Theatre Performance and Production B.A. → `Majoring___Minoring.txt`
- Total credit hours for Theatre major → `Majoring___Minoring.txt`, `SU_General_Policies.txt`
- Theatre Core required courses → `Majoring___Minoring.txt`
- THE72-164 Design Fundamentals (enrollment eligibility, requirement) → `Majoring___Minoring.txt`, `Design_Courses.txt`
- THE72-183 Fundamentals of Acting (requirement, description) → `Majoring___Minoring.txt`, `Performance_Courses.txt`
- THE72-193 Movement and Voice (requirement, description) → `Majoring___Minoring.txt`, `Performance_Courses.txt`
- THE72-211 Theatre Laboratory (six semesters, senior year requirement) → `Majoring___Minoring.txt`, `Design_Courses.txt`
- THE72-234 Theatre History (required for all paths including minor) → `Majoring___Minoring.txt`, `History_and_Literature_Courses.txt`
- THE72-254 Devising (elective option B.A. Theatre Arts) → `Majoring___Minoring.txt`
- THE72-314 Playwriting (elective option B.A. Theatre Arts, course description) → `Majoring___Minoring.txt`, `History_and_Literature_Courses.txt`
- THE72-794 Costume Design (design elective, course description) → `Majoring___Minoring.txt`, `Design_Courses.txt`
- THE72-804 Scenic Design (design elective, course description) → `Majoring___Minoring.txt`, `Design_Courses.txt`
- THE72-824 Lighting Design (design elective, course description) → `Majoring___Minoring.txt`, `Design_Courses.txt`
- THE72-921 Theatre Capstone: Research Methods → `Majoring___Minoring.txt`
- THE72-923 Theatre Capstone → `Majoring___Minoring.txt`
- Theatre Company course description and commitment → `Design_Courses.txt`
- Independent study and internship credits not counting toward major → `Majoring___Minoring.txt`
- Upper-level elective requirements (number and eligibility) → `Majoring___Minoring.txt`
- Course numbering system (72=Design, 73=Performance, 74=History) → `Majoring___Minoring.txt`
- Teacher certification in theatre → `Majoring___Minoring.txt`, `SU_General_Policies.txt`
- Theatre scholarship (Sarofim Scholarship) → `Theatre_Scholarship.txt`
- Scholarship audition requirements and deadline (Feb 15, 2026) → `Theatre_Scholarship.txt`
- Whether auditioning is required to major in theatre → `Theatre_Scholarship.txt`
- Non-majors auditioning for productions → `Theatre_Scholarship.txt`, `Performance_Courses.txt`
- High-impact learning experience requirement (two required before graduation) → `High_Impact_Learning_Experiences.txt`
- Sarofim Travel Grant (up to $700/year) → `High_Impact_Learning_Experiences.txt`
- New York Arts Program (NYAP) internship → `New_York_Internships.txt`, `High_Impact_Learning_Experiences.txt`
- NYAP application deadlines (October and March) → `New_York_Internships.txt`
- NYAP contact information (Dana Luna, SU; Susan Chilrey, NYAP) → `New_York_Internships.txt`
- London internship through CAPA → `Internships_Abroad.txt`
- London internship credit (6 semester hours) and duration (6 weeks) → `Internships_Abroad.txt`
- Semester in London contact and costs → `Semester_in_London.txt`, `SU_Resources_and_Financial.txt`
- Theatre Arts in London course (fall London program only) → `History_and_Literature_Courses.txt`
- Study abroad options for theatre students → `High_Impact_Learning_Experiences.txt`, `Semester_in_London.txt`, `Internships_Abroad.txt`, `SU_Resources_and_Financial.txt`
- Funding for study abroad (Sarofim Travel Grant, Paideia scholarships, OIL scholarships) → `High_Impact_Learning_Experiences.txt`, `SU_Resources_and_Financial.txt`
- Mask and Wig Association → `High_Impact_Learning_Experiences.txt`
- Alpha Psi Omega (theatre honor society) → `High_Impact_Learning_Experiences.txt`
- Guest Artists and Workshops series → `High_Impact_Learning_Experiences.txt`
- Theatre facilities (Alma Thomas Theater, Jones Theater, Heather Hall) → `Facilities.txt`
- Alma Thomas Theater (750-seat proscenium) → `Facilities.txt`
- Jesse H. and Mary Gibbs Jones Theater (325-seat thrust) → `Facilities.txt`
- Heather Jean McGaughey Black Box / Heather Hall (50-seat) → `Facilities.txt`
- Costume shop, scene shop, computer design studio → `Facilities.txt`
- Current season productions and dates → `Current_Season.txt`
- Men on Boats (Nov 2025, Jones Theater) → `Current_Season.txt`
- BKLYN The Musical (Feb/Mar 2026, Jones Theater) → `Current_Season.txt`
- Tracks by Peter Tarsi (Apr 2026, Heather Hall) → `Current_Season.txt`
- Season ticket discount (20% for all 3 productions) → `Current_Season.txt`, `Box_Office.txt`
- How to buy theatre tickets → `Box_Office.txt`
- Box office hours and location → `Box_Office.txt`, `Ticket_Policies.txt`, `Parking_and_Directions.txt`
- Ticket discounts (senior, youth, student, educator, military) → `Box_Office.txt`
- Group discount (15% for groups of 10+) → `Box_Office.txt`
- SU student complimentary tickets (1 per production) → `Box_Office.txt`
- SU faculty/staff complimentary tickets (2 per production) → `Box_Office.txt`
- ADA and accommodated seating → `Box_Office.txt`, `Ticket_Policies.txt`
- Ticket refund and exchange policy → `Ticket_Policies.txt`
- Season ticket exchange (within same production, 48 hours advance) → `Ticket_Policies.txt`
- Donating tickets back (24 hours advance, boxoffice@southwestern.edu) → `Ticket_Policies.txt`
- Will Call and ticket pickup policy → `Ticket_Policies.txt`
- Emailed receipt is not an electronic ticket → `Ticket_Policies.txt`
- Parking and directions to Sarofim School of Fine Arts → `Parking_and_Directions.txt`
- Free parking on campus for fine arts events → `Parking_and_Directions.txt`
- Theatre faculty contact information → `Our_Faculty___Staff.txt`
- Department Chair contact (Desiderio Roybal) → `Our_Faculty___Staff.txt`
- Production Coordinator contact (Kerry Bechtel) → `Our_Faculty___Staff.txt`
- Costume Shop Manager contact (Glenda Wolfe) → `Our_Faculty___Staff.txt`
- Technical Director contact (Chaz Sanders) → `Our_Faculty___Staff.txt`
- Dramaturgy prerequisite for SSFA productions → `History_and_Literature_Courses.txt`
- Advanced Directing prerequisite (Theatre 73-894) → `Performance_Courses.txt`
- Feminism and Performance Paideia designations (FAP, WA) → `Performance_Courses.txt`
- Theatre Sustainability cross-listing with Environmental Studies → `Design_Courses.txt`
- Theatre Dance cross-listing with Dance 79-414 → `Performance_Courses.txt`
- Work-study jobs in theatre (faculty assistant, box office, backstage crew) → `Theatre.txt`
- Graduation requirements (credits, GPA, residency) → `Majoring___Minoring.txt`, `SU_General_Policies.txt`
- Paideia general education requirements → `SU_General_Policies.txt`
- Transfer credit eligibility for theatre major → `Majoring___Minoring.txt`, `SU_General_Policies.txt`
- Academic probation and dismissal → `SU_General_Policies.txt`
- Pass/D/F grading option → `SU_General_Policies.txt`
- Incomplete grade policy → `SU_General_Policies.txt`
- Financial aid impact of withdrawal → `SU_Resources_and_Financial.txt`
- Tuition refund schedule → `SU_Resources_and_Financial.txt`

---

## SECTION 6 — QUERY PATTERN MAP

**Q:** What are the requirements for a Theatre major at Southwestern?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Primary requirements document lists all three degree paths with core courses and elective requirements.
**Disambiguation Needed:** Yes — clarify whether student wants Theatre Arts B.A. (35-36 cr), Theatre Performance and Production B.A. (39-45 cr), or Minor.

**Q:** What is the difference between the Theatre Arts degree and the Theatre Performance and Production degree?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Both degree paths are defined in this document; key differences section explicitly compares elective structure and credit hours.
**Disambiguation Needed:** No.

**Q:** Do I have to audition to be a theatre major?
**Docs:** `Theatre_Scholarship.txt`
**Routing Note:** Document explicitly states auditioning is NOT mandatory to pursue a theatre major; any admitted student may declare theatre.
**Disambiguation Needed:** No.

**Q:** How do I apply for the Sarofim Theatre Scholarship?
**Docs:** `Theatre_Scholarship.txt`
**Routing Note:** Full audition requirements, submission deadline (Feb 15, 2026), and contact info (Roybal) are in this document.
**Disambiguation Needed:** No.

**Q:** I'm a freshman interested in theatre design. What courses should I take first?
**Docs:** `Majoring___Minoring.txt`, `Design_Courses.txt`
**Routing Note:** THE72-164 Design Fundamentals is the required entry course and is only open to first-year students and sophomores without instructor permission.
**Disambiguation Needed:** No — freshman status clarified in query.

**Q:** What courses count as the design elective for the Theatre Arts BA?
**Docs:** `Majoring___Minoring.txt`, `Design_Courses.txt`
**Routing Note:** The requirements doc lists the three choices (THE72-794, THE72-804, THE72-824); Design_Courses.txt provides descriptions of each.
**Disambiguation Needed:** No.

**Q:** Can I use my independent study credits to fulfill theatre major requirements?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Document explicitly states: "Credits from Independent studies and academic internships may not be used to fulfill Theatre Major requirements."
**Disambiguation Needed:** No.

**Q:** I want to do the Theatre minor. What five courses do I need?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Minor requirements section lists THE72-164, THE72-234, and three additional courses.
**Disambiguation Needed:** No.

**Q:** I'm a sophomore thinking about adding a second major alongside theatre. Which theatre BA is better for that?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** B.A. Theatre Arts is explicitly described as "designed for the Theatre student who has a second major or multiple minors."
**Disambiguation Needed:** No.

**Q:** What shows are playing this season?
**Docs:** `Current_Season.txt`
**Routing Note:** Lists all three 2025-2026 productions with dates, venues, and descriptions.
**Disambiguation Needed:** No.

**Q:** How do I get complimentary tickets as an SU student?
**Docs:** `Box_Office.txt`, `Ticket_Policies.txt`
**Routing Note:** Box Office document states SU students receive one complimentary ticket per production; pickup instructions in Ticket Policies.
**Disambiguation Needed:** No.

**Q:** Can I exchange my season tickets if I can't make the show?
**Docs:** `Ticket_Policies.txt`
**Routing Note:** Policy specifies exchange rules for season ticket holders: within same production, up to 48 hours before curtain, with availability, in person or by phone.
**Disambiguation Needed:** No.

**Q:** I transferred in from a community college. Will my theatre courses count toward the major?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Requirements doc notes the 60% residency rule for major credits; General Policies covers transfer credit evaluation rules (C- minimum, P/CR not accepted, ELEC coding).
**Disambiguation Needed:** Yes — ask what specific courses were taken to assess relevance.

**Q:** How many credits do I need to graduate?
**Docs:** `SU_General_Policies.txt`
**Routing Note:** 127-credit minimum, residency requirements, and GPA thresholds are university-wide policies in General Policies; major credit minimums from Majoring & Minoring.
**Disambiguation Needed:** No — but note major credits must be clarified from `Majoring___Minoring.txt`.

**Q:** What is the New York Arts Program and how do I apply?
**Docs:** `New_York_Internships.txt`, `High_Impact_Learning_Experiences.txt`
**Routing Note:** NYAP document has full program description, deadlines (October and March), contact info; High Impact doc confirms it as one of two required high-impact experiences.
**Disambiguation Needed:** No.

**Q:** How do I get funding to study abroad for theatre?
**Docs:** `High_Impact_Learning_Experiences.txt`, `SU_Resources_and_Financial.txt`
**Routing Note:** High Impact doc describes Sarofim Travel Grant ($700/yr), Paideia scholarships, OIL scholarships; Resources & Financial doc has fuller financial aid information.
**Disambiguation Needed:** No.

**Q:** What is the London internship program?
**Docs:** `Internships_Abroad.txt`, `Semester_in_London.txt`
**Routing Note:** Internships Abroad covers the CAPA-facilitated internship (6 weeks, 6 credits); Semester in London provides contact for full program details.
**Disambiguation Needed:** Yes — distinguish between the credit-bearing internship program and the semester-long academic program.

**Q:** I failed THE72-234 Theatre History. What do I do?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Requirements doc confirms THE72-234 is required for all paths; General Policies covers course repeat policies, GPA recalculation, academic standing implications.
**Disambiguation Needed:** No.

**Q:** I'm on academic probation. Can I still be in a theatre production?
**Docs:** `SU_General_Policies.txt`, `Theatre.txt`
**Routing Note:** Academic probation rules are in General Policies; theatre participation policies for students under academic restrictions may require advisor escalation.
**Disambiguation Needed:** No — but escalate to advisor.

**Q:** What is Theatre Laboratory and do I have to take it every semester?
**Docs:** `Majoring___Minoring.txt`, `Design_Courses.txt`
**Routing Note:** Requirements doc specifies six semesters of THE72-211 as a core requirement; Design Courses describes the content.
**Disambiguation Needed:** No.

**Q:** Where do the theatre shows take place?
**Docs:** `Facilities.txt`, `Current_Season.txt`
**Routing Note:** Facilities lists all three performance spaces; Current Season specifies which venue each 2025-2026 production uses.
**Disambiguation Needed:** No.

**Q:** What is the Heather Hall / black box theatre?
**Docs:** `Facilities.txt`
**Routing Note:** Facilities document identifies the Heather Jean McGaughey Black Box as a 50-seat performance and classroom space; also used for Theatre Sustainability course and Tracks (spring 2026).
**Disambiguation Needed:** No.

**Q:** Can I take Feminism and Performance to satisfy a Paideia requirement?
**Docs:** `Performance_Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Performance Courses notes FAP and WA Paideia designations for this course; General Policies has full Paideia structure.
**Disambiguation Needed:** No — but confirm student's Paideia status via General Policies.

**Q:** I want to be a dramaturg for an SSFA production. What do I need?
**Docs:** `History_and_Literature_Courses.txt`
**Routing Note:** Dramaturgy course description states it is a prerequisite for students intending to dramaturg SSFA productions.
**Disambiguation Needed:** No.

**Q:** How do I direct a show? What courses do I need?
**Docs:** `Performance_Courses.txt`
**Routing Note:** Directing for the Theatre is the prerequisite for Advanced Directing (Theatre 73-894); both described in Performance Courses.
**Disambiguation Needed:** No.

**Q:** Do I need to take a capstone to graduate in theatre?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Both B.A. tracks require THE72-921 Theatre Capstone: Research Methods AND THE72-923 Theatre Capstone as core requirements.
**Disambiguation Needed:** No.

**Q:** What high-impact learning experiences do I need as a theatre major?
**Docs:** `High_Impact_Learning_Experiences.txt`
**Routing Note:** Document states theatre majors must engage in at least two high-impact experiences before graduation; lists all available options.
**Disambiguation Needed:** No.

**Q:** I'm considering a double major in theatre and another subject. Can I do that?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** The B.A. Theatre Arts is explicitly designed for students with a second major; General Policies covers paired major restrictions and double major rules.
**Disambiguation Needed:** Yes — clarify which second major, as paired major restrictions may apply.

**Q:** What is the box office phone number?
**Docs:** `Box_Office.txt`
**Routing Note:** Box office phone 512-863-1378 appears in this document.
**Disambiguation Needed:** No.

**Q:** Can I use financial aid for the London Semester?
**Docs:** `Semester_in_London.txt`, `High_Impact_Learning_Experiences.txt`, `SU_Resources_and_Financial.txt`
**Routing Note:** High Impact doc notes London Semester can be supported by general tuition scholarships and financial aid; Semester in London doc refers to OIL for cost details; Resources & Financial has fuller financial aid information.
**Disambiguation Needed:** No — but direct to Tisha Temple for current cost details.

**Q:** Is parking free when I come to a show?
**Docs:** `Parking_and_Directions.txt`
**Routing Note:** Document explicitly states parking is free on campus and all lots are available to guests.
**Disambiguation Needed:** No.

**Q:** I want to take an acting class but I'm not a theatre major. Can I?
**Docs:** `Performance_Courses.txt`, `Theatre_Scholarship.txt`
**Routing Note:** Theatre Performance Practicum notes it is for non-majors; Scholarship doc states any student may audition for productions; Performance Courses lists courses open to all students.
**Disambiguation Needed:** No.

**Q:** What happened to my ticket refund request?
**Docs:** `Ticket_Policies.txt`
**Routing Note:** Policy is explicit: all sales are final with no refunds for single-ticket purchases; escalate to box office.
**Disambiguation Needed:** No.

**Q:** I want to work in costume design. Which courses should I take and which degree track is better?
**Docs:** `Majoring___Minoring.txt`, `Design_Courses.txt`, `History_and_Literature_Courses.txt`
**Routing Note:** Relevant courses include THE72-794 Costume Design, Costume History I & II, Makeup Design; both B.A. tracks allow focus on design; Production and Performance B.A. allows more elective depth.
**Disambiguation Needed:** No — but advisor consultation recommended for full planning.

**Q:** When does the senior Theatre Company requirement apply?
**Docs:** `Design_Courses.txt`, `Majoring___Minoring.txt`
**Routing Note:** Both documents note the final two semesters of Theatre Company (THE72-211) must occur during the student's senior year.
**Disambiguation Needed:** No.

---

## SECTION 7 — CROSS-REFERENCE FLAGS

- **Topic:** Graduation requirements (credit minimums, residency, GPA)
  - **What major docs cover:** Major-specific credit hours (35-36 or 39-45), core course structure, minor course count
  - **What general docs must cover:** 127-credit minimum, 64-credit SU residency, last 32 in-residence, 2.000 overall GPA, 2.000 major GPA, 60% major residency, Application for Diploma process
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` alongside `Majoring___Minoring.txt` for any graduation eligibility question

- **Topic:** Paideia general education requirements
  - **What major docs cover:** Feminism and Performance carries FAP and WA designations; Theatre Sustainability cross-listed with Environmental Studies; Theatre Dance cross-listed with Dance
  - **What general docs must cover:** Full FYS/AES, Language & Cultures, FRA, Power and Justice, Exploration & Breadth Parts I and II structure; Paideia with Distinction option
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` for any question about gen-ed, breadth requirements, or Paideia

- **Topic:** Pass/D/F grading option
  - **What major docs cover:** Not addressed
  - **What general docs must cover:** Full Pass/D/F policy, deadlines to switch, GPA impact
  - **Routing instruction:** Retrieve `SU_General_Policies.txt` for any grading option question

- **Topic:** Academic probation, dismissal, and appeal
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Good Standing definition, Warning, Probation, Dismissal thresholds, Academic Standards Committee appeal process
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Transfer credit rules
  - **What major docs cover:** Notes that 60% of major credits must be earned at SU; independent study and internship credits excluded from major
  - **What general docs must cover:** C- minimum required; P/CR grades not accepted; ELEC-0XX/3XX coding; 60% major residency rule; AP/IB/CAPE/CLEP policies
  - **Routing instruction:** Retrieve both `Majoring___Minoring.txt` AND `SU_General_Policies.txt`

- **Topic:** Study abroad credit and programs
  - **What major docs cover:** Theatre Arts in London course (fall program); London internship (6 credits); Semester in London contact info; NYAP program details; Sarofim Travel Grant
  - **What general docs must cover:** SAISS process, approved programs, deadlines, up to 19 transfer credits per semester, financial aid applicability
  - **Routing instruction:** Retrieve `SU_Resources_and_Financial.txt` for program details; retrieve `SU_General_Policies.txt` for credit transfer rules

- **Topic:** Financial aid impact of credit load changes or withdrawal
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Withdrawal and financial aid impact, tuition refund schedule, part-time financial aid thresholds
  - **Routing instruction:** Always retrieve `SU_Resources_and_Financial.txt`

- **Topic:** Tuition refund schedule
  - **What major docs cover:** Not covered
  - **What general docs must cover:** 80%/60%/50%/40%/30%/0% by week; summer refund schedule
  - **Routing instruction:** Always retrieve `SU_Resources_and_Financial.txt`

- **Topic:** Internship and experiential learning opportunities
  - **What major docs cover:** NYAP internship details, London CAPA internship (6 credits/6 weeks), Sarofim Travel Grant ($700/year), high-impact experience requirement (2 before graduation)
  - **What general docs must cover:** SURF, King Creativity Fund, academic vs. funded internship distinction, health professions shadowing
  - **Routing instruction:** Retrieve `SU_Resources_and_Financial.txt` for broader university internship funding programs

- **Topic:** Latin Honors and Paideia with Distinction
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Cum Laude/Magna Cum Laude/Summa Cum Laude GPA thresholds, Dean's List criteria, Paideia with Distinction requirements and deadlines
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Disability accommodations and ADA seating
  - **What major docs cover:** Box Office and Ticket Policies note ADA seating is available by calling 512-863-1378; no further detail
  - **What general docs must cover:** Services for Students with Disabilities, ADA accommodations for coursework, disability-related course substitution policy
  - **Routing instruction:** Retrieve `SU_General_Policies.txt` for academic disability accommodation; `Box_Office.txt` for performance seating; direct student to SSD office for academic accommodations

- **Topic:** Teacher certification in theatre
  - **What major docs cover:** `Majoring___Minoring.txt` states "See the Education Department for Information Regarding Teacher Certification in Theatre"
  - **What general docs must cover:** B.S.Ed. degree requirements, education certification requirements
  - **Routing instruction:** Refer student to Education Department; retrieve `SU_General_Policies.txt` for B.S.Ed. framework if needed

- **Topic:** Paired major and double major restrictions
  - **What major docs cover:** B.A. Theatre Arts described as suitable for students with a second major; no explicit restrictions stated
  - **What general docs must cover:** Paired major restrictions (cannot minor in either discipline; cannot declare as double major)
  - **Routing instruction:** Retrieve `SU_General_Policies.txt` for any double major or paired major policy question

---

## SECTION 8 — KNOWN GAPS

- **Topic:** Individual course credit hours
  - **Reason for gap:** Most course descriptions in provided documents do not specify credit hours (3 or 4 credits)
  - **Recommended fallback:** Direct student to the official SU course catalog or academic advisor

- **Topic:** Course offering schedule (which semester each course is offered)
  - **Reason for gap:** No documents indicate fall-only, spring-only, or alternate-year offering schedules for theatre courses
  - **Recommended fallback:** Direct student to current course schedule on SU registrar website or academic advisor

- **Topic:** Specific prerequisites for most theatre courses beyond Design Fundamentals and Advanced Directing
  - **Reason for gap:** Most course descriptions do not list explicit prerequisite course numbers
  - **Recommended fallback:** Direct student to academic advisor or course catalog

- **Topic:** Maximum enrollment / section availability for theatre courses
  - **Reason for gap:** Not addressed in any provided document
  - **Recommended fallback:** Direct student to registrar or department

- **Topic:** Grade requirements (minimum grade) for courses to count toward major
  - **Reason for gap:** General SU policy requires C- minimum for major credit, but theatre-specific documents do not restate this
  - **Recommended fallback:** Retrieve `SU_General_Policies.txt`; direct to advisor for confirmation

- **Topic:** Specific costs for Semester in London
  - **Reason for gap:** `Semester_in_London.txt` explicitly defers to the Office of Intercultural Learning for current costs, dates, and payment deadlines
  - **Recommended fallback:** Contact Tisha Temple — templet@southwestern.edu — (512) 863-1857

- **Topic:** Specific costs for NYAP internship program
  - **Reason for gap:** Not addressed in `New_York_Internships.txt`
  - **Recommended fallback:** Contact Dana Luna (SU Internship Coordinator, 512-863-1346) or NYAP directly (212-563-0255)

- **Topic:** Departmental Honors thesis/project requirements in Theatre
  - **Reason for gap:** Not addressed in any Theatre-specific document
  - **Recommended fallback:** Direct student to Department Chair Desiderio Roybal or `SU_General_Policies.txt`

- **Topic:** GPA requirement to maintain major or continue in the program
  - **Reason for gap:** Theatre documents do not specify a major GPA threshold beyond the university minimum (2.000); no Theatre-specific GPA gate identified
  - **Recommended fallback:** Retrieve `SU_General_Policies.txt` for university minimums; confirm with advisor

- **Topic:** Audition process for departmental productions (non-scholarship)
  - **Reason for gap:** Documents state any student may audition for productions but do not describe the audition process for regular season productions
  - **Recommended fallback:** Contact Theatre Department directly or check department announcements

- **Topic:** Alpha Psi Omega eligibility and membership requirements
  - **Reason for gap:** Referenced in `High_Impact_Learning_Experiences.txt` as an available experience but not described
  - **Recommended fallback:** Contact Theatre Department

- **Topic:** Mask and Wig Association activities and membership
  - **Reason for gap:** Referenced in `High_Impact_Learning_Experiences.txt` but not described
  - **Recommended fallback:** Contact Theatre Department

- **Topic:** Edinburgh festival program (Don Swan on the Royal Mile) details
  - **Reason for gap:** Mentioned in passing in `High_Impact_Learning_Experiences.txt` photo caption but not described as a current or recurring program
  - **Recommended fallback:** Contact Theatre Department for availability

- **Topic:** Specific 2025-2026 Music Season concerts (Sarofim Music Series)
  - **Reason for gap:** Box Office mentions the Music Series ticket discount but no document lists the concerts
  - **Recommended fallback:** Check Fine Arts Box Office website or calendar

- **Topic:** Production audition dates for 2025-2026 season
  - **Reason for gap:** Not in any provided document
  - **Recommended fallback:** Check Theatre Department announcements or contact department

---

## SECTION 9 — ROUTING DECISION GUIDE

**Default document for broad major questions:** `Majoring___Minoring.txt`

**When to retrieve multiple documents:**
- Any question about a specific course: retrieve `Majoring___Minoring.txt` (to confirm if required) AND the relevant course document (`Design_Courses.txt`, `Performance_Courses.txt`, or `History_and_Literature_Courses.txt`)
- Any graduation requirement question: retrieve `Majoring___Minoring.txt` AND `SU_General_Policies.txt`
- Any study abroad or internship question: retrieve the specific program document (`New_York_Internships.txt`, `Internships_Abroad.txt`, or `Semester_in_London.txt`) AND `High_Impact_Learning_Experiences.txt` AND `SU_Resources_and_Financial.txt`
- Any funding question: retrieve `High_Impact_Learning_Experiences.txt` AND `SU_Resources_and_Financial.txt`
- Any ticket/performance question: retrieve `Box_Office.txt` AND `Ticket_Policies.txt`
- Any question about where shows are performed: retrieve `Current_Season.txt` AND `Facilities.txt`

**Degree path disambiguation triggers:**
- If student says "theatre major" or "the theatre degree" without specifying B.A. Theatre Arts vs. B.A. Theatre Performance and Production, ask: "Are you in the Theatre Arts B.A. (designed for double majors, 35-36 credits) or the Theatre Performance and Production B.A. (39-45 credits)? Or are you asking about the Theatre minor?"
- If student asks about elective requirements without specifying degree, disambiguation is needed before routing to elective sections

**High-confidence single-document retrievals:**
- Current season productions, dates, venues → `Current_Season.txt`
- Box office hours, phone number, ticket purchasing methods → `Box_Office.txt`
- Ticket refund and exchange policies → `Ticket_Policies.txt`
- Parking and directions → `Parking_and_Directions.txt`
- Faculty and staff contact information → `Our_Faculty___Staff.txt`
- Scholarship audition requirements and deadline → `Theatre_Scholarship.txt`
- London semester contact → `Semester_in_London.txt`
- NYAP application deadlines and contact → `New_York_Internships.txt`
- London CAPA internship credit and duration → `Internships_Abroad.txt`
- Theatre facilities and performance spaces → `Facilities.txt`

**Always escalate to human advisor:**
- Exceptions to the "independent study/internship credits cannot count toward major" rule
- Course substitution or waiver requests
- Academic appeals (probation, dismissal)
- Transfer credit evaluation for specific theatre courses
- Teacher certification in theatre (refer to Education Department)
- Any question about current-year costs for study abroad programs
- Departmental Honors requirements
- Production casting or audition scheduling

**Always retrieve `SU_General_Policies.txt` alongside major docs when:**
- Any graduation eligibility or credit count question
- Any Paideia / general education requirement question
- Any grading policy question (Pass/D/F, grade appeals, incomplete grades)
- Any academic standing question (probation, dismissal, appeal)
- Any transfer credit or credit-by-examination question
- Any disability accommodation question (academic — not box office ADA seating)
- Any honors or distinction question (Latin Praise, Departmental Honors, Paideia with Distinction)
- Any double major or paired major policy question
- Teacher certification inquiry

**Always retrieve `SU_Resources_and_Financial.txt` alongside major docs when:**
- Any study abroad program question (deadlines, eligibility, credit transfer, costs)
- Any financial aid or tuition question
- Any refund or withdrawal-and-money question
- Any internship funding question beyond the Sarofim Travel Grant
- Any question about SURF or King Creativity Fund
# Skills Index — Philosophy
**Institution:** Southwestern University, Georgetown, Texas
**Last Ingested:** 2026-03-29
**Ingestion Model:** Claude Sonnet 4.6
**Document Count:** 7
**Degree Paths Covered:** B.A. in Philosophy, Minor in Philosophy

---

## SECTION 2 — DOCUMENT REGISTRY

### Philosophy Department Overview
- **degree_relevance:** All
- **description:** Provides the Philosophy Department's overarching mission, curricular philosophy, and strengths. States the department offers a "distinctive, innovative, and interdisciplinary approach." Emphasizes grounding in western philosophy while enabling critical engagement with contemporary questions. Highlights interdisciplinary intersections with Latin American Studies, Feminist Studies, Race and Ethnicity Studies, Queer Theory, and Marxist thought. Notes the department's active commitment to departing from philosophy's historically white-male-dominated tendency. Also references the A–Z Philosophy BlackList project (launched 2025 as part of Black History Month programming, now featuring 158+ names) and includes a featured alumni story (Lily Rodriguez '19, History major, interning at USCIS in D.C.).
- **critical_data:**
  - Department chair: Professor Michael Bray
  - Key strength areas: Latin American Studies, Feminist Studies, Race & Ethnicity Studies, Queer Theory, Marxist thought, media theory, political theory
  - A–Z Philosophy BlackList: launched 2025, 158 names as of 2026
  - Group advising available
  - Alumni pathways include archival research and public sector internships

---

### Philosophy Courses
- **degree_relevance:** B.A. | Minor
- **description:** Lists all current or recent course offerings in the Philosophy department, with full course descriptions and Paideia designations. Introductory-level offerings (numbered 18-104 through 18-194) include: Selected Introductory Topics (18-104), What Is Philosophy? (18-114), Critical Thinking (18-124), Philosophy of Love & Sex (18-144), Self, Ethics, Society (18-164), and Introduction to Feminist Philosophies (18-194). Upper-level courses (18-214 through 18-374) include Politics and Economics, Environmental Philosophy, Native American Philosophies, Philosophy Race & Revolution, Latin American Philosophy, Marxisms, The Embodied Self, 19th/20th Century Philosophy, Critical Philosophy of Race, and Feminist Ethics. The four upper-level Core Humanities sequences (18-614 through 18-644) are: Ethos/Identities/Differences, Being/Structure/Change, Experience/Language/Knowledge, and Peoples/Power/Organization — each requiring one prior Philosophy course. The Colloquium (18-914) is the capstone, required for majors in their final year, offered every fall.
- **critical_data:**
  - Course prefix: 18-XXX (Philosophy department = 18)
  - Capstone: 18-914 Colloquium in Philosophy — required for majors, offered every fall, prerequisite: 18-604
  - Upper-level Core (Critical Histories) courses (18-614, 18-624, 18-634, 18-644): each requires one prior Philosophy course
  - Several courses contribute to certificates: Legal Studies, Feminist Studies, Race & Ethnicity Studies, Environmental Studies, International Studies, Latin American and Border Studies, Spanish for Healthcare Professions
  - All courses carry (H) Humanities designation
  - 18-194, 18-284 also carry (SJ) and (PJ) designations
  - 18-914 carries (WA) Writing-Attentive designation
  - 18-614 through 18-644 carry (CRITICAL HISTORIES) designation

---

### High-Impact Learning Experiences
- **degree_relevance:** All
- **description:** Describes immersive high-impact learning opportunities specific to Philosophy majors at Southwestern. Covers two primary opportunities beyond general university options: student research via the Colloquium capstone course and the Philosophy Club student organization. The Colloquium is described as a collaborative workshop in which each student pursues an independent research project guided by the full philosophy faculty, culminating in a public presentation and final thesis submission. The Philosophy Club is a student organization promoting discussion of current philosophical topics and their societal relevance.
- **critical_data:**
  - Capstone (Colloquium) = independent research project + public presentation + final thesis
  - Philosophy faculty guide all capstone students collectively, not individually
  - Philosophy Club is an official student organization
  - Links to more detail on capstone projects and the Philosophy Club

---

### Student Research (Capstone Projects)
- **degree_relevance:** B.A.
- **description:** Provides detailed description of the Colloquium capstone structure and six example student research projects. Each semester, capstone students pursue independent research under collective philosophy faculty guidance. All students present their developing research to the class throughout the semester for critique and refinement. The culminating requirement is a public presentation of a key section followed by final thesis submission. Example projects cover: gentrification and capitalism (Kevin Lentz, using Neil Smith, David Harvey, Henri Lefebvre), women in philosophy and patriarchal structures (Sadie Clarendon, feminist philosophy), the exclusionary nature of Western philosophy and Latin alternative traditions (Alan Ramos), behavioral genetics and Aristotelian philosophy (Joshua Page), Nietzsche's ethics (Hunter White), and American media framing of terrorism through capitalist ideology (Sarah Cook, using Marx, Barthes, Debord, Virilio).
- **critical_data:**
  - Capstone structure: semester-long independent project + class critique throughout + public presentation of key section + final thesis
  - All faculty guide all students collectively
  - Research spans analytic and continental traditions, political philosophy, feminist philosophy, and philosophy of science
  - Six sample projects provided with specific texts used

---

### Philosophy Club
- **degree_relevance:** All
- **description:** Describes the Philosophy Club student organization in detail. Advised by Visiting Assistant Professor Katie Howard. Open to all students including non-philosophy majors and minors; the only requirement is an open mind and willingness to engage respectfully with difficult topics. Meetings feature a student-chosen topic or question written on the whiteboard, followed by open discussion about its significance and potential societal solutions. Often addresses controversial topics. Meets every Friday at 7 p.m. in Fondren–Jones Science Center, room 360. If the room is unavailable, students should text 940.536.7977 for the alternate location.
- **critical_data:**
  - Advisor: Visiting Assistant Professor Katie Howard
  - Meets: Fridays at 7 p.m., Fondren–Jones Science Center, room 360
  - Backup location text: 940.536.7977
  - Open to ALL students, not just philosophy majors
  - Format: topic/question on whiteboard → open discussion
  - No membership requirements except open mind

---

### Our Faculty
- **degree_relevance:** All
- **description:** Lists the current full-time philosophy faculty with names, titles, specialties, email addresses, phone numbers, and office locations. Three faculty members are listed: Professor Michael Bray (Department Chair, Political Philosophy & Critical Theory, Mood-Bridwell 142A, braym@southwestern.edu, 512.863.1131), Assistant Professor Jorge M. Lizarzaburu (Political Philosophy, Latin American Philosophy, Philosophy of Biology, Mood-Bridwell 143, lizarzaj@southwestern.edu, 512.863.1038), and Assistant Professor Zinhle ka'Nobuhlaluse (Critical Philosophy of Race, Feminist Philosophies focusing on South African and African American Black Feminisms, Decolonial Philosophies and Critical Apartheid Thought, kanobuhlaluse@southwestern.edu, 512.863.1133). Note: Philosophy Club advisor Katie Howard (Visiting Assistant Professor) is not listed in this document but appears in Philosophy_Club.txt.
- **critical_data:**
  - Michael Bray: Professor & Chair, Political Philosophy/Critical Theory, braym@southwestern.edu, 512.863.1131, Mood-Bridwell 142A
  - Jorge M. Lizarzaburu: Assistant Professor, Political Philosophy/Latin American Philosophy/Philosophy of Biology, lizarzaj@southwestern.edu, 512.863.1038, Mood-Bridwell 143
  - Zinhle ka'Nobuhlaluse: Assistant Professor, Critical Philosophy of Race/Feminist Philosophies/Decolonial Philosophies, kanobuhlaluse@southwestern.edu, 512.863.1133
  - Katie Howard: Visiting Assistant Professor (Philosophy Club advisor; contact info in Philosophy_Club.txt only)

---

### Notable Faculty & Student Achievements
- **degree_relevance:** All
- **description:** Chronological log of faculty publications, presentations, awards, and public engagements from 2020 through March 2026. Primarily documents the work of Assistant Professor Zinhle ka'Nobuhlaluse, including a Kristeller-Popkin Travel Fellowship ($4,000) from the Journal of the History of Philosophy (March 2025), presentations at major conferences (SPEP, Spindel Conference, philoSOPHIA, LAWP), publications in Simone de Beauvoir Studies and Feminist Formations, an essay in the APA Blog, and a workshop at University of Witwatersrand. Also includes: Professor Michael Bray's Crisis Point podcast episode (Sheffield/SPERI, Feb 2025) on democracy and populism, his Jacobin essay on Laclau and Mouffe (Aug 2023), Professor Phil Hopkins's multi-year consulting work with Austin Police Department (2020–2022) and related media coverage including a 60 Minutes feature, and Assistant Professor Jorge Lizarzaburu's 2023 teaching award and paper at the International Society for History, Philosophy, and Social Studies of Biology.
- **critical_data:**
  - ka'Nobuhlaluse: Kristeller-Popkin Fellowship ($4,000, March 2025); publications in Feminist Formations special issue, Simone de Beauvoir Studies, APA Blog, Caribbean Philosophical Association blog
  - Bray: Crisis Point podcast guest (Sheffield, 2025); Jacobin essay (2023); article in Spectre (2020)
  - Hopkins: APD consulting (2020–2022); featured in 60 Minutes; panelist for Austin Justice Coalition, I-AMM, National Association of Social Workers; guest on The Partially Examined Life podcast
  - Lizarzaburu: Teaching Award (2022-23); paper at ISHPSSB Toronto (2023)
  - Visiting Assistant Professor Gwen Daugs: paper at APA Eastern Division 2024

---

## SECTION 3 — DEGREE PATH SUMMARIES

### SU Universal Graduation Requirements (All Degree Paths)

These apply to ALL students regardless of major. Source: `SU_General_Policies.txt`

| Requirement | Detail |
|---|---|
| Minimum Total Credits | 127 credits |
| Minimum SU Residency Credits | 64 credits (last 32 must be in-residence) |
| Minimum Overall GPA | 2.000 (both cumulative and SU-only) |
| Minimum Major GPA | 2.000 average (no grade below C- counts toward major) |
| Major Residency Requirement | At least 60% of major credits at SU |
| Minor Residency Requirement | At least 12 credits at SU (if minor pursued) |
| Graduation Application | "Application for Diploma" must be filed; financial holds block diploma |
| Degree Conferral Dates | December, May, August only; Commencement once per year in May |

### SU Universal Curriculum Requirements (Paideia Framework)

- First-Year Seminar (FYS) or Advanced-Entry Seminar (AES): 4 credits
- Languages and Cultures: through third-semester proficiency
- Fitness and Recreational Activity (FRA): 1 credit
- Power and Justice (PJ) course: 1 course, 3–4 credits — **cannot be satisfied by transfer credit**
- Exploration and Breadth: 6 courses outside the major, one from each of FA, H, NS, ScS (Part I) + one additional from two of those areas (Part II)

**Note:** Philosophy courses carry the **(H) Humanities** Paideia designation and can satisfy the Humanities slot in Exploration & Breadth for non-majors. Courses 18-194 and 18-284 also carry **(PJ)** Power and Justice and **(SJ)** Social Justice designations.

---

### B.A. in Philosophy

**Note:** The provided documents do not include a formal degree requirements sheet (e.g., a requirements checklist or catalog page). The information below is inferred from course descriptions, the capstone requirement, and departmental references. For complete and authoritative B.A. requirements, retrieve `SU_General_Policies.txt` and direct student to academic advisor or course catalog.

| Field | Detail |
|---|---|
| Total Credit Hours | 127 (SU universal minimum) |
| Major Credit Hours | Not specified in provided documents — advisor confirmation required |
| Minimum GPA (Overall) | 2.000 |
| Minimum GPA (Major) | 2.000 |
| Residency Requirement | At least 60% of major credits at SU |
| Source Documents | `Courses.txt`, `High-Impact_Learning_Experiences.txt`, `Student_Research.txt` |

**Required Capstone:**
- 18-914 Colloquium in Philosophy — required of all majors, normally taken in the final year, offered every fall. Prerequisite: 18-604 (referenced in course listing but not described in the provided documents — advisor confirmation required).

**Observed Curriculum Structure (inferred from course catalog):**
- Introductory-level courses (18-104 through 18-194): serve as entry points; no prerequisites stated
- Upper-level courses (18-214 through 18-374): no stated prerequisite beyond implied progression
- Critical Histories upper-level core (18-614 through 18-644): each requires "one course in Philosophy"
- Capstone (18-914): requires completion of 18-604

**Curricular Strengths / Tracks (de facto, not official tracks):**
- Feminist Philosophy: 18-144, 18-194, 18-324, 18-354, 18-374
- Race & Decolonial Philosophy: 18-244, 18-254, 18-284, 18-354
- Political Economy / Marxism: 18-214, 18-314
- Ethics & Society: 18-164, 18-374
- Latin American Philosophy: 18-284, 18-314

**Certificate Contributions (courses that count toward other programs):**
- Legal Studies Certificate: 18-124, 18-164, 18-644
- Feminist Studies: 18-144, 18-194, 18-324, 18-354, 18-374
- Race and Ethnicity Studies: 18-144 (Allied), 18-194 (Allied), 18-244 (Group-Theme), 18-254 (Allied), 18-284 (Allied), 18-354 (Concepts)
- Environmental Studies: 18-234
- International Studies: 18-214, 18-244, 18-254, 18-284, 18-314, 18-354
- Latin American and Border Studies: 18-244, 18-254, 18-284
- Spanish for Healthcare Professions Certificate: 18-164

---

### Minor in Philosophy

**Note:** Formal minor requirements are not listed in the provided documents. The course catalog references a minor but does not specify credit hour minimums or required courses for the minor. Direct student to advisor or SU general catalog.

| Field | Detail |
|---|---|
| Credit Hours | Not specified in provided documents |
| Residency Requirement | At least 12 credits at SU |
| Source Documents | `Courses.txt`, `Philosophy.txt` |
| Recommended Action | Advising appointment required for minor declaration |

---

### Key Differences Between Degree Paths

- The B.A. requires the capstone (18-914 Colloquium); the minor does not.
- The B.A. requires 18-604 as a prerequisite for the capstone; the minor does not include this requirement.
- The minor offers flexibility to take any philosophy courses without the research/thesis obligation.
- Non-philosophy-major students taking philosophy courses for Paideia (H) credit are not pursuing either path and have no philosophy-specific requirements.
- **Disambiguation needed:** If a student says "philosophy requirements" without specifying major vs. minor, always clarify which they mean.

---

## SECTION 4 — COURSE INDEX

### Philosophy (18-XXX) Courses

**SU Course Numbering:** Prefix 18 = Philosophy. The 5th digit indicates credit hours. All courses below are 4 credit hours unless noted otherwise (digit 5 = 4 in all listed course numbers).

---

### 18-104 — Selected Introductory Topics
- **Credit Hours:** 4
- **Prerequisites:** None
- **Corequisites:** None
- **Offered:** Unknown (may repeat with change in topic)
- **Required For:** Not listed as required; counts as elective for majors
- **Counts As:** Introductory elective; (H) Humanities Paideia
- **Notes:** May be repeated with change in topic
- **Source:** `Courses.txt`

### 18-114 — What Is Philosophy?
- **Credit Hours:** 4
- **Prerequisites:** None
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Introductory; (H) Humanities Paideia
- **Notes:** Introduction to conceptions and practices of philosophy
- **Source:** `Courses.txt`

### 18-124 — Critical Thinking
- **Credit Hours:** 4
- **Prerequisites:** None
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Introductory; (H) Humanities Paideia; contributes to Legal Studies Certificate
- **Notes:** Covers logic, argumentation, fallacies; applicable to legal, moral, and political reasoning
- **Source:** `Courses.txt`

### 18-144 — Philosophy of Love & Sex
- **Credit Hours:** 4
- **Prerequisites:** None
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Introductory; (H) Humanities Paideia; contributes to Feminist Studies and Race & Ethnicity Studies (Allied)
- **Notes:** Explores ethical, religious, political, cultural dimensions of love and sex; uses diverse media formats
- **Source:** `Courses.txt`

### 18-164 — Self, Ethics, Society
- **Credit Hours:** 4
- **Prerequisites:** None
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Introductory; (H) Humanities Paideia; contributes to Legal Studies Certificate and Spanish for Healthcare Professions Certificate
- **Notes:** Focuses on self-development and ethical responsibility in social contexts
- **Source:** `Courses.txt`

### 18-194 — Introduction to Feminist Philosophies
- **Credit Hours:** 4
- **Prerequisites:** None
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Introductory; (H) (SJ) (PJ) Paideia designations; contributes to Feminist Studies and Race & Ethnicity Studies (Allied)
- **Notes:** Interdisciplinary; draws on philosophy, anthropology, sociology, psychoanalysis, cultural studies
- **Source:** `Courses.txt`

### 18-214 — Politics and Economics
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia; contributes to Economics, International Studies, Legal Studies Certificate
- **Notes:** Traces modern understanding of politics/economics as distinct spheres; examines Marxist, Polanyian, feminist critiques
- **Source:** `Courses.txt`

### 18-234 — Environmental Philosophy
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia; contributes to Environmental Studies
- **Notes:** Examines philosophical/ethical/political questions raised by climate change and ecological crisis
- **Source:** `Courses.txt`

### 18-244 — Native American Philosophies
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia; contributes to International Studies, Latin American and Border Studies, Race & Ethnicity Studies (Group-Theme)
- **Notes:** Surveys Andean, Nahua, Mayan, Haudenosaunee, Diné, Lakota traditions within Modernity/Coloniality/Decoloniality framework
- **Source:** `Courses.txt`

### 18-254 — Philosophy, Race & Revolution
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia; contributes to International Studies, Latin American and Border Studies, Race & Ethnicity Studies (Allied)
- **Notes:** Oriented by the Haitian Revolution; traces race and human rights through abolition, civil rights, anticolonial struggles
- **Source:** `Courses.txt`

### 18-284 — Latin American Philosophy
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) (SJ) (PJ) Paideia designations; contributes to International Studies, Race & Ethnicity Studies (Allied)
- **Notes:** Covers Bolívar, Martí, Vasconcelos, Alcoff, Mariátegui, Anzaldúa, Rivera Cusicanqui, Quijano, Mignolo, Freire
- **Source:** `Courses.txt`

### 18-314 — Marxisms
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia; contributes to International Studies
- **Notes:** Introduction to Marxist theory from Marx to present; covers class, gender, race, political organization, alternatives to capitalism
- **Source:** `Courses.txt`

### 18-324 — The Embodied Self
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia; contributes to Feminist Studies, Race & Ethnicity Studies (Allied)
- **Notes:** Explores psychological, phenomenological, anthropological, political, and epistemological dimensions of the self
- **Source:** `Courses.txt`

### 18-334 — 19th/20th Century Philosophy
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia
- **Notes:** Covers Hegel through poststructuralism; existentialism, phenomenology, psychoanalysis, standpoint epistemology
- **Source:** `Courses.txt`

### 18-354 — Critical Philosophy of Race
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia; contributes to Feminist Studies, International Studies, Race & Ethnicity Studies (Concepts Course)
- **Notes:** Examines race as constructed category; intersections with gender, class, nationalism, imperialism
- **Source:** `Courses.txt`

### 18-374 — Feminist Ethics
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as required
- **Counts As:** Upper-level; (H) Humanities Paideia; contributes to Feminist Studies
- **Notes:** Traces history of feminist ethics; covers human rights, social justice, transnational perspectives, biomedical ethics
- **Source:** `Courses.txt`

### 18-614 — Ethos, Identities, Differences
- **Credit Hours:** 4
- **Prerequisites:** One course in Philosophy
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Likely required for major (Critical Histories sequence); confirm with advisor
- **Counts As:** Upper-level core; (H) (CRITICAL HISTORIES) Paideia designations
- **Notes:** Examines interactions between individual and social identity; how societal ethos is formed and transformed by difference
- **Source:** `Courses.txt`

### 18-624 — Being, Structure, Change
- **Credit Hours:** 4
- **Prerequisites:** One course in Philosophy
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Likely required for major (Critical Histories sequence); confirm with advisor
- **Counts As:** Upper-level core; (H) (CRITICAL HISTORIES) Paideia designations
- **Notes:** Focuses on conceptual categories through which humans understand the world; ontology, structure, space, time, change
- **Source:** `Courses.txt`

### 18-634 — Experience, Language, Knowledge
- **Credit Hours:** 4
- **Prerequisites:** One course in Philosophy
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Likely required for major (Critical Histories sequence); confirm with advisor
- **Counts As:** Upper-level core; (H) (CRITICAL HISTORIES) Paideia designations
- **Notes:** Epistemology course; examines how knowledge is shaped by experience, language, perception, and history
- **Source:** `Courses.txt`

### 18-644 — Peoples, Power, Organization
- **Credit Hours:** 4
- **Prerequisites:** One course in Philosophy
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Likely required for major (Critical Histories sequence); confirm with advisor
- **Counts As:** Upper-level core; (H) (CRITICAL HISTORIES) Paideia designations; contributes to Legal Studies Certificate
- **Notes:** Focuses on collective political identities, power relations, social transformation; asks how philosophy relates to justice
- **Source:** `Courses.txt`

### 18-914 — Colloquium in Philosophy
- **Credit Hours:** 4
- **Prerequisites:** 18-604 (note: 18-604 is listed as prerequisite but not described in any provided document — confirm with advisor or catalog)
- **Corequisites:** None
- **Offered:** Every fall
- **Required For:** B.A. in Philosophy (required of all majors, normally in their final year)
- **Counts As:** Capstone; (H) (WA) Paideia designations
- **Notes:** Collaborative workshop format; each student pursues independent research project; all students critique each other's work throughout semester; culminates in public presentation and final thesis
- **Source:** `Courses.txt`, `High-Impact_Learning_Experiences.txt`, `Student_Research.txt`

---

---

## SECTION 6 — QUERY PATTERN MAP

**Q:** I'm a freshman and I'm interested in philosophy. Where do I start?
**Docs:** `Philosophy.txt`, `Courses.txt`
**Routing Note:** Overview doc explains department mission and strengths; course catalog lists introductory options (18-104 through 18-194) with no prerequisites.
**Disambiguation Needed:** No

**Q:** What is the philosophy major like at Southwestern?
**Docs:** `Philosophy.txt`, `Courses.txt`
**Routing Note:** Philosophy.txt gives the department overview; Courses.txt provides the full curriculum.
**Disambiguation Needed:** No

**Q:** What's the difference between the philosophy major and the philosophy minor?
**Docs:** `Courses.txt`, `High-Impact_Learning_Experiences.txt`
**Routing Note:** The capstone (18-914) is required for the major but not the minor; formal minor credit requirements not in any document — direct to advisor.
**Disambiguation Needed:** No, but note gap in minor requirements

**Q:** Do I need to take Critical Thinking before I can take upper-level philosophy courses?
**Docs:** `Courses.txt`
**Routing Note:** Course prerequisites section: 18-614 through 18-644 require "one course in Philosophy" but 18-214 through 18-374 have no stated prerequisite.
**Disambiguation Needed:** No

**Q:** What is Philosophy 18-614?
**Docs:** `Courses.txt`
**Routing Note:** Course index entry for 18-614 Ethos, Identities, Differences with full description and prerequisite.
**Disambiguation Needed:** No

**Q:** What does the philosophy capstone involve?
**Docs:** `Student_Research.txt`, `High-Impact_Learning_Experiences.txt`, `Courses.txt`
**Routing Note:** All three documents describe the capstone; Student_Research.txt has the most detail on structure and examples; Courses.txt lists prerequisites and offering schedule.
**Disambiguation Needed:** No

**Q:** When is the Colloquium offered?
**Docs:** `Courses.txt`
**Routing Note:** 18-914 is offered every fall; this is stated explicitly in Courses.txt.
**Disambiguation Needed:** No

**Q:** Can you show me examples of past philosophy capstone projects?
**Docs:** `Student_Research.txt`
**Routing Note:** Six sample capstone research projects with descriptions and source texts are listed in Student_Research.txt.
**Disambiguation Needed:** No

**Q:** I'm a sophomore trying to decide between the B.A. and the minor. What's the commitment difference?
**Docs:** `Courses.txt`, `High-Impact_Learning_Experiences.txt`
**Routing Note:** B.A. requires capstone (18-914) + 18-604 prerequisite; minor credit requirements are not documented — advising appointment needed.
**Disambiguation Needed:** No, but flag gap in minor requirements

**Q:** I want to add philosophy as a minor. How many credits do I need?
**Docs:** `Courses.txt` (partial), `SU_General_Policies.txt`
**Routing Note:** Minor credit requirement not stated in major documents; SU general policies cover minor residency (12 SU credits minimum) but full minor requirements require advisor or catalog.
**Disambiguation Needed:** No

**Q:** I'm thinking about double majoring in philosophy and something else. Can I do that?
**Docs:** `SU_General_Policies.txt`
**Routing Note:** Double major policies, paired major restrictions, and credit hour implications are covered only in SU_General_Policies.txt.
**Disambiguation Needed:** Yes — ask what the second major is, since paired majors cannot be declared as double majors

**Q:** Does taking philosophy courses count toward any Paideia requirements?
**Docs:** `Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Courses.txt shows all philosophy courses carry (H) Humanities designation; 18-194 and 18-284 carry (PJ) and (SJ); full Paideia structure is in SU_General_Policies.txt.
**Disambiguation Needed:** No

**Q:** Which philosophy courses count toward the Power and Justice requirement?
**Docs:** `Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Courses.txt shows (PJ) designation on 18-194 and 18-284; SU_General_Policies.txt defines the PJ requirement fully.
**Disambiguation Needed:** No

**Q:** I'm a junior. Do any philosophy courses count toward a Feminist Studies certificate?
**Docs:** `Courses.txt`
**Routing Note:** Course descriptions in Courses.txt explicitly note which courses contribute to Feminist Studies (18-144, 18-194, 18-324, 18-354, 18-374).
**Disambiguation Needed:** No

**Q:** I failed a required philosophy course. What happens to my major GPA and what are my options?
**Docs:** `Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Courses.txt shows capstone prerequisite chain; SU_General_Policies.txt covers course repeat policies, GPA recalculation, academic standing, and minimum GPA of 2.000 for major.
**Disambiguation Needed:** Yes — ask which course was failed and whether it was a required or elective course

**Q:** I'm applying for graduation. What do I need to check for philosophy?
**Docs:** `Courses.txt`, `High-Impact_Learning_Experiences.txt`, `SU_General_Policies.txt`
**Routing Note:** Courses.txt confirms 18-914 is the capstone requirement; SU_General_Policies.txt covers Application for Diploma, financial holds, credit minimums, GPA requirements.
**Disambiguation Needed:** No

**Q:** Can transfer credits count toward the philosophy major?
**Docs:** `SU_General_Policies.txt`
**Routing Note:** Transfer credit policies (C- minimum, P/CR not accepted, 60% major residency, ELEC coding) are covered only in SU_General_Policies.txt; major documents do not address transfer credit specifically.
**Disambiguation Needed:** No, but flag need for advisor to evaluate specific transfer courses

**Q:** I took AP courses in high school. Can any of them count for philosophy?
**Docs:** `SU_General_Policies.txt`
**Routing Note:** AP credit policy (scores of 4-5) is in SU_General_Policies.txt; there is no philosophy-specific AP course, so AP credits would likely count as general electives, not major credits.
**Disambiguation Needed:** No

**Q:** Who should I talk to about advising for the philosophy major?
**Docs:** `Our_Faculty.txt`, `Philosophy.txt`
**Routing Note:** Our_Faculty.txt lists all faculty with contact info; Philosophy.txt references group advising; the department chair is Michael Bray.
**Disambiguation Needed:** No

**Q:** How do I contact Professor Lizarzaburu?
**Docs:** `Our_Faculty.txt`
**Routing Note:** Our_Faculty.txt lists lizarzaj@southwestern.edu, 512.863.1038, Mood-Bridwell 143.
**Disambiguation Needed:** No

**Q:** What does Professor ka'Nobuhlaluse research?
**Docs:** `Our_Faculty.txt`, `Notable_Faculty___Student_Achievements.txt`
**Routing Note:** Our_Faculty.txt gives specialty; Achievements doc provides extensive detail on recent publications, grants, and conference presentations.
**Disambiguation Needed:** No

**Q:** Has any philosophy professor won any grants or awards recently?
**Docs:** `Notable_Faculty___Student_Achievements.txt`
**Routing Note:** Achievements doc covers ka'Nobuhlaluse's Kristeller-Popkin Fellowship ($4,000, 2025), Lizarzaburu's teaching award (2023), and other achievements.
**Disambiguation Needed:** No

**Q:** Is there a philosophy club I can join?
**Docs:** `Philosophy_Club.txt`, `High-Impact_Learning_Experiences.txt`
**Routing Note:** Both documents confirm the Philosophy Club; Philosophy_Club.txt has meeting time, location, format, and advisor contact.
**Disambiguation Needed:** No

**Q:** When and where does philosophy club meet?
**Docs:** `Philosophy_Club.txt`
**Routing Note:** Fridays at 7 p.m., Fondren–Jones Science Center, room 360; backup contact: 940.536.7977.
**Disambiguation Needed:** No

**Q:** I'm not a philosophy major. Can I go to philosophy club?
**Docs:** `Philosophy_Club.txt`
**Routing Note:** Philosophy_Club.txt explicitly states the club is open to all students, not just philosophy majors or minors.
**Disambiguation Needed:** No

**Q:** What philosophy courses count toward the Legal Studies certificate?
**Docs:** `Courses.txt`
**Routing Note:** Courses.txt notes 18-124, 18-164, and 18-644 contribute to the Legal Studies Certificate.
**Disambiguation Needed:** No

**Q:** Does philosophy offer anything related to environmental studies?
**Docs:** `Courses.txt`
**Routing Note:** 18-234 Environmental Philosophy contributes to Environmental Studies; description focuses on climate change and ecological crisis.
**Disambiguation Needed:** No

**Q:** I want to study abroad. Will those credits count toward my philosophy degree?
**Docs:** `SU_Resources_and_Financial.txt`, `SU_General_Policies.txt`
**Routing Note:** Study abroad credit transfer (up to 19 credits per semester, SAISS process) is covered in SU_Resources_and_Financial.txt; credit transfer rules are in SU_General_Policies.txt; philosophy major documents do not address study abroad specifically.
**Disambiguation Needed:** Yes — advise student to also check with philosophy advisor about which courses abroad would be approved for major credit

**Q:** I'm struggling with my GPA. Am I in danger of being dismissed?
**Docs:** `SU_General_Policies.txt`
**Routing Note:** Academic probation, warning, and dismissal thresholds are exclusively in SU_General_Policies.txt. Philosophy major requires 2.000 overall and major GPA minimum.
**Disambiguation Needed:** No, but always recommend advisor meeting

**Q:** Can I take any philosophy courses Pass/Fail?
**Docs:** `SU_General_Policies.txt`, `Courses.txt`
**Routing Note:** SU uses Pass/D/F (not simply Pass/Fail); full policy and deadlines are in SU_General_Policies.txt; Courses.txt does not specify any philosophy course restrictions on P/D/F.
**Disambiguation Needed:** No

**Q:** What is the A-Z Philosophy BlackList?
**Docs:** `Philosophy.txt`
**Routing Note:** Philosophy.txt mentions this as a department initiative launched in 2025 as Black History Month programming, now an expanding intellectual archive with 158+ names as of 2026.
**Disambiguation Needed:** No

**Q:** What are the research interests of the philosophy department as a whole?
**Docs:** `Philosophy.txt`, `Our_Faculty.txt`, `Notable_Faculty___Student_Achievements.txt`
**Routing Note:** Philosophy.txt outlines departmental strengths; Our_Faculty.txt lists individual faculty specialties; Achievements doc shows recent active research areas.
**Disambiguation Needed:** No

**Q:** I withdrew from a philosophy course. Does that affect my financial aid?
**Docs:** `SU_Resources_and_Financial.txt`, `SU_General_Policies.txt`
**Routing Note:** Financial aid impact of withdrawal and tuition refund schedules are exclusively in SU_Resources_and_Financial.txt; withdrawal policies are in SU_General_Policies.txt.
**Disambiguation Needed:** No

**Q:** What is 18-604 and do I need it before the capstone?
**Docs:** `Courses.txt`
**Routing Note:** 18-914 lists 18-604 as a prerequisite; however, 18-604 is not described or listed as a standalone course in any provided document. This is a known gap — direct student to advisor or course catalog.
**Disambiguation Needed:** No, but flag as gap

**Q:** What's the difference between 18-354 Critical Philosophy of Race and 18-254 Philosophy, Race & Revolution?
**Docs:** `Courses.txt`
**Routing Note:** Both course descriptions are in Courses.txt; 18-254 is historically oriented (starting with Haitian Revolution), while 18-354 is a contemporary survey of critical race philosophy with intersectional focus.
**Disambiguation Needed:** No

---

## SECTION 7 — CROSS-REFERENCE FLAGS

- **Topic:** Graduation requirements (credit minimums, residency, GPA)
  - **What major docs cover:** Capstone requirement (18-914); informal curriculum structure
  - **What general docs must cover:** 127-credit minimum, 64-credit SU residency, last 32 in-residence, 2.000 GPA, 60% major residency, Application for Diploma process
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` alongside major requirements doc for any graduation eligibility question

- **Topic:** Paideia general education requirements
  - **What major docs cover:** Courses.txt lists (H), (PJ), (SJ), (WA), (CRITICAL HISTORIES) designations for each course
  - **What general docs must cover:** Full structure of FYS/AES, Language & Cultures, FRA, Power and Justice, Exploration & Breadth Parts I and II, Paideia with Distinction
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` for any question about gen-ed, breadth requirements, or Paideia

- **Topic:** Pass/D/F grading option
  - **What major docs cover:** Not addressed in any philosophy document
  - **What general docs must cover:** Full Pass/D/F policy, deadlines to switch, GPA impact
  - **Routing instruction:** Retrieve `SU_General_Policies.txt` for any grading option question

- **Topic:** Academic probation, dismissal, and appeal
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Good Standing definition, Warning, Probation, Dismissal thresholds, Academic Standards Committee appeal process
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Transfer credit rules
  - **What major docs cover:** Not addressed; no philosophy-specific transfer credit guidance provided
  - **What general docs must cover:** C- minimum required; P/CR grades not accepted; ELEC-0XX/3XX coding; 60% major residency rule; AP/IB/CAPE/CLEP policies
  - **Routing instruction:** Retrieve both `Courses.txt` (for which courses count toward major) and `SU_General_Policies.txt`; advise student to meet with philosophy advisor to evaluate specific transfer courses

- **Topic:** Study abroad credit and programs
  - **What major docs cover:** Not mentioned in any philosophy document
  - **What general docs must cover:** SAISS process, approved programs (London, ISEP, NYAP, CHIP, CYA), deadlines, up to 19 transfer credits per semester, financial aid applicability
  - **Routing instruction:** Retrieve `SU_Resources_and_Financial.txt` for program details; retrieve `SU_General_Policies.txt` for credit transfer rules; advise student to also consult philosophy advisor about major credit applicability

- **Topic:** Financial aid impact of credit load changes or withdrawal
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Withdrawal and financial aid impact, tuition refund schedule, part-time financial aid thresholds
  - **Routing instruction:** Always retrieve `SU_Resources_and_Financial.txt`

- **Topic:** Tuition refund schedule
  - **What major docs cover:** Not covered
  - **What general docs must cover:** 80%/60%/50%/40%/30%/0% by week; summer refund schedule
  - **Routing instruction:** Always retrieve `SU_Resources_and_Financial.txt`

- **Topic:** Internship and experiential learning funding
  - **What major docs cover:** Not addressed; no philosophy-specific funded internship programs listed
  - **What general docs must cover:** SURF, King Creativity Fund, health professions shadowing, NYAP, CHIP
  - **Routing instruction:** Retrieve `SU_Resources_and_Financial.txt` for program details

- **Topic:** Latin Honors and Paideia with Distinction
  - **What major docs cover:** Not addressed
  - **What general docs must cover:** Cum Laude/Magna Cum Laude/Summa Cum Laude GPA thresholds, Dean's List criteria, Paideia with Distinction requirements and deadlines
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Disability accommodations and course substitution
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Services for Students with Disabilities, ADA accommodations, disability-related course substitution policy and appeals
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`; direct student to SSD office

- **Topic:** Double major with philosophy
  - **What major docs cover:** Not addressed
  - **What general docs must cover:** Paired major restrictions (cannot minor in either discipline; cannot declare as double major if paired), 127+ credit implications
  - **Routing instruction:** Retrieve `SU_General_Policies.txt`; note that philosophy is in the Humanities area and B.S. requires a Natural Sciences or Psychology first major

- **Topic:** Course repeat policy
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Course repeat limits, GPA recalculation rules
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Minor in Philosophy — formal requirements
  - **What major docs cover:** Not covered in any provided document
  - **What general docs must cover:** Minimum 12 SU credits for minor residency
  - **Routing instruction:** Retrieve `SU_General_Policies.txt` for residency; direct student to advisor or course catalog for credit hour and course requirements for the minor

---

## SECTION 8 — KNOWN GAPS

- **Topic:** B.A. in Philosophy — total credit hours required for the major
  - **Reason for gap:** No degree requirements sheet or catalog page was included in the provided documents; total major credit hours are not stated anywhere
  - **Recommended fallback:** Direct student to academic advisor or the official SU course catalog; retrieve `SU_General_Policies.txt` for universal 127-credit graduation requirement

- **Topic:** Minor in Philosophy — credit hours and required courses
  - **Reason for gap:** No minor requirements document was provided; minor is referenced but not defined
  - **Recommended fallback:** Direct student to academic advisor or official course catalog

- **Topic:** 18-604 — course description and content
  - **Reason for gap:** 18-604 is listed as a prerequisite for 18-914 in Courses.txt but is never described or listed as a course offering in any provided document
  - **Recommended fallback:** Direct student to advisor or course catalog to confirm 18-604 requirements

- **Topic:** Offering schedules for individual courses (fall vs. spring vs. both)
  - **Reason for gap:** Courses.txt does not specify offering semesters for any course except 18-914 (every fall) and the general note that the capstone is offered annually
  - **Recommended fallback:** Direct student to advisor or current semester course schedule

- **Topic:** Required vs. elective courses for the major
  - **Reason for gap:** No formal degree requirements checklist was provided; which courses are required versus elective for the B.A. is not explicitly stated
  - **Recommended fallback:** Direct student to advisor; the Critical Histories sequence (18-614 through 18-644) and 18-914 are clearly required; all other requirements need advisor confirmation

- **Topic:** GPA required for major declaration or continuation
  - **Reason for gap:** No philosophy-specific GPA thresholds beyond the universal 2.000 minimum are stated in any provided document
  - **Recommended fallback:** Retrieve `SU_General_Policies.txt`; advise student to confirm with advisor

- **Topic:** Philosophy internship or career advising resources
  - **Reason for gap:** No career services or internship information specific to philosophy majors is present in any provided document
  - **Recommended fallback:** Direct student to SU Career Services or academic advisor; retrieve `SU_Resources_and_Financial.txt` for general funded internship opportunities

- **Topic:** Current semester course schedule and section availability
  - **Reason for gap:** Not available in any static document; real-time information required
  - **Recommended fallback:** Direct student to SU course registration system or advisor

- **Topic:** Group advising schedule and sign-up process
  - **Reason for gap:** Philosophy.txt references "Philosophy Group Advising" but provides no dates, times, or sign-up process
  - **Recommended fallback:** Direct student to philosophy department or advising office

- **Topic:** Paideia Minor — whether a philosophy minor can constitute a Paideia Minor
  - **Reason for gap:** Not addressed in any provided document
  - **Recommended fallback:** Retrieve `SU_General_Policies.txt`; direct student to advisor

---

## SECTION 9 — ROUTING DECISION GUIDE

**Default document for broad major questions:** `Philosophy.txt` for mission/overview; `Courses.txt` for curriculum and course-specific questions

**When to retrieve multiple documents:**
- Any question about the capstone: retrieve `Courses.txt` + `Student_Research.txt` + `High-Impact_Learning_Experiences.txt`
- Any question about graduation eligibility: retrieve `Courses.txt` + `SU_General_Policies.txt`
- Any question about Paideia credit from philosophy courses: retrieve `Courses.txt` + `SU_General_Policies.txt`
- Any question about faculty research or contact: retrieve `Our_Faculty.txt` + `Notable_Faculty___Student_Achievements.txt`
- Any question about minor requirements: retrieve `Courses.txt` + `SU_General_Policies.txt` + escalate to advisor
- Any question about transfer credit: retrieve `SU_General_Policies.txt` + advise student to meet with philosophy advisor
- Any question about study abroad and philosophy credit: retrieve `SU_Resources_and_Financial.txt` + `SU_General_Policies.txt` + escalate to advisor
- Any question about Philosophy Club: retrieve `Philosophy_Club.txt` (single document sufficient)

**Degree path disambiguation triggers:**
- If student says "the philosophy degree" or "philosophy requirements" without specifying major vs. minor, ask: "Are you asking about the philosophy major (B.A.) or the philosophy minor?"
- If student asks about credit hours for the philosophy program without specifying, always flag that the minor credit requirements are not available in current documents

**High-confidence single-document retrievals:**
- Philosophy Club meeting time/location → `Philosophy_Club.txt`
- Individual course description → `Courses.txt`
- Faculty contact information → `Our_Faculty.txt`
- Faculty publications and awards → `Notable_Faculty___Student_Achievements.txt`
- Sample capstone project topics → `Student_Research.txt`
- Department mission and values → `Philosophy.txt`

**Always escalate to human advisor:**
- Any question about declaring or changing the philosophy major or minor
- Any question about course substitutions or waivers
- Any question about whether a specific transfer course counts toward the major
- Any question about 18-604 content or alternatives
- Any question about formal minor credit hour requirements
- Any question requiring knowledge of required vs. elective courses in the major
- Academic appeals, GPA concerns affecting standing, or withdrawal decisions
- Any exception to published policies

**Always retrieve `SU_General_Policies.txt` alongside major docs when:**
- Any graduation eligibility or credit count question
- Any Paideia / general education requirement question
- Any grading policy question (Pass/D/F, grade appeals, incomplete grades)
- Any academic standing question (probation, dismissal, appeal)
- Any transfer credit or credit-by-examination question
- Any disability accommodation question
- Any honors or distinction question (Latin Praise, Departmental Honors, Paideia with Distinction)
- Any double major or paired major question

**Always retrieve `SU_Resources_and_Financial.txt` alongside major docs when:**
- Any study abroad program question (deadlines, eligibility, credit transfer)
- Any financial aid or tuition question
- Any refund or withdrawal-and-money question
- Any internship funding question (SURF, King Creativity Fund, funded internships)
- Any health professions experience question (St. David's, Houston Methodist)

# Skills Index — Political Science
**Institution:** Southwestern University, Georgetown, Texas
**Last Ingested:** 2026-03-30
**Ingestion Model:** Claude Sonnet 4.6
**Document Count:** 7
**Degree Paths Covered:** B.A. in Political Science, Minor in Political Science

---

## SECTION 1 — METADATA BLOCK

This index covers all documents provided for the Political Science department at Southwestern University. It supports an AI routing model that retrieves relevant documents to answer student questions. The documents cover degree requirements, course offerings, faculty, the honors thesis process, and current/future course schedules.

---

## SECTION 2 — DOCUMENT REGISTRY

### Political Science — Department Overview
- **degree_relevance:** All
- **description:** Provides a high-level introduction to the Political Science major and minor at Southwestern University. Explains why studying political science matters, the range of topics covered (campaigns and elections, domestic and foreign policy, international politics, civil discourse, media and politics, gender and politics, political leadership, revolution and conflict, ideologies), and the analytical and communication skills students develop. Notes career paths for graduates including law, government, K–12 and higher education, nonprofits, urban planning, business, and international affairs. Includes brief alumni spotlights: Sierra Rupp '23 earned a Critical Language Scholarship and Fulbright grant; Alexandria Glenn '26 and Zaria Renfro '28 earned Gilman Scholarships for study abroad. Does not contain course lists or specific degree requirements.
- **critical_data:**
  - Topics covered: campaigns/elections, foreign/domestic policy, international politics, gender and politics, revolution and conflict, ideologies
  - Career outcomes: law, government, K–12 and higher education, nonprofits, urban planning, business, international affairs
  - Alumni awards: Critical Language Scholarship, Fulbright, Gilman Scholarship
  - Does NOT specify credit hours or course requirements

---

### Majoring and Minoring in Political Science
- **degree_relevance:** B.A., Minor
- **description:** Specifies the exact course requirements for both the B.A. major (9 courses) and the Minor (5 courses) in Political Science. The major requires four core courses — PSC32-114 American Politics, PSC32-144 Comparative Politics, PSC32-364 Introduction to Political Theory, and PSC32-384 International Politics — plus PSC32-964 Senior Seminar (Capstone), two additional Political Science courses from any subfield, and two Political Science courses at the 500–600 level (at least one taken prior to the Senior Seminar). The minor requires the same four core courses plus one additional approved course. Also references teacher certification in Political Science through the Education Department. Explains that the major cultivates critical thinking, creative communication, analysis of decision-making processes, and political analysis of social data and texts.
- **critical_data:**
  - Major: 9 courses total
  - Core required: PSC32-114, PSC32-144, PSC32-364, PSC32-384, PSC32-964 (Capstone)
  - Electives: 2 additional courses from any subfield
  - Upper-level: 2 courses at 500–600 level (at least 1 before Senior Seminar)
  - Minor: 5 courses — same 4 core + 1 additional approved course
  - Teacher certification available through Education Department

---

### Political Science — Course Offerings
- **degree_relevance:** B.A., Minor
- **description:** Lists all current and recent course offerings in the Political Science department with full course descriptions. Covers introductory courses (PSC32-114 through PSC32-284), mid-level courses (PSC32-324 through PSC32-464), advanced 500–600 level courses (PSC32-504 through PSC32-654), and the capstone (PSC32-964). Each entry includes course number, full title, description, prerequisites (where applicable), and Paideia designations (ScS, SJ, PJ, WA). Courses span subfields: American Politics, Comparative Politics, International Relations, Political Theory, Race & Ethnic Politics, Gender & Politics, and area studies (Latin America, Middle East, Japan, China, Europe, Germany/Japan). This is the primary source for course descriptions, prerequisites, and cross-listed/contributing programs.
- **critical_data:**
  - 28 courses listed across all levels
  - Introductory courses (no prerequisites): PSC32-114, PSC32-144, PSC32-184, PSC32-194, PSC32-204 (no prereq stated), PSC32-224, PSC32-234, PSC32-264, PSC32-284
  - Mid-level prerequisites include PSC32-114 or PSC32-144 for most 300-level courses
  - 500–600 level prereqs: PSC32-364 or PSC32-384 (or instructor permission)
  - PSC32-344 Texas Legislative Internship: requires 8 credits in PSC, offered every other Spring during TX legislative session
  - PSC32-964 Senior Seminar: requires instructor permission
  - Many courses carry Paideia designations: (ScS), (SJ), (PJ), (WA)
  - Several courses contribute to other programs: International Studies, Latin American and Border Studies, Race and Ethnicity Studies, Feminist Studies, Legal Studies Certificate, East Asian Studies, Data Science

---

### For Current Students — Tentative Course Schedules
- **degree_relevance:** All
- **description:** Lists tentative course offerings by semester for Spring 2024, Fall 2024, and Spring 2025, including instructor assignments. Spring 2024 offerings include American Politics (Goodwin), Comparative Politics (Aha), Politics of Latin America and the Caribbean (Selbin), Governing Diversity (Goodwin), Authoritarianism and Democracy in East Central Europe (Aha), International Politics (Snyder, WA), Candidates/Campaigns/Citizens (Sydnor), Public Opinion 500–600 (Sydnor), U.S. Foreign Policy 500–600 (Snyder), and Capstone (Selbin). Fall 2024 includes two sections of American Politics (Goodwin), plus upper-level and 500–600 level courses. Spring 2025 includes the Texas Legislative Internship (Sydnor, offered in session years only), American Political Thought, Resistance/Rebellion/Revolution 500–600, and others.
- **critical_data:**
  - Spring 2024: 10 courses, instructors assigned
  - Fall 2024: 9 courses including 2 sections of American Politics
  - Spring 2025: 9 courses including Texas Legislative Internship
  - 500–600 level courses identified per semester (relevant for capstone prerequisites)
  - Labeled "tentative" — not guaranteed
  - Texas Legislative Internship only offered Spring 2025 (legislative session year)
  - Several TBA instructor slots in Fall 2024 and Spring 2025

---

### Our Faculty
- **degree_relevance:** All
- **description:** Lists the full-time and visiting faculty in the Political Science department with names, titles, email addresses, and phone numbers. Faculty include: Alisa Gaunder (Professor), Alex Goodwin (Assistant Professor, specialty: American Politics, Race and Ethnic Politics, Black Politics, Policing, Social Movements), Zoe Moss (Assistant Professor), Alexander Norris (Visiting Assistant Professor), and Bob Snyder (Professor, specialty: International Politics, U.S. Foreign Policy, Middle East Politics, Terrorism). Office locations are listed for Gaunder (CB 159) and Snyder (Mood-Bridwell 228). Moss and Norris do not have specialties or office locations listed. This is the primary contact source for students seeking advisor information.
- **critical_data:**
  - 5 faculty listed (4 full-time + 1 visiting)
  - Gaunder: gaundera@southwestern.edu, 512.863.1418, CB 159
  - Goodwin: goodwina@southwestern.edu, 512.863.1319
  - Moss: mossz@southwestern.edu, 512.863.1602
  - Norris: norris3@southwestern.edu (visiting, no phone listed)
  - Snyder: snyderb@southwestern.edu, 512.863.1938, Mood-Bridwell 228

---

### The Honors Thesis
- **degree_relevance:** B.A.
- **description:** Describes the Honors Thesis program in Political Science, which is available by invitation to exceptional junior-year majors. Criteria for invitation include: demonstrated potential for independent research, interest in graduate education (preferred but not required), overall GPA of 3.4 or higher (required by the university), strong performance across multiple faculty members' courses, and unanimous departmental approval. The timeline spans the junior year spring semester (PSC32-951, 1 credit Independent Study for proposal writing) and both semesters of the senior year (PSC32-984 Honors, 4 credits each semester). Students assemble a three-member committee, must present at an academic conference or the SU Student Works Symposium, and earn the distinction "Honors in Political Science" upon successful defense. The Honors Thesis replaces the Senior Seminar (Capstone). Contrasts with Capstone: Honors is year-long, graduate-level expectations; Capstone is one semester, all majors required.
- **critical_data:**
  - Invitation extended in fall of junior year by unanimous faculty vote
  - GPA minimum: 3.4 overall (university requirement)
  - Research proposal must be unanimously approved by all 5 full-time faculty
  - Course numbers: PSC32-951 (1 cr, spring junior year), PSC32-984 (4 cr × 2 semesters senior year)
  - Three-member committee: primary advisor (dept), second reader (dept), outside reader (another dept)
  - Must present at academic conference OR SU Student Works Symposium
  - Replaces PSC32-964 Senior Seminar
  - Not all completed theses meet the criteria for graduating with Honors

---

### Notable Faculty and Student Achievements
- **degree_relevance:** All
- **description:** Contains information about awards, honors, publications, grants, or recognitions earned by faculty and students in the Political Science department. This document may include items such as research grants, fellowships, national awards, presentations, and publications. Content is supplementary context for prospective and current students who want to understand the department's academic profile and reputation.
- **critical_data:**
  - Content not fully available for direct citation in this index; consult file directly for specifics
  - Likely overlaps with alumni achievement information in `Political_Science.txt`

---

## SECTION 3 — DEGREE PATH SUMMARIES

### SU Universal Graduation Requirements (All Degree Paths)

Source: `SU_General_Policies.txt`

| Requirement | Detail |
|---|---|
| Minimum Total Credits | 127 credits |
| Minimum SU Residency Credits | 64 credits (last 32 must be in-residence) |
| Minimum Overall GPA | 2.000 (both cumulative and SU-only) |
| Minimum Major GPA | 2.000 average (no grade below C- counts toward major) |
| Major Residency Requirement | At least 60% of major credits at SU |
| Minor Residency Requirement | At least 12 credits at SU (if minor pursued) |
| Graduation Application | "Application for Diploma" must be filed; financial holds block diploma |
| Degree Conferral Dates | December, May, August only; Commencement held once per year in May |

### SU Universal Curriculum Requirements (Paideia Framework)

Source: `SU_General_Policies.txt`

- **FYS/AES:** First-Year Seminar or Advanced-Entry Seminar (4 credits)
- **Languages and Cultures:** Through third-semester proficiency (up to 12 credits)
- **FRA:** Fitness and Recreational Activity (1 credit)
- **Power and Justice (PJ):** 1 course, 3–4 credits — cannot be satisfied by transfer credit
- **Exploration and Breadth:** 6 courses outside major (18–24 credits): Part I — one course each from Fine Arts (FA), Humanities (H), Natural Sciences (NS), Social Sciences (ScS); Part II — one additional course from two of those four areas

Several Political Science courses carry Paideia designations (ScS), (SJ), (PJ), and (WA) and may satisfy Exploration and Breadth or Power and Justice requirements.

---

### B.A. in Political Science

| Field | Detail |
|---|---|
| Total Courses Required | 9 courses in Political Science |
| Minimum GPA (Overall) | 2.000 |
| Minimum GPA (Major) | 2.000 |
| Residency Requirement | At least 60% of major courses at SU |
| Source Document | `Majoring___Minoring.txt` |

**Required Core Courses:**
- PSC32-114 — American Politics (4 credits)
- PSC32-144 — Comparative Politics (4 credits)
- PSC32-364 — Introduction to Political Theory (4 credits)
- PSC32-384 — International Politics (4 credits)
- PSC32-964 — Senior Seminar / Capstone (credits per enrollment)

**Elective Requirements:**
- Two additional Political Science courses from any subfield (any level except 500–600 counting as the upper-level requirement)
- Two Political Science courses at the 500–600 level; at least one must be taken prior to the Senior Seminar

**Capstone / Culminating Requirement:**
PSC32-964 Senior Seminar — requires instructor permission. May be replaced by the Honors Thesis (PSC32-984 × 2 semesters) for students who receive and complete an invitation to Honors.

**Notable Constraints:**
- At least one 500–600 level course must be completed before enrolling in Senior Seminar
- 500–600 level courses have prerequisites: PSC32-364 or PSC32-384, or instructor permission
- Honors Thesis students take PSC32-951 (spring junior year, 1 cr) and PSC32-984 (×2 senior year, 4 cr each) in place of PSC32-964
- Texas Legislative Internship (PSC32-344) counts as no more than one internship toward the major; offered every other Spring

---

### Minor in Political Science

| Field | Detail |
|---|---|
| Total Courses Required | 5 courses in Political Science |
| Source Document | `Majoring___Minoring.txt` |

**Required Core Courses:**
- PSC32-114 — American Politics
- PSC32-144 — Comparative Politics
- PSC32-364 — Introduction to Political Theory
- PSC32-384 — International Politics
- One additional approved Political Science course

**Residency Requirement:** At least 12 credits at SU (per university policy)

---

### Key Differences Between Degree Paths

| Feature | B.A. Major | Minor |
|---|---|---|
| Total courses | 9 | 5 |
| 500–600 level courses | 2 required | Not required |
| Capstone/Seminar | PSC32-964 required | Not required |
| Electives | 2 additional courses | 1 additional approved course |
| Teacher certification | Via Education Dept | Via Education Dept |

The B.A. is the only degree path that leads to graduation with Honors in Political Science. The minor shares the same four core courses as the major and is a natural pathway for students double-majoring or adding Political Science as a complement to another field.

---

## SECTION 4 — COURSE INDEX

### PSC32-114 — American Politics
- **Credit Hours:** 4
- **Prerequisites:** None
- **Corequisites:** None
- **Offered:** Fall and Spring (multiple sections Fall 2024; confirmed in schedule)
- **Required For:** B.A. Major, Minor
- **Counts As:** Core requirement; Paideia (ScS)
- **Notes:** Serves as a civics course and introduction to American political theory and methods. Gateway course — required before most mid-level American Politics courses.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### PSC32-144 — Comparative Politics
- **Credit Hours:** 4
- **Prerequisites:** None
- **Corequisites:** None
- **Offered:** Fall and Spring (Spring 2024, Spring 2025 confirmed)
- **Required For:** B.A. Major, Minor
- **Counts As:** Core requirement; Paideia (ScS); contributes to International Studies
- **Notes:** Introductory survey of major political systems, Western and non-Western. Gateway to several 400–600 level comparative and IR courses.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### PSC32-184 — Politics of Latin America & Caribbean
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Spring (Spring 2024 confirmed as Dr. Selbin)
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS) (SJ) (PJ); contributes to International Studies, Latin American and Border Studies, Race and Ethnicity Studies/Allied Course
- **Notes:** Also accessible to students with prior regional knowledge.
- **Source:** `Courses.txt`

---

### PSC32-194 — Film, Literature and the Cold War
- **Credit Hours:** 4
- **Prerequisites:** None; open only to first-year and sophomore students (juniors/seniors may enroll with instructor permission)
- **Corequisites:** None
- **Offered:** Unknown (not listed in current schedule)
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS)
- **Notes:** Enrollment restriction: primarily for first-years and sophomores.
- **Source:** `Courses.txt`

---

### PSC32-204 — American Political Thought
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Spring 2025 (TBA instructor)
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS) (SJ) (PJ); contributes to Legal Studies Certificate
- **Notes:** Covers pre-founding era through twentieth century, American exceptionalism, democracy paradoxes.
- **Source:** `Courses.txt`

---

### PSC32-214 — Race and Ethnic Politics
- **Credit Hours:** 4
- **Prerequisites:** PSC32-114, Philosophy 18-354, or instructor approval
- **Corequisites:** None
- **Offered:** Unknown (not listed in current schedule by this title; note "Governing Diversity: Race & Ethnicity in American Politics" listed Spring 2024 under Goodwin may be a variant)
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to Environmental Studies, Race and Ethnicity Studies/Group-Theme Course, Legal Studies Certificate
- **Notes:** Covers African American, American Indian, Latina/o, and Asian American/Pacific Islander political experiences; draws on critical race theory.
- **Source:** `Courses.txt`

---

### PSC32-224 — Middle East Politics
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to International Studies
- **Notes:** Focuses on major Arab states, Israel, and Iran.
- **Source:** `Courses.txt`

---

### PSC32-234 — Sex, Power, and Politics
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to Feminist Studies
- **Notes:** Uses feminist political theory and intersectional approach.
- **Source:** `Courses.txt`

---

### PSC32-264 — Political Ideologies
- **Credit Hours:** 4
- **Prerequisites:** None; open only to first-year and sophomore students (juniors/seniors with instructor permission)
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS)
- **Notes:** Enrollment restriction: primarily for first-years and sophomores.
- **Source:** `Courses.txt`

---

### PSC32-284 — Japanese Politics, Culture & Society
- **Credit Hours:** 4
- **Prerequisites:** None; open only to first-year and sophomore students (juniors/seniors with instructor permission)
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to East Asian Studies, International Studies
- **Notes:** Enrollment restriction: primarily for first-years and sophomores.
- **Source:** `Courses.txt`

---

### PSC32-324 — Sex and the State
- **Credit Hours:** 4
- **Prerequisites:** PSC32-114 or PSC32-364, or instructor permission
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to Feminist Studies, Legal Studies Certificate
- **Notes:** Examines sex work, reproductive care, and sexual conduct through constitutional law and political theory.
- **Source:** `Courses.txt`

---

### PSC32-344 — Texas Legislative Politics Internship
- **Credit Hours:** 4
- **Prerequisites:** PSC32-114 and one additional Political Science course; 8 credits in Political Science; good academic and disciplinary standing
- **Corequisites:** None
- **Offered:** Every other Spring when Texas legislature is in session (Spring 2025 confirmed)
- **Required For:** Elective for major (counts as no more than one internship toward major)
- **Counts As:** Paideia (ScS)
- **Notes:** Work environment at Texas State Capitol under faculty supervision. Open to juniors and seniors. Only one internship can count toward the major.
- **Source:** `Courses.txt`

---

### PSC32-354 — Hip Hop and Mass Incarceration
- **Credit Hours:** 4
- **Prerequisites:** None (strongly encouraged to have taken PSC32-214 or Philosophy 18-184)
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to Race and Ethnicity Studies
- **Notes:** Strongly encouraged prerequisites, not required.
- **Source:** `Courses.txt`

---

### PSC32-364 — Introduction to Political Theory
- **Credit Hours:** 4
- **Prerequisites:** PSC32-114 or PSC32-144
- **Corequisites:** None
- **Offered:** Fall and Spring (Fall 2024 TBA; Spring 2025 confirmed, 2 sections)
- **Required For:** B.A. Major, Minor
- **Counts As:** Core requirement; Paideia (ScS) (WA) (SJ) (PJ); contributes to Legal Studies Certificate
- **Notes:** Writing Attentive (WA) designation. Gateway to most 500–600 level courses.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### PSC32-384 — International Politics
- **Credit Hours:** 4
- **Prerequisites:** PSC32-114 or PSC32-144
- **Corequisites:** None
- **Offered:** Fall and Spring (both semesters confirmed in schedule, Dr. Snyder, WA)
- **Required For:** B.A. Major, Minor
- **Counts As:** Core requirement; Paideia (WA) (ScS); contributes to International Studies
- **Notes:** Writing Attentive (WA) designation. Also a gateway to most 500–600 level courses.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### PSC32-394 — Research Methods in Political Science
- **Credit Hours:** 4
- **Prerequisites:** PSC32-114 or PSC32-144, or instructor permission
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to Data Science
- **Notes:** Covers research design, hypothesis testing, qualitative and quantitative data analysis. Contributes to Data Science program.
- **Source:** `Courses.txt`

---

### PSC32-414 — European Politics
- **Credit Hours:** 4
- **Prerequisites:** PSC32-144
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to International Studies
- **Notes:** Covers political cultures, structures, and European Union.
- **Source:** `Courses.txt`

---

### PSC32-444 — The Chinese Cultural Revolution
- **Credit Hours:** 4
- **Prerequisites:** PSC32-144, or instructor permission
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS); contributes to East Asian Studies, International Studies
- **Notes:** Seminar course on causes and effects of the Cultural Revolution.
- **Source:** `Courses.txt`

---

### PSC32-454 — Candidates, Campaigns, & Citizens
- **Credit Hours:** 4
- **Prerequisites:** PSC32-114
- **Corequisites:** None
- **Offered:** Fall and Spring (Fall 2024 and Spring 2024 confirmed, Dr. Sydnor)
- **Required For:** Elective for major
- **Counts As:** Paideia (ScS)
- **Notes:** Covers voting behavior, campaign strategies, and political engagement.
- **Source:** `Courses.txt`

---

### PSC32-504 — Advanced Topics in Political Science
- **Credit Hours:** 4
- **Prerequisites:** PSC32-364 or PSC32-384, or instructor permission
- **Corequisites:** None
- **Offered:** Varies by topic (may be repeated with change of topic)
- **Required For:** Counts toward two required 500–600 level courses for major
- **Counts As:** Paideia (ScS); 500–600 level requirement
- **Notes:** Repeatable with different topics. Contributes to prerequisite for Senior Seminar.
- **Source:** `Courses.txt`

---

### PSC32-544 — International Conflict
- **Credit Hours:** 4
- **Prerequisites:** PSC32-364 or PSC32-384, or instructor permission
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major (500–600 level)
- **Counts As:** Paideia (ScS); 500–600 level requirement; contributes to International Studies
- **Notes:** Substantial writing required.
- **Source:** `Courses.txt`

---

### PSC32-564 — Black Politics
- **Credit Hours:** 4
- **Prerequisites:** PSC32-364 or PSC32-384, or instructor permission
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major (500–600 level)
- **Counts As:** Paideia (ScS); 500–600 level requirement; contributes to Race and Ethnicity Studies/Group-Theme Course
- **Notes:** Covers development of African American politics from intellectual responses to systemic racism through electoral politics.
- **Source:** `Courses.txt`

---

### PSC32-584 — U.S. Foreign Policy
- **Credit Hours:** 4
- **Prerequisites:** PSC32-364 or PSC32-384, or instructor permission
- **Corequisites:** None
- **Offered:** Spring (Spring 2024 confirmed as 500–600 level Craft of Research, Dr. Snyder)
- **Required For:** Elective for major (500–600 level)
- **Counts As:** Paideia (ScS); 500–600 level requirement
- **Notes:** Focus on Cold War and post-Cold War foreign policy. Listed as "Craft of Research" 500–600 level in schedule.
- **Source:** `Courses.txt`

---

### PSC32-624 — Germany & Japan: Losers of World War II
- **Credit Hours:** 4
- **Prerequisites:** PSC32-364 or PSC32-384, or instructor permission
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major (500–600 level)
- **Counts As:** Paideia (ScS); 500–600 level requirement; contributes to East Asian Studies, International Studies
- **Notes:** Comparative study of democratic institutions, economic growth, and political culture in postwar Japan and Germany.
- **Source:** `Courses.txt`

---

### PSC32-634 — Resistance, Rebellion & Revolution
- **Credit Hours:** 4
- **Prerequisites:** PSC32-364 or PSC32-384, or instructor permission
- **Corequisites:** None
- **Offered:** Spring 2025 (Dr. Selbin, listed as 500–600 Craft of Research)
- **Required For:** Elective for major (500–600 level)
- **Counts As:** Paideia (ScS); 500–600 level requirement; contributes to International Studies
- **Notes:** Substantial research and writing component. "Craft of Research" designation.
- **Source:** `Courses.txt`

---

### PSC32-644 — Radical Democracy, Radical Activism
- **Credit Hours:** 4
- **Prerequisites:** PSC32-364, or instructor permission
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Elective for major (500–600 level)
- **Counts As:** Paideia (ScS); 500–600 level requirement; contributes to Feminist Studies, Legal Studies Certificate
- **Notes:** Uses activist archives (Redstockings, Black Panthers, SDS, ACT UP) to develop political theories of democracy.
- **Source:** `Courses.txt`

---

### PSC32-654 — Women and Politics in Europe and Asia
- **Credit Hours:** 4
- **Prerequisites:** PSC32-364 or PSC32-384, or instructor permission
- **Corequisites:** None
- **Offered:** Fall 2024 (Dr. Gaunder, listed as 500–600 Craft of Research)
- **Required For:** Elective for major (500–600 level)
- **Counts As:** Paideia (ScS) (SJ) (PJ); 500–600 level requirement; contributes to East Asian Studies, Feminist Studies, International Studies
- **Notes:** "Craft of Research" designation. Comparative study of women's political representation.
- **Source:** `Courses.txt`

---

### PSC32-964 — Senior Seminar (Capstone)
- **Credit Hours:** 4 (assumed; confirm with registrar)
- **Prerequisites:** Requires instructor permission; students must have completed at least one 500–600 level course prior to enrollment
- **Corequisites:** None
- **Offered:** Spring and Fall (confirmed each semester in schedule)
- **Required For:** B.A. Major (unless replaced by Honors Thesis)
- **Counts As:** Capstone requirement for major
- **Notes:** May be replaced by PSC32-984 (Honors Thesis) for students completing the Honors program. Permission of instructor required.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### PSC32-951 — Independent Study (Honors Proposal)
- **Credit Hours:** 1
- **Prerequisites:** Invitation to Honors program
- **Corequisites:** None
- **Offered:** Spring of junior year (for invited students)
- **Required For:** Honors Thesis track only
- **Counts As:** Honors thesis proposal development
- **Notes:** Taken during spring of junior year to develop Honors Thesis research proposal. Not a substitute for other major requirements.
- **Source:** `The_Honors_Thesis.txt`

---

### PSC32-984 — Honors Thesis
- **Credit Hours:** 4 (per semester; taken both semesters of senior year)
- **Prerequisites:** Departmental invitation and approved research proposal (PSC32-951 completed)
- **Corequisites:** None
- **Offered:** Both semesters of senior year
- **Required For:** Honors Thesis track (replaces PSC32-964 Capstone)
- **Counts As:** Capstone equivalent; qualifies student for "Honors in Political Science" distinction
- **Notes:** Must be enrolled for two full semesters. Requires committee of 3 members. Must present at academic conference or SU Student Works Symposium.
- **Source:** `The_Honors_Thesis.txt`

---

## SECTION 5 — TOPIC INDEX

- Political Science major overview → `Political_Science.txt`
- Why study political science → `Political_Science.txt`
- Career paths after political science degree → `Political_Science.txt`
- Alumni outcomes: law, government, nonprofits, education → `Political_Science.txt`
- Alumni: Sierra Rupp Critical Language Scholarship / Fulbright → `Political_Science.txt`
- Alumni: Alexandria Glenn / Zaria Renfro Gilman Scholarships → `Political_Science.txt`
- B.A. in Political Science requirements → `Majoring___Minoring.txt`
- Minor in Political Science requirements → `Majoring___Minoring.txt`
- Total courses required for the major → `Majoring___Minoring.txt`
- Total courses required for the minor → `Majoring___Minoring.txt`
- Core required courses for major and minor → `Majoring___Minoring.txt`
- PSC32-114 American Politics — required status → `Majoring___Minoring.txt`, `Courses.txt`
- PSC32-144 Comparative Politics — required status → `Majoring___Minoring.txt`, `Courses.txt`
- PSC32-364 Introduction to Political Theory — required status → `Majoring___Minoring.txt`, `Courses.txt`
- PSC32-384 International Politics — required status → `Majoring___Minoring.txt`, `Courses.txt`
- PSC32-964 Senior Seminar / Capstone — required status → `Majoring___Minoring.txt`, `Courses.txt`
- 500–600 level course requirements for major → `Majoring___Minoring.txt`
- Elective course options for the major → `Majoring___Minoring.txt`, `Courses.txt`
- Difference between major and minor → `Majoring___Minoring.txt`
- Teacher certification in Political Science → `Majoring___Minoring.txt`
- Full course catalog and descriptions → `Courses.txt`
- Course prerequisites — all courses → `Courses.txt`
- PSC32-184 Politics of Latin America & Caribbean → `Courses.txt`
- PSC32-194 Film, Literature and the Cold War → `Courses.txt`
- PSC32-204 American Political Thought → `Courses.txt`
- PSC32-214 Race and Ethnic Politics → `Courses.txt`
- PSC32-224 Middle East Politics → `Courses.txt`
- PSC32-234 Sex, Power, and Politics → `Courses.txt`
- PSC32-264 Political Ideologies → `Courses.txt`
- PSC32-284 Japanese Politics, Culture & Society → `Courses.txt`
- PSC32-324 Sex and the State → `Courses.txt`
- PSC32-344 Texas Legislative Internship → `Courses.txt`
- PSC32-354 Hip Hop and Mass Incarceration → `Courses.txt`
- PSC32-394 Research Methods in Political Science → `Courses.txt`
- PSC32-414 European Politics → `Courses.txt`
- PSC32-444 The Chinese Cultural Revolution → `Courses.txt`
- PSC32-454 Candidates, Campaigns, & Citizens → `Courses.txt`
- PSC32-504 Advanced Topics in Political Science → `Courses.txt`
- PSC32-544 International Conflict → `Courses.txt`
- PSC32-564 Black Politics → `Courses.txt`
- PSC32-584 U.S. Foreign Policy → `Courses.txt`
- PSC32-624 Germany & Japan: Losers of World War II → `Courses.txt`
- PSC32-634 Resistance, Rebellion & Revolution → `Courses.txt`
- PSC32-644 Radical Democracy, Radical Activism → `Courses.txt`
- PSC32-654 Women and Politics in Europe and Asia → `Courses.txt`
- PSC32-951 Independent Study (Honors Proposal) → `The_Honors_Thesis.txt`
- PSC32-984 Honors Thesis → `The_Honors_Thesis.txt`
- Courses contributing to International Studies → `Courses.txt`
- Courses contributing to Feminist Studies → `Courses.txt`
- Courses contributing to Legal Studies Certificate → `Courses.txt`
- Courses contributing to Race and Ethnicity Studies → `Courses.txt`
- Courses contributing to East Asian Studies → `Courses.txt`
- Courses contributing to Latin American and Border Studies → `Courses.txt`
- Courses contributing to Data Science → `Courses.txt`
- Courses contributing to Environmental Studies → `Courses.txt`
- Paideia (ScS) courses in poli sci → `Courses.txt`
- Paideia Power and Justice (PJ) courses in poli sci → `Courses.txt`
- Paideia Writing Attentive (WA) courses in poli sci → `Courses.txt`
- Social Justice (SJ) courses in poli sci → `Courses.txt`
- Courses open only to first-year and sophomore students → `Courses.txt`
- Enrollment restrictions by year/class standing → `Courses.txt`
- Texas Legislative Internship eligibility and schedule → `Courses.txt`, `For_Current_Students.txt`
- Internship credit toward the major (limit: 1) → `Courses.txt`
- Spring 2024 course offerings → `For_Current_Students.txt`
- Fall 2024 course offerings → `For_Current_Students.txt`
- Spring 2025 course offerings → `For_Current_Students.txt`
- Who is teaching a specific course → `For_Current_Students.txt`
- When is American Politics offered → `For_Current_Students.txt`
- When is Capstone offered → `For_Current_Students.txt`
- 500–600 "Craft of Research" courses and schedule → `For_Current_Students.txt`
- Faculty contact information → `Our_Faculty.txt`
- Professor Gaunder contact/specialty → `Our_Faculty.txt`
- Professor Goodwin contact/specialty → `Our_Faculty.txt`
- Professor Moss contact information → `Our_Faculty.txt`
- Professor Norris contact information → `Our_Faculty.txt`
- Professor Snyder contact/specialty → `Our_Faculty.txt`
- Faculty research specialties → `Our_Faculty.txt`
- Visiting faculty → `Our_Faculty.txt`
- Honors Thesis program overview → `The_Honors_Thesis.txt`
- Invitation criteria for Honors → `The_Honors_Thesis.txt`
- GPA requirement for Honors (3.4 overall) → `The_Honors_Thesis.txt`
- Honors Thesis timeline (junior/senior year) → `The_Honors_Thesis.txt`
- Honors Thesis vs. Capstone differences → `The_Honors_Thesis.txt`
- Honors Thesis committee structure → `The_Honors_Thesis.txt`
- Presenting at Student Works Symposium (Honors) → `The_Honors_Thesis.txt`
- Honors in Political Science graduation distinction → `The_Honors_Thesis.txt`
- Research proposal process for Honors → `The_Honors_Thesis.txt`
- Studying abroad while completing Honors Thesis → `The_Honors_Thesis.txt`
- Faculty awards and student recognition → `Notable_Faculty___Student_Achievements.txt`
- Graduation requirements (credit minimums, GPA) → `SU_General_Policies.txt`
- Paideia general education requirements → `SU_General_Policies.txt`
- Pass/D/F grading policy → `SU_General_Policies.txt`
- Academic probation and dismissal → `SU_General_Policies.txt`
- Transfer credit evaluation rules → `SU_General_Policies.txt`, `Majoring___Minoring.txt`
- Study abroad credit applicability → `SU_Resources_and_Financial.txt`, `SU_General_Policies.txt`
- Financial aid and tuition → `SU_Resources_and_Financial.txt`
- SURF and research funding → `SU_Resources_and_Financial.txt`
- Latin Honors (cum laude thresholds) → `SU_General_Policies.txt`

---

## SECTION 6 — QUERY PATTERN MAP

**Q:** What courses do I need to take to major in political science?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Document lists all 9 required courses for the B.A. including the 4 core, 2 electives, 2 upper-level 500–600, and capstone.
**Disambiguation Needed:** No

---

**Q:** What's the difference between the major and the minor?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Document explicitly lists both the 9-course major and 5-course minor requirements; differences are clear from comparison.
**Disambiguation Needed:** No

---

**Q:** I'm a freshman — what poli sci class should I take first?
**Docs:** `Courses.txt`, `Majoring___Minoring.txt`
**Routing Note:** PSC32-114 American Politics and PSC32-144 Comparative Politics have no prerequisites and are the required gateways for the major. Some introductory courses (PSC32-194, PSC32-264, PSC32-284) are open only to first-years and sophomores.
**Disambiguation Needed:** No

---

**Q:** Can I take Introduction to Political Theory before American Politics?
**Docs:** `Courses.txt`
**Routing Note:** PSC32-364 requires PSC32-114 or PSC32-144; routing model confirms prerequisite from course index.
**Disambiguation Needed:** No

---

**Q:** What are the 500-level courses available in political science?
**Docs:** `Courses.txt`, `For_Current_Students.txt`
**Routing Note:** Courses.txt lists all 500–600 level courses; For_Current_Students.txt shows which are offered in current/upcoming semesters.
**Disambiguation Needed:** No

---

**Q:** I want to add a poli sci minor — what do I still need to take?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Minor requires 5 courses (same 4 core + 1 approved elective); routing model compares with courses student reports having taken.
**Disambiguation Needed:** Yes — system should ask which courses the student has already completed.

---

**Q:** Who teaches International Politics?
**Docs:** `Our_Faculty.txt`, `For_Current_Students.txt`
**Routing Note:** Faculty file shows Snyder specializes in International Politics; schedule confirms he teaches it both semesters.
**Disambiguation Needed:** No

---

**Q:** I'm a junior — how do I get invited to do the Honors Thesis?
**Docs:** `The_Honors_Thesis.txt`
**Routing Note:** Document explains the invitation process, criteria (GPA 3.4+, unanimous faculty vote, demonstrated research passion), and timeline.
**Disambiguation Needed:** No

---

**Q:** How is the Honors Thesis different from the Capstone?
**Docs:** `The_Honors_Thesis.txt`
**Routing Note:** Document contains an explicit section comparing the two: Honors is year-long, graduate-level, by invitation; Capstone is one semester, required of all majors.
**Disambiguation Needed:** No

---

**Q:** Can I do the Honors Thesis if I'm studying abroad in the fall of my senior year?
**Docs:** `The_Honors_Thesis.txt`, `SU_Resources_and_Financial.txt`
**Routing Note:** Honors Thesis document notes students studying abroad during senior fall must still enroll in PSC32-984 for 4 credits; details of study abroad enrollment process are in Resources document.
**Disambiguation Needed:** No

---

**Q:** When is the Texas Legislative Internship offered?
**Docs:** `Courses.txt`, `For_Current_Students.txt`
**Routing Note:** Courses.txt states it is offered every other Spring when the Texas legislature is in session; For_Current_Students.txt confirms it is on the Spring 2025 schedule.
**Disambiguation Needed:** No

---

**Q:** What political science courses count toward the Legal Studies Certificate?
**Docs:** `Courses.txt`
**Routing Note:** Course descriptions list contributing programs; courses contributing to Legal Studies Certificate are PSC32-204, PSC32-214, PSC32-324, PSC32-364, PSC32-644.
**Disambiguation Needed:** No

---

**Q:** I transferred in with some political science credits — do they count toward the major?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Major residency requirement (60% of major credits at SU) is in general policies; Majoring document specifies the required courses; routing model must retrieve both.
**Disambiguation Needed:** Yes — system should ask which courses and from which institution.

---

**Q:** I failed American Politics — what are my options?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** PSC32-114 is required for the major; course repeat and grade replacement policies are in SU_General_Policies.txt; both documents needed.
**Disambiguation Needed:** No

---

**Q:** What courses can I take as a sophomore that are only open to underclassmen?
**Docs:** `Courses.txt`
**Routing Note:** PSC32-194, PSC32-264, and PSC32-284 are explicitly restricted to first-year and sophomore students.
**Disambiguation Needed:** No

---

**Q:** I want to go to law school — which poli sci courses should I take?
**Docs:** `Courses.txt`, `Political_Science.txt`
**Routing Note:** Political_Science.txt mentions law as a career path; Courses.txt shows which courses contribute to Legal Studies Certificate (relevant for pre-law students).
**Disambiguation Needed:** No

---

**Q:** What is Dr. Snyder's email address?
**Docs:** `Our_Faculty.txt`
**Routing Note:** Faculty document lists snyderb@southwestern.edu and office location Mood-Bridwell 228.
**Disambiguation Needed:** No

---

**Q:** Is there a course on race and politics?
**Docs:** `Courses.txt`
**Routing Note:** PSC32-214 Race and Ethnic Politics, PSC32-354 Hip Hop and Mass Incarceration, PSC32-564 Black Politics all cover race and politics topics.
**Disambiguation Needed:** No

---

**Q:** Do I need to take Research Methods for the poli sci major?
**Docs:** `Majoring___Minoring.txt`, `Courses.txt`
**Routing Note:** Majoring document does not list PSC32-394 as a core requirement; it counts as an elective. Routing model confirms from both documents.
**Disambiguation Needed:** No

---

**Q:** What poli sci courses count toward the Power and Justice Paideia requirement?
**Docs:** `Courses.txt`
**Routing Note:** Courses with (PJ) designation: PSC32-184, PSC32-204, PSC32-364, PSC32-654. Routing model identifies these from Paideia designations in course descriptions.
**Disambiguation Needed:** No

---

**Q:** Who teaches the Capstone?
**Docs:** `For_Current_Students.txt`
**Routing Note:** Schedule shows Capstone taught by Dr. Selbin (Spring 2024), Dr. Sydnor (Fall 2024 and Spring 2025).
**Disambiguation Needed:** Yes — ask which semester the student is planning to enroll.

---

**Q:** I'm considering a double major in poli sci and another subject — is that possible?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Double major policies, including paired major restrictions, are in SU_General_Policies.txt; Majoring document confirms the required coursework for the major.
**Disambiguation Needed:** Yes — system should note which other major to check for paired restrictions.

---

**Q:** How do I graduate with Honors in Political Science?
**Docs:** `The_Honors_Thesis.txt`, `SU_General_Policies.txt`
**Routing Note:** Honors Thesis document covers the departmental Honors process; SU_General_Policies.txt covers Latin Honors and university-wide honors distinctions. Both needed for a complete answer.
**Disambiguation Needed:** No

---

**Q:** What is the GPA requirement for the Honors Thesis?
**Docs:** `The_Honors_Thesis.txt`
**Routing Note:** Document states the university requires a 3.4 overall GPA; also notes GPA alone does not guarantee invitation.
**Disambiguation Needed:** No

---

**Q:** Are there any poli sci courses with a writing component?
**Docs:** `Courses.txt`
**Routing Note:** Courses with (WA) Writing Attentive designation: PSC32-364 and PSC32-384. PSC32-544 and PSC32-634 also note substantial writing required. Some 500–600 level courses are labeled "Craft of Research" in the schedule.
**Disambiguation Needed:** No

---

**Q:** I want to study women and politics — which courses should I take?
**Docs:** `Courses.txt`
**Routing Note:** PSC32-234, PSC32-324, PSC32-644, PSC32-654 all focus on gender/women and politics and contribute to Feminist Studies.
**Disambiguation Needed:** No

---

**Q:** What's the prerequisite for Black Politics?
**Docs:** `Courses.txt`
**Routing Note:** PSC32-564 requires PSC32-364 or PSC32-384, or instructor permission.
**Disambiguation Needed:** No

---

**Q:** I have a financial hold — can I still register for poli sci classes?
**Docs:** `SU_General_Policies.txt`
**Routing Note:** Financial holds and their impact on registration and diplomas are covered in the general policies document.
**Disambiguation Needed:** No

---

**Q:** Does Political Science have any study abroad connections?
**Docs:** `Political_Science.txt`, `SU_Resources_and_Financial.txt`
**Routing Note:** Political_Science.txt mentions alumni international scholarships; Resources document covers specific study abroad programs and credit transfer rules.
**Disambiguation Needed:** No

---

**Q:** What courses can I take in poli sci that count toward International Studies?
**Docs:** `Courses.txt`
**Routing Note:** Courses contributing to International Studies: PSC32-144, PSC32-184, PSC32-224, PSC32-284, PSC32-384, PSC32-414, PSC32-444, PSC32-544, PSC32-624, PSC32-634, PSC32-654.
**Disambiguation Needed:** No

---

**Q:** Can I take PSC32-504 Advanced Topics more than once?
**Docs:** `Courses.txt`
**Routing Note:** Course description explicitly states it may be repeated with a change in topic.
**Disambiguation Needed:** No

---

**Q:** When is Women and Politics in Europe and Asia offered?
**Docs:** `For_Current_Students.txt`, `Courses.txt`
**Routing Note:** PSC32-654 was offered Fall 2024 (Dr. Gaunder, 500–600 Craft of Research); schedule document confirms this.
**Disambiguation Needed:** No

---

**Q:** I'm considering the poli sci major — will it prepare me for grad school?
**Docs:** `Political_Science.txt`, `The_Honors_Thesis.txt`
**Routing Note:** Political_Science.txt discusses graduate program preparation; Honors Thesis document explicitly addresses how the thesis prepares students for doctoral programs.
**Disambiguation Needed:** No

---

**Q:** What is the "Craft of Research" label I see on some 500-level courses?
**Docs:** `For_Current_Students.txt`
**Routing Note:** "Craft of Research" is a label applied to certain 500–600 level offerings in the schedule (PSC32-584, PSC32-634, PSC32-654); these appear to indicate the 500–600 level designation for upper-level elective requirement.
**Disambiguation Needed:** No

---

**Q:** How many internship courses can count toward my poli sci major?
**Docs:** `Courses.txt`
**Routing Note:** PSC32-344 description states "No more than one internship can count toward the major."
**Disambiguation Needed:** No

---

## SECTION 7 — CROSS-REFERENCE FLAGS

- **Topic:** Graduation requirements (credit minimums, residency, GPA)
  - **What major docs cover:** 9-course major structure and specific course requirements
  - **What general docs must cover:** 127-credit minimum, 64-credit SU residency, last 32 in-residence, 2.000 GPA, 60% major residency, Application for Diploma process
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` alongside `Majoring___Minoring.txt` for any graduation eligibility question

- **Topic:** Paideia general education requirements
  - **What major docs cover:** Which poli sci courses satisfy Paideia areas (ScS, PJ, WA, SJ designations listed per course)
  - **What general docs must cover:** Full structure of FYS/AES, Language & Cultures, FRA, Power and Justice, Exploration & Breadth Parts I and II, Paideia with Distinction
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` for any question about gen-ed, breadth requirements, or Paideia

- **Topic:** Pass/D/F grading option
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Full Pass/D/F policy, deadlines to switch, GPA impact
  - **Routing instruction:** Retrieve `SU_General_Policies.txt` for any grading option question

- **Topic:** Academic probation, dismissal, and appeal
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Good Standing definition, Warning, Probation, Dismissal thresholds, Academic Standards Committee appeal process
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Transfer credit rules
  - **What major docs cover:** 60% residency for major implied by general university rule; no explicit transfer course list
  - **What general docs must cover:** C- minimum required; P/CR grades not accepted; ELEC-0XX/3XX coding; 60% major residency rule; AP/IB/CAPE/CLEP policies
  - **Routing instruction:** Retrieve both `Majoring___Minoring.txt` and `SU_General_Policies.txt`

- **Topic:** Study abroad credit and programs
  - **What major docs cover:** Alumni have participated in international programs (mentioned in Political_Science.txt); Honors Thesis document notes study abroad during senior fall is possible but student must still enroll in PSC32-984
  - **What general docs must cover:** SAISS process, approved programs (London, ISEP, NYAP, CHIP, CYA), deadlines, up to 19 transfer credits per semester, financial aid applicability
  - **Routing instruction:** Retrieve `SU_Resources_and_Financial.txt` for program details; retrieve `SU_General_Policies.txt` for credit transfer rules

- **Topic:** Financial aid impact of credit load changes or withdrawal
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Withdrawal and financial aid impact, tuition refund schedule, part-time financial aid thresholds
  - **Routing instruction:** Always retrieve `SU_Resources_and_Financial.txt`

- **Topic:** Tuition refund schedule
  - **What major docs cover:** Not covered
  - **What general docs must cover:** 80%/60%/50%/40%/30%/0% by week; summer refund schedule; room no-refund rule
  - **Routing instruction:** Always retrieve `SU_Resources_and_Financial.txt`

- **Topic:** Internship and experiential learning opportunities
  - **What major docs cover:** PSC32-344 Texas Legislative Internship described with eligibility and credit rules
  - **What general docs must cover:** Academic vs. funded internship distinction, SURF, King Creativity Fund, health professions shadowing (St. David's, Houston Methodist), NYAP, CHIP
  - **Routing instruction:** Retrieve `SU_Resources_and_Financial.txt` for general program details alongside `Courses.txt` for legislative internship specifics

- **Topic:** Latin Honors and Paideia with Distinction
  - **What major docs cover:** Honors Thesis document covers Departmental Honors in Political Science (3.4 GPA, unanimous invitation)
  - **What general docs must cover:** Cum Laude/Magna Cum Laude/Summa Cum Laude GPA thresholds, Dean's List criteria, Paideia with Distinction requirements and deadlines
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`; retrieve `The_Honors_Thesis.txt` for departmental honors specifics

- **Topic:** Disability accommodations and course substitution
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Services for Students with Disabilities, ADA accommodations, disability-related course substitution policy and appeals
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`; direct student to SSD office

- **Topic:** Double major policies and paired major restrictions
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Paired major restrictions (cannot minor in either discipline; cannot declare as double major if paired)
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Course repeat policies and GPA recalculation
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Course repeat rules and GPA recalculation
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Incomplete grade policies
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Incomplete grade policy, deadlines, conversion to F
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

---

## SECTION 8 — KNOWN GAPS

- **Topic:** Department chair / director of undergraduate studies contact
  - **Reason for gap:** `Our_Faculty.txt` does not identify a department chair by title
  - **Recommended fallback:** Direct student to the department's general phone number (512.863.6511) or advise them to email any listed faculty member

- **Topic:** Advising appointment process and frequency requirements
  - **Reason for gap:** Not mentioned in any provided document; SU general policy notes one advising meeting per semester required
  - **Recommended fallback:** Retrieve `SU_General_Policies.txt`; direct student to contact faculty advisor

- **Topic:** Office hours for individual faculty
  - **Reason for gap:** Not included in any provided document
  - **Recommended fallback:** Direct student to contact faculty via email listed in `Our_Faculty.txt`

- **Topic:** Specific content of Notable Faculty and Student Achievements
  - **Reason for gap:** File title suggests relevant content but could not be confirmed from provided context
  - **Recommended fallback:** Retrieve `Notable_Faculty___Student_Achievements.txt` directly

- **Topic:** Course offerings beyond Spring 2025
  - **Reason for gap:** `For_Current_Students.txt` only lists through Spring 2025
  - **Recommended fallback:** Direct student to check the registrar or department website for current schedule

- **Topic:** Independent study or directed readings options outside Honors
  - **Reason for gap:** PSC32-951 is mentioned only for Honors track; no general independent study policy described
  - **Recommended fallback:** Direct student to academic advisor

- **Topic:** Specific grading policies for poli sci courses (attendance, late work)
  - **Reason for gap:** Individual faculty syllabi not included
  - **Recommended fallback:** Direct student to course syllabus or instructor

- **Topic:** Waitlist procedures for closed poli sci courses
  - **Reason for gap:** Not covered in any document
  - **Recommended fallback:** Direct student to registrar's office

- **Topic:** How to declare or officially add the Political Science major or minor
  - **Reason for gap:** Not covered in provided documents
  - **Recommended fallback:** Direct student to registrar's office or academic advisor

- **Topic:** Specific content of Dr. Gaunder's or Dr. Moss's courses (no schedule entry for Moss)
  - **Reason for gap:** `For_Current_Students.txt` does not list Moss as instructor for any scheduled course; Gaunder only appears in Fall 2024 Women and Politics
  - **Recommended fallback:** Direct student to contact faculty directly

- **Topic:** AP/IB credit that could satisfy poli sci course requirements
  - **Reason for gap:** Not addressed in major documents; general policies cover AP/IB credit
  - **Recommended fallback:** Retrieve `SU_General_Policies.txt`; advise student to contact the registrar for equivalency evaluation

---

## SECTION 9 — ROUTING DECISION GUIDE

**Default document for broad major questions:** `Majoring___Minoring.txt`

**When to retrieve multiple documents:**
- Any question about specific courses (prerequisite, content, schedule): retrieve `Courses.txt` AND `For_Current_Students.txt`
- Any graduation eligibility or credit count question: retrieve `Majoring___Minoring.txt` AND `SU_General_Policies.txt`
- Any Honors Thesis question that also involves study abroad: retrieve `The_Honors_Thesis.txt` AND `SU_Resources_and_Financial.txt`
- Any question about faculty expertise combined with course offerings: retrieve `Our_Faculty.txt` AND `For_Current_Students.txt`
- Any transfer credit question: retrieve `Majoring___Minoring.txt` AND `SU_General_Policies.txt`
- Any double major or minor combination question: retrieve `Majoring___Minoring.txt` AND `SU_General_Policies.txt`

**Degree path disambiguation triggers:**
- This major only offers a B.A. (no B.S.); no disambiguation needed between degree types
- If student says "the political science degree" assume B.A.
- If student asks about "the minor" proceed directly — only one minor path exists
- If student asks about "Honors" — confirm whether they mean Latin Honors (GPA-based, general policy) or Departmental Honors in Political Science (invitation-based, `The_Honors_Thesis.txt`)

**High-confidence single-document retrievals:**
- "What are the poli sci major requirements?" → `Majoring___Minoring.txt`
- "What's the prerequisite for [specific course]?" → `Courses.txt`
- "Who is Professor Snyder / Goodwin / Gaunder / Moss / Norris?" → `Our_Faculty.txt`
- "How does the Honors Thesis work?" → `The_Honors_Thesis.txt`
- "What courses are offered this semester?" → `For_Current_Students.txt`
- "Why should I major in poli sci? / What can I do with this major?" → `Political_Science.txt`

**Always escalate to human advisor:**
- Requests for a course substitution or waiver from required courses
- Academic appeals or petitions (failed courses, probation)
- Exceptions to the Honors Thesis proposal process
- Questions about whether a specific transfer course will count toward the major
- Questions about the student's individual degree audit or progress toward graduation
- Any financial aid or hold dispute

**Always retrieve `SU_General_Policies.txt` alongside major docs when:**
- Any graduation eligibility or credit count question
- Any Paideia / general education requirement question
- Any grading policy question (Pass/D/F, grade appeals, incomplete grades)
- Any academic standing question (probation, dismissal, appeal)
- Any transfer credit or credit-by-examination question
- Any disability accommodation question
- Any honors or distinction question (Latin Praise, Departmental Honors, Paideia with Distinction)

**Always retrieve `SU_Resources_and_Financial.txt` alongside major docs when:**
- Any study abroad program question (deadlines, eligibility, credit transfer)
- Any financial aid or tuition question
- Any refund or withdrawal-and-money question
- Any internship funding question (SURF, King Creativity Fund, funded internships)
- Any health professions experience question (St. David's, Houston Methodist)

---

*End of skills_index.md*

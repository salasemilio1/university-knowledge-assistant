# Skills Index — Religion
**Institution:** Southwestern University, Georgetown, Texas
**Last Ingested:** 2026-03-31
**Ingestion Model:** Claude Sonnet 4.6
**Document Count:** 5
**Degree Paths Covered:** B.A., Minor

---

## SECTION 2 — DOCUMENT REGISTRY

### Religion Program Overview
- **degree_relevance:** All
- **description:** Provides a high-level introduction to the Religion program at Southwestern University. Describes the academic discipline of religious studies as inherently interdisciplinary, drawing on philosophy, art, history, sociology, anthropology, psychology, language studies, literature, politics, economics, cultural studies, gender studies, and ethnic studies. Identifies Elaine Craddock as Professor of Religion and Department Chair. Notes that the program introduces students to a variety of global religious traditions with the goal of creating empathetic understanding of differences. Briefly references alumni outcomes ("Cultivating a Global Perspective") and links to Religion News. Does not list specific degree requirements or course offerings.
- **critical_data:**
  - Religion is an academic discipline at SU using critical analysis across many fields
  - Department Chair: Elaine Craddock, Professor of Religion
  - Program goal: empathetic understanding of global religious differences
  - Alumni pursue dual degree master's programs at world-renowned institutions

---

### Majoring and Minoring in Religion
- **degree_relevance:** B.A., Minor
- **description:** Specifies the complete course requirements for both the Religion major (9 courses) and the Religion minor (5 courses). For the major, students must complete REL19-814 Doing Religion, REL19-894 Religion Capstone (taken fall of senior year), three courses chosen from a world traditions group (Christian, Native American, Jewish, Islamic, Hindu, Buddhist), and four courses chosen from a thematic/upper-level elective pool. For the minor, students must complete REL19-814 Doing Religion plus four additional religion courses, at least two of which must be 300-level or higher. Notes that students may choose a paired major between Religion and Feminist Studies. Does not specify credit hour counts per course or GPA minimums beyond what is implied by SU policies.
- **critical_data:**
  - Major requires 9 courses total
  - REL19-814 Doing Religion: required for both major and minor
  - REL19-894 Religion Capstone: required for major; taken fall of senior year
  - Three courses from world traditions group (REL19-204, 214, 224, 244, 274, 284)
  - Four courses from thematic elective pool (REL19-304, 324, 334, 344, 354, 364, 374, 384, 394, 404, 414, 714)
  - Minor requires 5 courses: REL19-814 + four religion courses (at least two at 300-level or higher)
  - Paired major option available: Religion and Feminist Studies
  - Paired major restriction: cannot be declared as a double major (see cross-reference flags)

---

### Religion Courses
- **degree_relevance:** B.A., Minor
- **description:** Lists all current or recent course offerings in the Religion department with full descriptions and Paideia designation codes. Covers 21 named courses plus one cross-listed course (Muslims in Europe, see History 16-514). World traditions survey courses include Christian Traditions (19-204), Native American Traditions (19-214), Jewish Traditions (19-224), Islamic Traditions (19-244), Hindu Traditions (19-274), and Buddhist Traditions (19-284). Thematic upper-level courses include Selected Topics (19-304), Women Goddesses and Religion (19-324), Apocalypse and Dystopia (19-334), Animals and Religion (19-344), Gender and Sexuality in Native America (19-354), Pilgrimage (19-364), Sacred Space and the Environment (19-374), Rastas Saints and Virgins (19-384), Indigenous Myth and Narrative (19-394), Religion and Animation (19-404), The Body in Religion (19-414), Indigenous TV and Film (19-424), Muslims in Europe (19-534), Topics in Religion (19-714), Doing Religion (19-814), and Religion Capstone (19-894). Notes which courses are offered in specific semesters (Doing Religion in Spring; Capstone in Fall) and which carry Paideia designations (H, SJ, PJ, WA). Notes interdisciplinary contributions to programs such as International Studies, Feminist Studies, Environmental Studies, Anthropology, Race and Ethnicity Studies, East Asian Studies, Legal Studies, and Animal Studies.
- **critical_data:**
  - 21 courses listed plus one cross-listed (Muslims in Europe)
  - REL19-814 Doing Religion: offered Spring; carries (WA) designation; required for major and minor; research-intensive writing course
  - REL19-894 Religion Capstone: offered Fall; carries (WA) designation; required for major; open to others with instructor permission
  - World traditions courses: 19-204 (H, SJ, PJ), 19-214 (H, SJ, PJ), 19-224 (H), 19-244 (H), 19-274 (H, SJ, PJ), 19-284 (H, SJ, PJ)
  - Courses with (WA): 19-324, 19-814, 19-894
  - Courses with (PJ): 19-204, 19-214, 19-274, 19-284, 19-324, 19-334, 19-344, 19-384
  - 19-304 Selected Topics and 19-714 Topics in Religion may be repeated with change of topic
  - Muslims in Europe (19-534) cross-listed with History 16-514

---

### Our Faculty
- **degree_relevance:** All
- **description:** Lists the four faculty members in the Religion department with their titles, specialties, contact information, and office locations. Elaine Craddock is Professor of Religion and Department Chair, specializing in Hinduism, Feminism and Religion, Tamil literature, and Religion and the Body (craddoce@southwestern.edu, 512.863.1963). Laura Hobgood is Professor of Religion, specializing in animals in the Christian tradition, human-dog history, religion and ecology, ecofeminism, and human-animal studies (hobgood@southwestern.edu, 512.863.1669, Mood-Bridwell 136). Ken Mello is Associate Professor of Religion, specializing in Native American traditions, Contemporary Indigenous Identity, Sacred Space, Gender and Sexuality, and Indigenous Myth and Narratives (mellok@southwestern.edu, 512.863.1964, Mood-Bridwell 138). Sarah Summers is listed as Part-Time Instructor/Professor (summers3@southwestern.edu) with no listed specialty or office location.
- **critical_data:**
  - Elaine Craddock: Department Chair; Hinduism, Feminism, Tamil literature, Religion and the Body; craddoce@southwestern.edu; 512.863.1963
  - Laura Hobgood: Animals and religion, ecofeminism, religion and ecology; hobgood@southwestern.edu; 512.863.1669; Mood-Bridwell 136
  - Ken Mello: Native American traditions, sacred space, gender/sexuality; mellok@southwestern.edu; 512.863.1964; Mood-Bridwell 138
  - Sarah Summers: Part-time instructor; summers3@southwestern.edu

---

### Notable Faculty and Student Achievements
- **degree_relevance:** All
- **description:** Chronicles faculty accomplishments from September 2020 through March 2026. Professor Laura Hobgood is the most frequently featured faculty member, with ongoing media appearances, publications, documentary filmmaking, board service, and invited lectures related to animals, religion, and ecology. Professor Elaine Craddock published peer-reviewed articles in the International Journal of Hindu Studies (March 2024) and Feminist Review (December 2023) and has expertise in Tamil Nadu communities and caste/class/religion intersections. Part-Time Assistant Professor Andrea Gutiérrez won the 2021 DK Award for Outstanding Doctoral Thesis on Sanskrit. Professor Hobgood contributed chapters to Animals and Religion (Routledge, 2024), Religion and Nature in North America (Bloomsbury, 2024), and other edited volumes, and was featured in The Atlantic (2021) and on NPR. The Animals and Religion Group at the American Academy of Religion, co-founded by Dr. Hobgood, celebrated 25 years of activity in 2025.
- **critical_data:**
  - Laura Hobgood: featured on Relationscapes podcast (March 2026); AAR panel (December 2025); documentary interview "Immortality Key" (2024); chapters in Routledge and Bloomsbury volumes (2024); NPR appearances; Atlantic feature (2021)
  - Elaine Craddock: article in International Journal of Hindu Studies (March 2024); article in Feminist Review (December 2023)
  - Andrea Gutiérrez: 2021 DK Award for Outstanding Doctoral Thesis on Sanskrit (international prize, awarded once every 3 years)
  - Animals and Religion Group at AAR co-founded by Hobgood; 25 years active as of 2025
  - Hobgood joined Board of Directors of Education Congo (June 2023)

---

## SECTION 3 — DEGREE PATH SUMMARIES

### SU Universal Graduation Requirements (All Degree Paths)

These apply to ALL students regardless of major. Source: `SU_General_Policies.txt`

| Requirement | Detail |
|---|---|
| Minimum Total Credits | 127 credits |
| Minimum SU Residency Credits | 64 credits (last 32 must be in-residence) |
| Minimum Overall GPA | 2.000 (both cumulative and SU-only) |
| Minimum Major GPA | 2.000 average (no grade below C- counts toward major) |
| Major Residency Requirement | At least 60% of major credits at SU |
| Minor Residency Requirement | At least 12 credits at SU (if minor pursued) |
| Graduation Application | "Application for Diploma" must be filed; financial holds block diploma |
| Degree Conferral Dates | December, May, August only; Commencement held once per year in May |

### SU Universal Curriculum Requirements (Paideia Framework)

SU's general education model is called **Paideia**. Religion courses satisfy several Paideia areas:

- **(H) Humanities:** Most Religion courses carry this designation
- **(SJ) Social Justice:** Several Religion courses carry this designation
- **(PJ) Power and Justice:** REL19-204, 214, 274, 284, 324, 334, 344, 384 carry this designation
- **(WA) Writing Across the Curriculum:** REL19-324, 814, 894 carry this designation

Full Paideia framework details are in `SU_General_Policies.txt`.

---

### B.A. in Religion (Major)

| Field | Detail |
|---|---|
| Total Courses Required | 9 courses |
| Source Document | `Majoring___Minoring.txt` |

**Required Core Courses:**
- REL19-814 — Doing Religion (required; offered Spring; WA designation)
- REL19-894 — Religion Capstone (required; taken fall of senior year; WA designation)

**Required World Traditions Courses (choose 3 of 6):**
- REL19-204 — Christian Traditions
- REL19-214 — Native American Traditions
- REL19-224 — Jewish Traditions
- REL19-244 — Islamic Traditions
- REL19-274 — Hindu Traditions
- REL19-284 — Buddhist Traditions

**Required Thematic Electives (choose 4 of the following pool):**
- REL19-304 — Selected Topics
- REL19-324 — Women, Goddesses and Religion
- REL19-334 — Apocalypse and Dystopia
- REL19-344 — Animals and Religion
- REL19-354 — Gender and Sexuality in Native America
- REL19-364 — Pilgrimage
- REL19-374 — Sacred Space and the Environment
- REL19-384 — Rastas, Saints and Virgins
- REL19-394 — Indigenous Myth and Narrative
- REL19-404 — Religion and Animation
- REL19-414 — The Body in Religion
- REL19-714 — Topics in Religion

**Additional Graduation Requirements:**
- Religion Capstone (REL19-894) must be taken in the fall of senior year
- REL19-814 Doing Religion is research-intensive and carries significant writing requirements

**Notable Constraints:**
- Paired major option available with Feminist Studies; paired majors cannot be declared as separate double majors
- No explicit GPA minimum for the major stated in department documents (SU-wide 2.000 minimum applies)

---

### Minor in Religion

| Field | Detail |
|---|---|
| Total Courses Required | 5 courses |
| Source Document | `Majoring___Minoring.txt` |

**Required:**
- REL19-814 — Doing Religion

**Elective Courses (4 additional religion courses):**
- At least 2 must be 300-level or higher
- Any Religion-prefix courses may count (subject to standard SU policies)

**Residency Requirement:**
- At least 12 credits must be completed at SU (per SU-wide minor policy; source: `SU_General_Policies.txt`)

---

### Key Differences Between Degree Paths

| Factor | B.A. Major | Minor |
|---|---|---|
| Total Courses | 9 | 5 |
| Capstone Required | Yes (REL19-894, fall senior year) | No |
| World Traditions Requirement | 3 of 6 surveys | Not specified |
| Thematic Electives | 4 from pool | 4 total (at least 2 at 300-level) |
| Paired Major Option | Yes (with Feminist Studies) | N/A |

**Career and graduate school guidance:** The documents note that a Religion major prepares students for graduate work in liberal arts fields generally, is an excellent complementary major for double majors, and is suitable for those pursuing seminary education.

---

## SECTION 4 — COURSE INDEX

### REL19-204 — Christian Traditions
- **Credit Hours:** 4 (per SU numbering convention — last digit = credits)
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown (not specified)
- **Required For:** B.A. major (world traditions group, choose 3 of 6)
- **Counts As:** World Traditions requirement; Paideia (H) (SJ) (PJ)
- **Notes:** Contributes to International Studies. Surveys Jesus movement through contemporary Christianity in Global South.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-214 — Native American Traditions
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (world traditions group, choose 3 of 6)
- **Counts As:** World Traditions requirement; Paideia (H) (SJ) (PJ)
- **Notes:** Contributes to Anthropology, Race and Ethnicity Studies/Group-Theme Course, and Legal Studies Certificate. Multi-disciplinary approach.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-224 — Jewish Traditions
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (world traditions group, choose 3 of 6)
- **Counts As:** World Traditions requirement; Paideia (H)
- **Notes:** Contributes to International Studies. Surveys Hebrew Bible formation through modern era.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-244 — Islamic Traditions
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (world traditions group, choose 3 of 6)
- **Counts As:** World Traditions requirement; Paideia (H)
- **Notes:** Contributes to International Studies. Covers mysticism, gender, politics in diverse cultural contexts.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-274 — Hindu Traditions
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (world traditions group, choose 3 of 6)
- **Counts As:** World Traditions requirement; Paideia (H) (SJ) (PJ)
- **Notes:** Contributes to Feminist Studies and International Studies. Covers Indian subcontinent religious ideas, interaction with Buddhism/Jainism/Islam/Sikhism, gender and politics.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-284 — Buddhist Traditions
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (world traditions group, choose 3 of 6)
- **Counts As:** World Traditions requirement; Paideia (H) (SJ) (PJ)
- **Notes:** Contributes to East Asian Studies and International Studies. Covers historical Buddha through spread to North America; includes ethics and gender.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-304 — Selected Topics
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective
- **Notes:** May be repeated with change in topic. Lectures and readings on subjects of special interest.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-324 — Women, Goddesses and Religion
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H) (WA) (SJ) (PJ)
- **Notes:** Contributes to Anthropology, Feminist Studies, and International Studies. Cross-cultural; feminist methodology. WA designation means significant writing.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-334 — Apocalypse and Dystopia
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H) (SJ) (PJ)
- **Notes:** Contributes to Environmental Studies and International Studies. Examines end-times imagination in religious and secular contexts; social justice and environmental concerns.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-344 — Animals and Religion
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H) (SJ) (PJ)
- **Notes:** Contributes to Animal Studies, Environmental Studies, Feminist Studies, and International Studies. Ecofeminist and environmental methods. Examines factory farming and biomedical experimentation.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-354 — Gender and Sexuality in Native America
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H)
- **Notes:** Contributes to Anthropology and Feminist Studies. Examines gender and sexuality in Native American ritual and spiritual life, past and present.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-364 — Pilgrimage
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H)
- **Notes:** Contributes to Anthropology, Environmental Studies, and International Studies. Comparative and critical; covers sacred space, pilgrims' motivations, environmental impact, and tourism.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-374 — Sacred Space and the Environment
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H)
- **Notes:** Contributes to Environmental Studies and Race and Ethnicity Studies/Allied Course. Focus on Native American spiritual relationship to landscape and nature.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-384 — Rastas, Saints and Virgins
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H) (SJ) (PJ)
- **Notes:** Contributes to Anthropology, International Studies, and Race and Ethnicity Studies/Group-Theme Course. Covers Santeria, Rastafarians, Cult of the Virgin of Guadalupe and other ethnic religious traditions in the US.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-394 — Indigenous Myth and Narrative
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H)
- **Notes:** Contributes to Race and Ethnicity Studies/Group-Theme Course. Examines oral and written Native American narratives; includes audio recordings, music, poetry, and social media.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-404 — Religion and Animation
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H)
- **Notes:** No interdisciplinary contributions listed. Covers comics, graphic novels, cartoons, and films (Avengers, The Simpsons, South Park, Disney).
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-414 — The Body in Religion
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H)
- **Notes:** Contributes to Feminist Studies and International Studies. Feminist, cross-cultural examination of embodiment, sexuality, gender, and salvation.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-424 — Indigenous TV and Film
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as a major elective option in Majoring___Minoring.txt (see gap note)
- **Counts As:** Paideia (H)
- **Notes:** Examines mainstream media portrayal of indigenous peoples from North America, past and present. Covers representation, stereotypes, and identity formation.
- **Source:** `Courses.txt`

---

### REL19-534 — Muslims in Europe
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown
- **Required For:** Not listed as a major elective option
- **Counts As:** Paideia (H)
- **Notes:** Cross-listed with History 16-514. Students should confirm which prefix to register under.
- **Source:** `Courses.txt`

---

### REL19-714 — Topics in Religion
- **Credit Hours:** 4
- **Prerequisites:** None stated
- **Corequisites:** None
- **Offered:** Unknown; specific topics rotate (not necessarily every two years)
- **Required For:** B.A. major (thematic elective pool, choose 4)
- **Counts As:** Thematic elective; Paideia (H)
- **Notes:** May be repeated when topic varies. Rotating topics include: Ball Games, Baskets, and Living Skies; A Novel Approach to American Religious History; Yoga; Religion and Politics; Religion and Ecology. Critical investigation of religion and violence, media, authority, politics.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-814 — Doing Religion
- **Credit Hours:** 4
- **Prerequisites:** None stated (open to all students)
- **Corequisites:** None
- **Offered:** Spring
- **Required For:** B.A. major (required core); Minor (required)
- **Counts As:** Required core for both major and minor; Paideia (WA)
- **Notes:** Research-intensive; requires significant writing and application of methodological approaches. Reviews secular theories and methods (history, literary studies, feminist studies, psychology, sociology, anthropology, postcolonial, environmental, comparative studies).
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

### REL19-894 — Religion Capstone
- **Credit Hours:** 4
- **Prerequisites:** Permission of instructor for non-majors
- **Corequisites:** None
- **Offered:** Fall
- **Required For:** B.A. major (required; taken fall of senior year)
- **Counts As:** Capstone/culminating requirement; Paideia (WA)
- **Notes:** Research seminar intended for religion majors; open to others with instructor permission. Must be taken in fall of senior year.
- **Source:** `Courses.txt`, `Majoring___Minoring.txt`

---

---

## SECTION 6 — QUERY PATTERN MAP

**Q:** What courses do I need to take to major in Religion?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Complete major requirements (9 courses: core + world traditions group + thematic electives) are fully described in this document.
**Disambiguation Needed:** No

**Q:** What's the difference between the Religion major and the minor?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Both degree paths are described in the same document; the distinction (9 courses vs. 5 courses, capstone required for major, 300-level requirement for minor) is explicit.
**Disambiguation Needed:** No

**Q:** I'm a freshman — where do I start if I'm interested in Religion?
**Docs:** `Majoring___Minoring.txt`, `Courses.txt`, `Our_Faculty.txt`
**Routing Note:** Incoming students benefit from seeing major requirements, available courses, and faculty contact information. REL19-814 Doing Religion (offered Spring) is the required gateway course.
**Disambiguation Needed:** No; recommend they review major requirements and contact the department chair (Craddock)

**Q:** I'm thinking about a Religion major but also want to do Feminist Studies — can I?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Majoring___Minoring.txt mentions the Religion/Feminist Studies paired major option; SU_General_Policies.txt covers paired major restrictions (cannot be declared individually as a double major).
**Disambiguation Needed:** No; but answer should clarify the paired major restriction

**Q:** Which religion courses count for the Power and Justice Paideia requirement?
**Docs:** `Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Courses.txt lists (PJ) designations; SU_General_Policies.txt explains the Power and Justice requirement structure.
**Disambiguation Needed:** No

**Q:** I'm a sophomore trying to decide whether to add Religion as a second major — is it a lot of extra courses?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Majoring___Minoring.txt shows 9-course major; SU_General_Policies.txt covers double major policies and how shared courses count.
**Disambiguation Needed:** No; answer should note that Religion is described as an excellent complementary major

**Q:** Can I take Doing Religion in the fall?
**Docs:** `Courses.txt`
**Routing Note:** REL19-814 Doing Religion is explicitly listed as offered in Spring only.
**Disambiguation Needed:** No

**Q:** When should I take the Religion Capstone?
**Docs:** `Courses.txt`, `Majoring___Minoring.txt`
**Routing Note:** Both documents confirm the capstone is offered in Fall and must be taken in the fall of senior year.
**Disambiguation Needed:** No

**Q:** I'm a junior — can I add the Religion minor?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Majoring___Minoring.txt lists minor requirements (5 courses, REL19-814, at least 2 at 300-level); SU_General_Policies.txt covers minor residency and registration policies.
**Disambiguation Needed:** No; but advise student to meet with Religion department advisor

**Q:** What religion courses are available about Native American topics?
**Docs:** `Courses.txt`
**Routing Note:** Multiple courses cover Native American traditions: REL19-214 (survey), REL19-354 (gender/sexuality), REL19-374 (sacred space), REL19-394 (myth/narrative), REL19-424 (TV/film).
**Disambiguation Needed:** No

**Q:** I'm interested in animals and religion — what can I take?
**Docs:** `Courses.txt`, `Notable_Faculty___Student_Achievements.txt`
**Routing Note:** REL19-344 Animals and Religion is directly relevant; Notable Achievements doc provides context on Professor Hobgood's expertise.
**Disambiguation Needed:** No

**Q:** Does the Religion major have any writing-intensive courses?
**Docs:** `Courses.txt`
**Routing Note:** REL19-324 (WA), REL19-814 (WA), and REL19-894 (WA) all carry Writing Across the Curriculum designations.
**Disambiguation Needed:** No

**Q:** I'm a senior — what do I need to do to graduate with a Religion major?
**Docs:** `Majoring___Minoring.txt`, `Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Must confirm completion of 9 required courses including REL19-894 Capstone (fall of senior year); graduation application process and diploma requirements are in SU_General_Policies.txt.
**Disambiguation Needed:** No; but remind student to file Application for Diploma and check for financial holds

**Q:** I transferred in with some religion courses — do they count toward the major?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** SU_General_Policies.txt governs transfer credit (C- minimum, P/CR not accepted, 60% major residency); Majoring___Minoring.txt shows what the major requires. Both needed.
**Disambiguation Needed:** Yes — which specific courses did the student transfer in?

**Q:** I failed Doing Religion — can I retake it?
**Docs:** `Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Courses.txt confirms REL19-814 is required; SU_General_Policies.txt covers course repeat policies and GPA recalculation.
**Disambiguation Needed:** No

**Q:** What Paideia requirements does the Religion major help me satisfy?
**Docs:** `Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Courses.txt lists (H), (SJ), (PJ), and (WA) designations per course; SU_General_Policies.txt explains the full Paideia framework including Exploration and Breadth Parts I and II.
**Disambiguation Needed:** No

**Q:** Who teaches Native American traditions in the Religion department?
**Docs:** `Our_Faculty.txt`
**Routing Note:** Ken Mello's specialty includes Native American traditions; Our_Faculty.txt provides contact information.
**Disambiguation Needed:** No

**Q:** What is Professor Hobgood known for?
**Docs:** `Our_Faculty.txt`, `Notable_Faculty___Student_Achievements.txt`
**Routing Note:** Our_Faculty.txt lists specialties; Notable Achievements provides full publication and media history.
**Disambiguation Needed:** No

**Q:** Can I take Topics in Religion more than once?
**Docs:** `Courses.txt`
**Routing Note:** REL19-714 Topics in Religion explicitly states it may be repeated when the topic varies.
**Disambiguation Needed:** No

**Q:** Does the Religion department have anything related to environmental studies?
**Docs:** `Courses.txt`, `Our_Faculty.txt`
**Routing Note:** Multiple courses (REL19-334, 344, 364, 374) contribute to Environmental Studies; both Hobgood and Craddock have environmental-related specialties.
**Disambiguation Needed:** No

**Q:** I have a financial hold on my account — can I still register for religion classes?
**Docs:** `SU_General_Policies.txt`
**Routing Note:** Financial hold impacts are a university-wide policy; SU_General_Policies.txt covers holds and their impact on registration, transcripts, and diplomas.
**Disambiguation Needed:** No

**Q:** Is there a religion course about Islam?
**Docs:** `Courses.txt`
**Routing Note:** REL19-244 Islamic Traditions and REL19-534 Muslims in Europe (cross-listed with History) both cover Islam.
**Disambiguation Needed:** No

**Q:** Does the Religion major require any science courses?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Religion major documents list no science requirements within the major; SU_General_Policies.txt clarifies that B.S. (not available in Religion) requires science; B.A. does not add science beyond Paideia Exploration and Breadth.
**Disambiguation Needed:** No

**Q:** Can I take the Religion Capstone if I'm not a Religion major?
**Docs:** `Courses.txt`
**Routing Note:** REL19-894 is open to non-majors with permission of the instructor.
**Disambiguation Needed:** No

**Q:** What does "Doing Religion" cover?
**Docs:** `Courses.txt`
**Routing Note:** Full course description in Courses.txt; covers theories and methods of secular religious studies across multiple disciplines.
**Disambiguation Needed:** No

**Q:** Are there study abroad options that would count toward the Religion major?
**Docs:** `Majoring___Minoring.txt`, `SU_Resources_and_Financial.txt`, `SU_General_Policies.txt`
**Routing Note:** Majoring___Minoring.txt specifies major requirements; SU_Resources_and_Financial.txt covers study abroad programs (London, ISEP, NYAP, CHIP, CYA); SU_General_Policies.txt covers transfer credit rules and 60% residency requirement.
**Disambiguation Needed:** Yes — what program is the student considering and which religion course would substitute?

**Q:** How do I contact the Religion department?
**Docs:** `Our_Faculty.txt`, `Religion.txt`
**Routing Note:** Our_Faculty.txt lists all faculty emails and phone numbers; Elaine Craddock (craddoce@southwestern.edu, 512.863.1963) is Department Chair.
**Disambiguation Needed:** No

**Q:** What is the Religion and Animation course about?
**Docs:** `Courses.txt`
**Routing Note:** REL19-404 Religion and Animation is fully described in Courses.txt with examples (Avengers, Ned Flanders, South Park).
**Disambiguation Needed:** No

**Q:** Is Pilgrimage a 300-level course?
**Docs:** `Courses.txt`
**Routing Note:** REL19-364 Pilgrimage — per SU numbering convention, digits 3-4 are "36" placing it in the 300-level range.
**Disambiguation Needed:** No

**Q:** What awards have Religion faculty won recently?
**Docs:** `Notable_Faculty___Student_Achievements.txt`
**Routing Note:** Andrea Gutiérrez won the 2021 DK Award for Outstanding Doctoral Thesis on Sanskrit; Hobgood's many honors and publications documented through 2026.
**Disambiguation Needed:** No

**Q:** I'm thinking about seminary after graduation — is Religion a good major?
**Docs:** `Majoring___Minoring.txt`
**Routing Note:** Majoring___Minoring.txt explicitly states the Religion major is good for those seeking seminary education.
**Disambiguation Needed:** No

**Q:** Does the Religion minor satisfy any Paideia requirements?
**Docs:** `Courses.txt`, `SU_General_Policies.txt`
**Routing Note:** Individual Religion courses satisfy various Paideia areas (H, SJ, PJ, WA) as noted in Courses.txt; SU_General_Policies.txt explains how these apply to the Paideia framework.
**Disambiguation Needed:** No

**Q:** What's the difference between Selected Topics (19-304) and Topics in Religion (19-714)?
**Docs:** `Courses.txt`
**Routing Note:** Both are repeatable topics courses; 19-304 is described as lectures and readings on special interest subjects; 19-714 is a critical investigation at a higher level. Both are in the thematic elective pool for the major.
**Disambiguation Needed:** No

**Q:** Is there a grade minimum to stay in the Religion major?
**Docs:** `Majoring___Minoring.txt`, `SU_General_Policies.txt`
**Routing Note:** Major documents do not specify a major GPA minimum explicitly; SU_General_Policies.txt covers the university-wide 2.000 major GPA requirement and academic standing thresholds.
**Disambiguation Needed:** No

**Q:** I want to study gender and religion — what courses should I take?
**Docs:** `Courses.txt`
**Routing Note:** Multiple courses address gender: REL19-324 Women Goddesses and Religion; REL19-354 Gender and Sexuality in Native America; REL19-414 The Body in Religion; REL19-274 Hindu Traditions (gender issues noted).
**Disambiguation Needed:** No

---

## SECTION 7 — CROSS-REFERENCE FLAGS

- **Topic:** Graduation requirements (credit minimums, residency, GPA)
  - **What major docs cover:** 9-course major requirement; minor requirements
  - **What general docs must cover:** 127-credit minimum, 64-credit SU residency, last 32 in-residence, 2.000 GPA, 60% major residency, Application for Diploma process
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` alongside major requirements doc for any graduation eligibility question

- **Topic:** Paideia general education requirements
  - **What major docs cover:** Which Religion courses carry Paideia designations (H, SJ, PJ, WA)
  - **What general docs must cover:** Full structure of FYS/AES, Language & Cultures, FRA, Power and Justice, Exploration & Breadth Parts I and II, Paideia with Distinction
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` for any question about gen-ed, breadth requirements, or Paideia

- **Topic:** Pass/D/F grading option
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Full Pass/D/F policy, deadlines to switch, GPA impact
  - **Routing instruction:** Retrieve `SU_General_Policies.txt` for any grading option question

- **Topic:** Academic probation, dismissal, and appeal
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Good Standing definition, Warning, Probation, Dismissal thresholds, Academic Standards Committee appeal process
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Transfer credit rules
  - **What major docs cover:** Not specified; major residency (60%) implied by SU policy
  - **What general docs must cover:** C- minimum required; P/CR grades not accepted; ELEC-0XX/3XX coding; 60% major residency rule; AP/IB/CAPE/CLEP policies
  - **Routing instruction:** Retrieve both `Majoring___Minoring.txt` and `SU_General_Policies.txt`

- **Topic:** Study abroad credit and programs
  - **What major docs cover:** Not addressed
  - **What general docs must cover:** SAISS process, approved programs (London, ISEP, NYAP, CHIP, CYA), deadlines, up to 19 transfer credits per semester, financial aid applicability
  - **Routing instruction:** Retrieve `SU_Resources_and_Financial.txt` for program details; retrieve `SU_General_Policies.txt` for credit transfer rules

- **Topic:** Financial aid impact of credit load changes or withdrawal
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Withdrawal and financial aid impact, tuition refund schedule, part-time financial aid thresholds
  - **Routing instruction:** Always retrieve `SU_Resources_and_Financial.txt`

- **Topic:** Tuition refund schedule
  - **What major docs cover:** Not covered
  - **What general docs must cover:** 80%/60%/50%/40%/30%/0% by week; summer refund schedule
  - **Routing instruction:** Always retrieve `SU_Resources_and_Financial.txt`

- **Topic:** Internship and experiential learning opportunities
  - **What major docs cover:** Not mentioned in Religion documents
  - **What general docs must cover:** SURF, King Creativity Fund, health professions shadowing, NYAP, CHIP
  - **Routing instruction:** Retrieve `SU_Resources_and_Financial.txt` for program details

- **Topic:** Latin Honors and Paideia with Distinction
  - **What major docs cover:** Not mentioned
  - **What general docs must cover:** Cum Laude/Magna Cum Laude/Summa Cum Laude GPA thresholds, Dean's List criteria, Paideia with Distinction requirements and deadlines
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`

- **Topic:** Disability accommodations and course substitution
  - **What major docs cover:** Not covered
  - **What general docs must cover:** Services for Students with Disabilities, ADA accommodations, disability-related course substitution policy and appeals
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt`; direct student to SSD office

- **Topic:** Paired major restriction (Religion and Feminist Studies)
  - **What major docs cover:** `Majoring___Minoring.txt` mentions the paired major is available but does not explain restrictions
  - **What general docs must cover:** Paired majors cannot be declared individually as a double major; students cannot minor in either discipline of a paired major
  - **Routing instruction:** Always retrieve `SU_General_Policies.txt` for any paired major question

- **Topic:** Course repeat policy
  - **What major docs cover:** Not covered (though 19-304 and 19-714 explicitly allow repeating with topic change)
  - **What general docs must cover:** General course repeat policy and GPA recalculation rules
  - **Routing instruction:** Retrieve `SU_General_Policies.txt`

---

## SECTION 8 — KNOWN GAPS

- **Topic:** Credit hours per individual course (confirmed count)
  - **Reason for gap:** No course descriptions in Courses.txt explicitly state credit hours; credit hours are inferred from SU's last-digit naming convention (e.g., 19-814 = 4 credits)
  - **Recommended fallback:** Direct student to the official SU course catalog for confirmed credit hour counts or advise them to verify with the registrar

- **Topic:** Specific GPA minimum for the Religion major
  - **Reason for gap:** Not stated in department documents; SU-wide 2.000 minimum applies
  - **Recommended fallback:** Direct student to `SU_General_Policies.txt` and their academic advisor

- **Topic:** Whether REL19-424 Indigenous TV and Film counts toward the Religion major thematic elective pool
  - **Reason for gap:** REL19-424 appears in Courses.txt but is NOT listed in the thematic elective pool in Majoring___Minoring.txt; may be an omission
  - **Recommended fallback:** Direct student to department advisor (Craddock or Mello) for confirmation

- **Topic:** Prerequisite requirements for any Religion course
  - **Reason for gap:** No prerequisites are listed for any Religion course in the provided documents
  - **Recommended fallback:** Check the official SU course catalog or contact the department; some upper-level courses may have instructor approval requirements

- **Topic:** Typical course offerings schedule (which courses run which semesters)
  - **Reason for gap:** Only REL19-814 (Spring) and REL19-894 (Fall) have confirmed semester offerings; all others are unknown
  - **Recommended fallback:** Direct student to the current SU course schedule or department advisor

- **Topic:** Maximum number of times REL19-304 Selected Topics may be repeated
  - **Reason for gap:** Document says "may be repeated with change in topic" without specifying a limit
  - **Recommended fallback:** Confirm with academic advisor whether multiple credits from repeated Selected Topics can count toward the four thematic elective slots

- **Topic:** Whether REL19-534 Muslims in Europe counts toward the Religion major
  - **Reason for gap:** REL19-534 is cross-listed with History 16-514 but is not listed in the major elective pool in Majoring___Minoring.txt
  - **Recommended fallback:** Confirm with Religion department advisor

- **Topic:** Advising appointment requirements or process
  - **Reason for gap:** No information about advising appointments in any provided Religion document
  - **Recommended fallback:** Direct student to academic advisor; SU_General_Policies.txt may address general advising requirements

- **Topic:** Whether Religion courses taken Pass/D/F count toward the major
  - **Reason for gap:** Not addressed in department documents
  - **Recommended fallback:** Retrieve `SU_General_Policies.txt`; direct student to advisor

- **Topic:** Religion department's specific policies on incomplete grades, late drops, or withdrawals from required courses
  - **Reason for gap:** Not covered in any provided Religion document
  - **Recommended fallback:** Retrieve `SU_General_Policies.txt`; direct student to registrar

- **Topic:** Specific faculty course assignments (which professor teaches which course)
  - **Reason for gap:** Our_Faculty.txt lists specialties but does not specify which courses each professor currently teaches
  - **Recommended fallback:** Direct student to current SU course schedule or contact the department

---

## SECTION 9 — ROUTING DECISION GUIDE

**Default document for broad major questions:** `Majoring___Minoring.txt`

**When to retrieve multiple documents:**
- Any question about Paideia credit or gen-ed satisfaction: retrieve `Courses.txt` AND `SU_General_Policies.txt`
- Any graduation eligibility or diploma question: retrieve `Majoring___Minoring.txt` AND `SU_General_Policies.txt`
- Any transfer credit question: retrieve `Majoring___Minoring.txt` AND `SU_General_Policies.txt`
- Any study abroad question: retrieve `SU_Resources_and_Financial.txt` AND `SU_General_Policies.txt`
- Any paired major (Religion/Feminist Studies) question: retrieve `Majoring___Minoring.txt` AND `SU_General_Policies.txt`
- Any question about a specific course's Paideia designation: retrieve `Courses.txt` AND `SU_General_Policies.txt`
- Any faculty research or contact question: retrieve `Our_Faculty.txt`; if publication history needed, also retrieve `Notable_Faculty___Student_Achievements.txt`

**Degree path disambiguation triggers:**
- Religion offers only B.A. and Minor — no B.S. option exists; if a student asks about "the Religion B.S." clarify that only B.A. and Minor are available
- If a student says "the Religion degree" without specifying major vs. minor, ask which they mean before routing

**High-confidence single-document retrievals:**
- "What courses do I need for the Religion major?" → `Majoring___Minoring.txt`
- "What is the Religion minor?" → `Majoring___Minoring.txt`
- "Tell me about [specific course name or number]" → `Courses.txt`
- "Who teaches [topic] in Religion?" → `Our_Faculty.txt`
- "What has Professor [name] published?" → `Notable_Faculty___Student_Achievements.txt`
- "What is Doing Religion about?" → `Courses.txt`
- "When is the capstone offered?" → `Courses.txt`

**Always escalate to human advisor:**
- Requests for exceptions to the major requirements (e.g., substituting a non-listed course for a thematic elective)
- Whether a specific transferred religion course counts toward the major or minor
- Course repeat limits for Selected Topics or Topics in Religion counting toward the major
- Whether REL19-424 or REL19-534 count toward the major (gap identified above)
- Academic appeals or GPA petitions
- Any financial aid decision or appeal

**Always retrieve `SU_General_Policies.txt` alongside major docs when:**
- Any graduation eligibility or credit count question
- Any Paideia / general education requirement question
- Any grading policy question (Pass/D/F, grade appeals, incomplete grades)
- Any academic standing question (probation, dismissal, appeal)
- Any transfer credit or credit-by-examination question
- Any disability accommodation question
- Any honors or distinction question (Latin Praise, Departmental Honors, Paideia with Distinction)
- Any paired major restriction question

**Always retrieve `SU_Resources_and_Financial.txt` alongside major docs when:**
- Any study abroad program question (deadlines, eligibility, credit transfer)
- Any financial aid or tuition question
- Any refund or withdrawal-and-money question
- Any internship funding question (SURF, King Creativity Fund, funded internships)
- Any health professions experience question (St. David's, Houston Methodist)
